...
!     Last change: msg 11/11/2018 12:28:24 PM
     ...
     +                                         R_EXE_DATE,EXE_TIME,SIZE)
         ...
         BUILD_NUM = "1.0.0.107 " // trim(BUILD_NUM_STR)
      RETURN
      END function BUILD_NUM
...
! SHORT FORM ITEMS
!
      integer :: file_trace_mm=0 ! Tracefile handle
...
!  END COMMON BLOCK SECTION FOR THE PRODUCTION MODULES
     ...
     +              BASE_FILE_FAMILY, cwd
      LOGICAL*1 TRANSACT_ANALYST_ONLY,
     ...
     +          SET_ASSET_ANALYST_ONLY_SWITCH,RUN_COAL_MODEL_ONLY
      ...
      logical :: poss_opts_exist
      character(len=2) :: cap_planning_method

        ! Put interface statement here because Lahey didn't
        ! like it in a module.
...
! last variable used 658-659 9/28/98 MSG CASH CS DIVIDENDS,
! PS_DIVIDEND_CASH_PAYMENTS
! last variable used 660-663 10/2/98 MSG ACCOUNTS RECEIVABLE,
! FUEL INVENTORY,
...
! last variable used 666 10/21/98 MSG Invectment in acquisitions
      ...
      if (file_trace_mm==0) then
        file_trace_mm=open_trace("midasmod.trace", rq_mm)
      end if
...
! SET SOME PARAMETERS
      ...
      CALL START_DISPLAY_TIME
       ...
       if (trim(base_file_definition)=="") then
        call end_program("midasmod:0004 - " //
     ...
     + trim(base_file_family) == "NONE") then
      ...
      CHR_RETURN_VALUE = DSEXEC_OBJECT(POINTS_IN_LOAD_CURVE)
        ...
        if(trim(cldata%scename)=="2023-   0") then
           call end_program("midasmod:0006 - study period string " // 
     +     "is incorrect.")
         end if
...
! OPEN AND READ DSS FILES FILE
      ...
      call SET_STARTING_RECORD_4_UNIT38()
      gc_end_point = 1
      GRX_SAVED_ENDPOINT = gc_end_point

! IF THE BASE CASE EXISTS THE OVERHEAD FINANCIAL CALCULATIONS ARE
! NOT DONE
...
! START FINANCIAL DRILLING REPORT
            ...
            ! very slow routine NEED ELIMINATION CLASS B4 ASSET ANALYSIS
            ...
            CALL FINASSET(.TRUE.)
            IF(.NOT. WVPA()) then
                CALL ProSymInputData(.TRUE.)  ! Processes base file
            end if
...
! ENTRY POINT FOR THE ENDPOINT LOOP
         ...
         poss_opts_exist=POSSIBLE_OPTIONS_EXIST()
         cap_planning_method=CAPACITY_PLANNING_METHOD()
         call write_trace_string(file_trace_mm, "SCENAME", 
     + cldata%SCENAME)
         IF(poss_opts_exist .AND. cap_planning_method == 'AN') THEN
            call write_trace_message(file_trace_mm, 
     +           "Calling MIDAS_ANN_DECOMP_SIMULATION")
     
            CALL MIDAS_ANN_DECOMP_SIMULATION(cldata%SCENAME)
         ELSE
           if(get_globecom_study_period()==0) then
      call end_program("midasmod:0005 - globecom_study_period is zero.")
           end if

            call write_trace_message(file_trace_mm, 
     +           "1. Calling MIDAS_NORMAL_SIMULATION")
     
            I2_RETURN_VALUE = MIDAS_NORMAL_SIMULATION(cldata%SCENAME)
         ENDIF
      ELSE
         call write_trace_message(file_trace_mm, 
     +           "2. Calling MIDAS_NORMAL_SIMULATION")
         I2_RETURN_VALUE = MIDAS_NORMAL_SIMULATION(cldata%SCENAME)
      ENDIF
      CALL DISPLAY_TIME
 ...
 1000 FORMAT('&',A,A,A,A)
     ...
     +                    SP_CAPEX_ACTIVE
      ...
      INTEGER*2 FA_UNIT/44/,NEW_ADDITIONS_FA/444/
         ...
         ! Reset call John's interested in right now
         CALL RESET_FUASTOL
         CALL RESET_ESCAL_OL
...
!     A SUBROUTINE TO INTEGRATE THE PROB CURVE TO FIND THE ENERGY
     ...
     +                      POINTS_IN_CURRENT_CURVE,POINTS_IN_LOAD_CURVE
      INTEGER*2 ,S_LAST_VARIABLE,
     +          STORE_LAST_VARIABLE

      INTEGER*2 LAST_VARIABLE,gc_max_years
...
!  COMMON BLOCK VARIABLES FOR /WORLD/
      ...
      INTEGER*2 BASE_YEAR_COMMON,MAXIMUM_YEARS
      ...
      COMMON /DATA_DRIVE_LOCATION/ DATA_DRIVE
         ...
         BASE_YEAR_COMMON = get_BASE_YEAR()
      ...
      ENTRY STORE_LAST_VARIABLE(S_LAST_VARIABLE)
         call set_last_variable(S_LAST_VARIABLE)

         STORE_LAST_VARIABLE = get_last_variable()
      RETURN
      END
     ...
     +                                                  DAYS_BY_MONTH(I)
      ...
      RETURN
      END FUNCTION SET_CALENDAR_INFORMATION
     ...
     +      BAL_LONG_MONTH_NAMES,LONG_MONTH_NAMES
      ...
      INTEGER (KIND=2) :: TEMP_YR,R_YR,R_MO
      ...
      ENTRY MONTH_YEAR_FORMATT(R_MO,R_YR)
         ...
         TEMP_YR = get_BASE_YEAR()+R_YR-2000
...
! THESE MAKEBINs AND DYNAMIC VECTOR ALLOCATIONS FOR CP&L MUST BE
! RUN BEFORE THE ASSET VECTOR ROUTINES 4/7/99 MSG

! CONVERT THE RPS PROGRAMS FILE TO BINARY
...
! REFERENCE LOAD
!
      ! First call is successful, but second call fails
...
C OPEN THE ESCALATION RATE DATA FILE
...
! ROUTINE WILL THE MESSAGES IN IT
      ...
      ENTRY STOP_NOFILE(FILE_TYPE,FILE_NAME, key)


         CALL MG_LOCATE_WRITE(20,0,'The '//trim(FILE_TYPE)//
     +             ' base file '//trim(FILE_NAME)//' does not exist.',
     +                                                   ALL_VERSIONS,1)
      er_message= trim(file_name) // ' missing at  ' // trim(key)
      call end_program(er_message)
      END
...
!
     ...
     +  DIMENSIONS,FIRST_YEAR,
     +  MIN(get_ENDYR(),FIRST_YEAR+INT2(4)),
     +  GET_NUM_OF_END_POINTS()
            NEXT_REC = 2
         ELSE
     ...
     +                                                   VALUES_OUT(I-1)
       ...
       ! Added to protect against "wild" implied escalation rates
         ...
         ELSE
            IF(DATA_TYPE == 'R') THEN
               DO I = 1, MIN(YEARS,AVAIL_DATA_YEARS)
                  IF(get_BASE_YEAR()+I >= ESCAL_VECT) THEN
                     VALUES_OUT(I) = VALUES_OUT(I-1) *
     +                                            (1.+VALUES_IN(I)/100.)
                  ELSE
            ...
            ELSEIF(DATA_TYPE == 'V') THEN
               ...
               DO I = 1, MIN(YEARS,AVAIL_DATA_YEARS)
                  IF(get_BASE_YEAR()+I < YR_2_CHANGE_2_VECTOR
     +             .OR. YR_2_CHANGE_2_VECTOR <= get_BASE_YEAR()) THEN
                     VALUES_OUT(I) = VALUES_IN(I)
                  ELSE
                     IF(get_BASE_YEAR()+I == ESCAL_VECT) THEN
     ...
     +                 R_CLASS_ID
      ...
      CHARACTER (LEN=38) R_ASSET_CLASS_NAME
         ...
         INCOME_ITEMS(:,globecom_YEAR,:) = R_INCOME_VARS(:,:)
...
! Merge2Issue: (probably ok) Left side uses mtly_depcn_and_amortz and right
! side uses mty_depn_n_amortn.  Using right-side value.
     +            R_INCOME_VARS(:,mty_depn_n_amortn)
...
!  read the base year 12 months
     ...
     +                                        TRAILING_12_MONTHS
      ...
      ENTRY BASE_YEAR_ACTUAL_INCOME_DATA()
         ...
         IF(gc_end_point > 1) RETURN  ! THE ACTUAL DATA HAS BEEN READ
         IF(IPALCO()) THEN
     ...
     +                                         IPL_REG_EXP_DATA(MO,YR,1)
                     INCOME_ITEMS(MO,YR,
! Merge2Issue: (probably ok) Left side used mtly_depcn_and_amortz but right side
! used mty_depn_n_amortn. Using right-side choice.
     +                           mty_depn_n_amortn) =
