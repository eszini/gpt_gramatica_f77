...
module OVLS_DATA
    ...
    implicit none
      ...
      ! TODO: Make number_of_data_files data-driven.
      INTEGER (KIND=2) , PARAMETER ::  NUMBER_OF_DATA_FILES=388
    ! Overlay filenames here so they can be written 
    ! as a list by MSGOLD95RW - jtr
    ! OFN is a mirro of OVERLAY_FILE_NAME(:)
    CHARACTER (len=5) :: OFN(NUMBER_OF_DATA_FILES)=""
    contains
    subroutine fill_overlays(ovls)
        character (len=5) :: ovls(NUMBER_OF_DATA_FILES)
        integer :: i
        do i=1, NUMBER_OF_DATA_FILES
            if(trim(ovls(i))/="NONE") then
                ofn(i)=ovls(i)
            end if
        end do
        
        
    end subroutine fill_overlays
    subroutine print_overlays()
        integer :: i, lasti, lt
        CHARACTER (len=5) :: OF(NUMBER_OF_DATA_FILES) ! for debugger
        of=ofn
        lasti=ubound(ofn,1)
        call write_log_entry("OVL", "Overlays follow...")
        do i = 1, lasti
            if(trim(ofn(i))/="NONE") then
                lt=len_trim(ofn(i))
                if(index(ofn(i), "-")/=lt) then
                    ! This one has a name.
                    call write_log_entry("OVL " // trim(itos(i)), trim(ofn(i)))
                end if
            end if
        end do
        call write_log_entry("OVL", "Overlays complete.")
        call flush_log_entry()

        
    end subroutine print_overlays
    
end module OVLS_DATA

!
      RECURSIVE SUBROUTINE FINDOVLS(OVLNAME,MATCH_FOUND,ep_loc,   &
                                    RUN_FUTURE_ASSETS)
      use OVLS_DATA
      use spindriftlib
      use prosymmodule
	  ...
	  integer (kind=2) :: ep_loc
!
! NOTE THAT FILE NUMBER 7 IS USED FOR EXPENSE AND REVENUE NAMES BUT IS 
! NOT "OVERLAYABLE" 11/27/01 MSG
!
      LOGICAL (kind=1) ::  RUN_FUTURE_ASSETS,CLASS_BASED_FORECAST,WVPA
!
      ...
      INTEGER (kind=2) ::  I,END_POINT_ord,RETURN_CODE,ISERROR
      ...
      CHARACTER (len=5) ::  FAMILY_NAMES(:),OVERLAY_FILES_IN_FAMILY(:,:), &
          OVERLAY_FILE_NAMES(0:16)
      ALLOCATABLE :: FAMILY_NAMES,OVERLAY_FILES_IN_FAMILY, &
          OVERLAY_FILE_EXISTS,OVERLAY_FAMILY_ACTIVE
...
!
      CHARACTER (len=5) :: OVERLAY_FILE_NAME(NUMBER_OF_DATA_FILES), &
          R_OVERLAY_FILE_NAMES(NUMBER_OF_DATA_FILES)
      ...
      CHARACTER (LEN=5) :: COAL_NODE_FILE,  & !  354
                  ...
                  COAL_MODEL_GENE_TRANS_FILE,  & !  374
...
!
      ...
      LOGICAL (kind=1) :: ENERGY_PRODUCTS_FILE_P2_EXISTS,   &
                ...
                COAL_SUPPLY_FCST_FILE_EXISTS, & !  356
                COAL_DEMAND_FCST_FILE_EXISTS, & !  357
...
!
      EQUIVALENCE (FILE_EXISTS(1),EAE_EXISTS), &
          (FILE_EXISTS(2),DDE_EXISTS), &
          (FILE_EXISTS(3),DBE_EXISTS), &
          (FILE_EXISTS(4),FAE_EXISTS), &
          (FILE_EXISTS(5),NFE_EXISTS), &
          (FILE_EXISTS(6),EPE_EXISTS),(FILE_EXISTS(7), &
          PRA_EXISTS),(FILE_EXISTS(8),PRF_EXISTS), &
          (FILE_EXISTS(9),INE_EXISTS),(FILE_EXISTS(10), &
          TAX_EXISTS),(FILE_EXISTS(11),INT_EXISTS), &
          (FILE_EXISTS(12),FIN_EXISTS),(FILE_EXISTS(13), &
          ESC_EXISTS),(FILE_EXISTS(14),FOS_EXISTS), &
          (FILE_EXISTS(15),HYD_EXISTS),(FILE_EXISTS(16), &
          UPE_EXISTS),(FILE_EXISTS(17),SFC_EXISTS), &
          (FILE_EXISTS(18),CFC_EXISTS),(FILE_EXISTS(19), &
          RFC_EXISTS),(FILE_EXISTS(20),IFC_EXISTS), &
          (FILE_EXISTS(21),O1F_EXISTS),(FILE_EXISTS(22),O2F_EXISTS), &
          (FILE_EXISTS(23),O3F_EXISTS), &
          (FILE_EXISTS(24),CLS_EXISTS),(FILE_EXISTS(25),CLH_EXISTS), &
          (FILE_EXISTS(26),LMG_EXISTS), &
          (FILE_EXISTS(27),LMS_EXISTS),(FILE_EXISTS(28),PFD_EXISTS), &
          (FILE_EXISTS(29),PNP_EXISTS), &
          (FILE_EXISTS(30),FNP_EXISTS),(FILE_EXISTS(31),CNP_EXISTS), &
          (FILE_EXISTS(32),CRA_EXISTS), &
          (FILE_EXISTS(33),COS_EXISTS),(FILE_EXISTS(34), &
          REFERENCE_LOAD_DATA_EXISTS),(FILE_EXISTS(35),LFI_EXISTS), &
          (FILE_EXISTS(36),ENV_EXISTS),(FILE_EXISTS(37),ECO_EXISTS), &
          (FILE_EXISTS(38),ASTRT_EXISTS), &
          (FILE_EXISTS(39),ASTVEC_EXISTS),(FILE_EXISTS(40),CNT_EXISTS), &
          (FILE_EXISTS(41),PRODP_EXISTS), &
          (FILE_EXISTS(42),AREA_EXISTS),(FILE_EXISTS(43), &
          FUEL_INVENTORY_EXISTS),(FILE_EXISTS(44),FUEL_PRICE_EXISTS), &
          (FILE_EXISTS(45),SERVICE_TRANSACTIONS_EXISTS), &
          (FILE_EXISTS(46),FUEL_EMISSIONS_FILE_EXISTS), &
          (FILE_EXISTS(47),OVER_UNDER_TABLE_EXISTS),(FILE_EXISTS(48), &
          CLASS_RUN_SPECS_EXISTS), &
          (FILE_EXISTS(49),CLASS_REVENUE_FORECAST_EXISTS), &
          (FILE_EXISTS(50),ELIMINATIONS_EXISTS), &
          (FILE_EXISTS(51),DETAILED_REPORTS_EXISTS),(FILE_EXISTS(52), &
          NEW_RUN_SPECS_EXISTS), &
          (FILE_EXISTS(53),CATAWBA2_CONTRACT_EXISTS),(FILE_EXISTS(54), &
          UNIT_OUTAGE_EXISTS), &
          (FILE_EXISTS(55),TRANSACT_TIE_EXISTS),(FILE_EXISTS(56), &
          MONTHLY_PAYABLES_EXISTS), &
          (FILE_EXISTS(57),DAY_TYPE_EXISTS),(FILE_EXISTS(58), &
          SCENARIO_MAKER_EXISTS), &
          (FILE_EXISTS(59),ENERGY_PRODUCTS_EXISTS),(FILE_EXISTS(60), &
          MARKET_PRICE_EXISTS), &
          ...
          (FILE_EXISTS(356),COAL_SUPPLY_FCST_FILE_EXISTS),   &
          (FILE_EXISTS(357),COAL_DEMAND_FCST_FILE_EXISTS),   &
      ...
      EQUIVALENCE (OVERLAY_FILE_NAME(1),EAE_FILE),(OVERLAY_FILE_NAME(2), &
        DDE_FILE),(OVERLAY_FILE_NAME(3),DBE_FILE),   &
        (OVERLAY_FILE_NAME(4),FAE_FILE),(OVERLAY_FILE_NAME(5), &
        NFE_FILE),(OVERLAY_FILE_NAME(6),EPE_FILE),   &
        (OVERLAY_FILE_NAME(7),EXPENSE_NAMES_RESERVED), &
        (OVERLAY_FILE_NAME(8),PRF_FILE),(OVERLAY_FILE_NAME(9),INE_FILE),  &
        (OVERLAY_FILE_NAME(10),TAX_FILE),(OVERLAY_FILE_NAME(11), &
        INT_FILE),(OVERLAY_FILE_NAME(12),FIN_FILE),   &
        (OVERLAY_FILE_NAME(13),ESC_FILE),(OVERLAY_FILE_NAME(14), &
        FOS_FILE),(OVERLAY_FILE_NAME(15),HYD_FILE),   &
        (OVERLAY_FILE_NAME(16),UPE_FILE),(OVERLAY_FILE_NAME(17), &
        SFC_FILE),(OVERLAY_FILE_NAME(18),CFC_FILE),   &
        (OVERLAY_FILE_NAME(19),RFC_FILE),(OVERLAY_FILE_NAME(20), &
        IFC_FILE),(OVERLAY_FILE_NAME(21),O1F_FILE),   &
        (OVERLAY_FILE_NAME(22),O2F_FILE),(OVERLAY_FILE_NAME(23), &
        O3F_FILE),(OVERLAY_FILE_NAME(24),CLS_FILE),   &
        (OVERLAY_FILE_NAME(25),CLH_FILE),(OVERLAY_FILE_NAME(26),LMG_FILE), &
        (OVERLAY_FILE_NAME(27),LMS_FILE),   &
        (OVERLAY_FILE_NAME(28),PFD_FILE),(OVERLAY_FILE_NAME(29),PNP_FILE), &
        (OVERLAY_FILE_NAME(30),FNP_FILE),   &
        (OVERLAY_FILE_NAME(31),CNP_FILE),(OVERLAY_FILE_NAME(32),CRA_FILE), &
        (OVERLAY_FILE_NAME(33),COS_FILE),   &
      ...
      DATA FILE_TYPES/'EAO','DDO','DBO','FAO','NFO','EXO','PAO', &
        'PFO','INO','TXO','ITO','FRO','ESO','CLO','ELO','UPO',   &
        'SFO','CFO','RFO','IFO','O1O','O2O','O3O','CSO','CHO', &
        'LGO','LSO','FPO','PNO','FNO','CNO','CRO',   &
        'COO','LDE','LFO','ENO','ECO','ARO','FVO','CTO','PPO', &
        'AAO','FIO','FCO','TRO','FEO','DAO','ASO',   &
        'RCO','EMO','DEO','RSO','C2O','TEO','TTO','MPO','DTO','SMO', &
        'TPO','MPO','PAO','RDO','RBO','TFO',   &
        'TIO','ADO','TGO','IPO','TNO','SPO','PRO','WHO','SHO','FOO', &
        'FBO','WCO','WTO','WDO','79O','80O',   &
        '81O','82O','83O','84O','F1O','F2O','F3O','F4O','F5O','F6O', &
        'F7O','F8O','F9O','C1O','C2O','C3O',   &
        'C4O','C5O','C6O','C7O','PSO','XXX','XXX','XXX','XXX','XXX', &
        'XXX','XXX','XXX','XXX','XXX','XXX',   &
        'XXX','XXX','XXX','XXX','XXX','XXX','XXX','XXX','XXX','XXX', &
        'XXX','XXX','XXX','XXX','XXX','XXX',   &
        'XXX','C8O','C9O','C0O','CAO','CBO','CCO','CDO','E1O','E2O', &
        'E3O','E4O','E5O','E6O','E7O','E8O',   &
        'E9O','E0O','EBO','EDO','EEO','EFO','T1O','T2O','T3O','T4O', &
        'T5O','T6O','T7O','T8O','T9O','T0O',   &
        'TBO','TDO','THO','TIO','CEO','CGO','EGO','EHO','ROO','X1O', &
        'X2O','X3O','X4O','X5O','SVO','S1O',   &
        'S2O','S3O','S4O','S5O','S6O','S7O','S8O','S9O','UDO','ICO', &
        'RMO','GPO','FHO','MRO','WRO','RTO',   &
        'RCO','R1O','R2O','R3O','SGO','EHO','GHO','OHO','P2O','P3O', &
        'P4O','P5O','CIO','CJO','CKO','CMO',   &
        'CPO','CQO','EIO','P6O','FXO','XXX','XXX','XXX','XXX','XXX', &
        'XXX','XXX','XXX','XXX','XXX','XXX',   &
        'XXX','XXX','XXX','XXX','XXX','XXX','XXX','XXX','XXX','XXX', &
        'XXX','XXX','XXX','XXX','XXX','XXX',   & !  240
...
!
      ...
      CALL RETURN_OVERLAY_FAMILY_LIST(OVLNAME,MATCH_FOUND, &
      OVERLAY_FILE_NAME,FILE_EXISTS)
      IF(MATCH_FOUND) THEN
...
!
         ...
         CALL COAL_TRANSPORT_LINKS_MAKEOVL(COAL_MODEL_GENE_TRANS_FILE,'ZD')  ! 374
...
!
         ...
         IF(FILE_EXISTS(307)) then
             CALL CL_MAKEOVL(OVERLAY_FILE_NAME(307),24)  ! CL_UNITS_CS
         end if
         IF(FILE_EXISTS(308)) then
             CALL CL_MAKEOVL(OVERLAY_FILE_NAME(308),25)  ! CL_UNITS_CU
         end if
         IF(FILE_EXISTS(309)) then 
            CALL CL_MAKEOVL(OVERLAY_FILE_NAME(309),26)  ! CL_UNITS_CV
         end if
         IF(FILE_EXISTS(310)) then
            CALL CL_MAKEOVL(OVERLAY_FILE_NAME(310),27)  ! CL_UNITS_CW
        end if
         IF(FILE_EXISTS(311)) THEN
            CALL CL_MAKEOVL(OVERLAY_FILE_NAME(311),28)  ! CL_UNITS_CX
        end if
         IF(FILE_EXISTS(312)) THEN
            CALL CL_MAKEOVL(OVERLAY_FILE_NAME(312),29)  ! CL_UNITS_CY
        end if
        
         IF(FILE_EXISTS(379)) then
            CALL CL_MAKEOVL(OVERLAY_FILE_NAME(379),30)
         end if
         IF(FILE_EXISTS(380)) THEN
            CALL CL_MAKEOVL(OVERLAY_FILE_NAME(380),31)
        end if
         IF(FILE_EXISTS(383)) THEN
            CALL CL_MAKEOVL(OVERLAY_FILE_NAME(383),32)
        end if
         IF(FILE_EXISTS(384)) THEN
            CALL CL_MAKEOVL(OVERLAY_FILE_NAME(384),33)
        end if 
        
         IF(FILE_EXISTS(387)) then
            CALL CL_MAKEOVL(OVERLAY_FILE_NAME(387),34)
        end if
        
         IF(FILE_EXISTS(388)) then
            CALL CL_MAKEOVL(OVERLAY_FILE_NAME(388),35)
        end if
...
!
         ...
         IF(PRODP_EXISTS) then
             CALL PP_MAKEOVL(PRODP_FILE) ! IN FRONT OF CL_MAKEOVL
         end if
!
...
!
         IF(MONTHLY_PAYABLES_EXISTS) then
              CALL MONTHLY_PAYABLES_MAKEOVL(MONTHLY_PAYABLES_FILE)
         end if
!
...
! CAPITAL RATES DATA
!
         IF(MONTHLY_CAPITIAL_RATES_EXISTS) then
             CALL MR_MAKEOVL(MONTHLY_CAPITIAL_RATES_FILE)
         end if
...
!
         ...
         IF(FX_FUTURE_ASSETS_CAPEX_EXISTS) then
             CALL FA_MAKEOVL(FX_FUTURE_ASSETS_CAPEX_FILE,1)
         end if
!
...
!
         IF(TRANS_CONSTRAINT_FILE_EXISTS) then
            CALL CONSTRAINT_MAKEOVL(TRANS_CONSTRAINT_FILE)
        end if
!
...
!
         DO I = 0, 14
            IF(THIS_OVERLAY_FILE_EXISTS(I)) THEN
               ...
               CALL GAS_DEMAND_FORECAST_MAKEOVL( &
                OVERLAY_FILE_NAMES,THIS_OVERLAY_FILE_EXISTS)
               EXIT
            ENDIF
         ENDDO
!
...
! 071406.
!
         IF(GAS_STORAGE_FILE_EXISTS) then
            CALL GAS_STORAGE_MAKEOVL(GAS_STORAGE_FILE)
         end if
         IF(GAS_NODE_FILE_EXISTS) then
            CALL GAS_NODE_MAKEOVL(GAS_NODE_FILE)
         end if
         IF(GAS_WEATHER_DEMAND_EXISTS) then
            CALL GAS_WEATHER_DEMAND_MAKEOVL(GAS_WEATHER_DEMAND_FILE)
        end if
         IF(GAS_TRANSPORT_FILE_EXISTS) then
            CALL GAS_LINK_MAKEOVL(GAS_TRANSPORT_FILE)
        end if
         IF(GAS_DEMAND_FORECAST_FILE_EXISTS) then
            CALL GAS_DEMAND_FORECAST_MAKEOVL(GAS_DEMAND_FORECAST_FILE)
        end if
!
! 070806.
!
         IF(GAS_SUPPLY_FORECAST_FILE_EXISTS) then
			CALL GAS_SUPPLY_FORECAST_MAKEOVL(GAS_SUPPLY_FORECAST_FILE)
		end if
!
...
!
         IF(trim(NEW_MARKET_PRICE_FILE) /= 'NONE' .AND. &
			OVERLAY_BOOK_IS_ACTIVE) then
				CALL NEW_MARKET_PRICE_MAKEOVL(NEW_MARKET_PRICE_FILE)
		end if
!
...
!
         IF(trim(ELECT_HOUR_MULT_FILE) /= 'NONE' .AND. &
			OVERLAY_BOOK_IS_ACTIVE) CALL EH_MAKEOVL(ELECT_HOUR_MULT_FILE)
!
! GAS_HOUR_MULT_FILE OVERLAY ! 10/23/03
!
         IF(trim(GAS_HOUR_MULT_FILE) /= 'NONE' .AND. &
		 OVERLAY_BOOK_IS_ACTIVE) CALL EH_MAKEOVL(GAS_HOUR_MULT_FILE)
!
!
! ELECT_HOUR_MULT_FILE OVERLAY ! 10/23/03
!
         IF(trim(OIL_HOUR_MULT_FILE) /= 'NONE' &
			.AND. OVERLAY_BOOK_IS_ACTIVE) then
				CALL EH_MAKEOVL(OIL_HOUR_MULT_FILE)
		end if
!
! SCENARIO_HOURLY OVERLAY ! 11/7/99. GAT.
!
         IF(trim(SCENARIO_HOURLY_FILE) /= 'NONE' .AND. &
			OVERLAY_BOOK_IS_ACTIVE) then
				CALL SCENARIO_HOURLY_MAKEOVL(SCENARIO_HOURLY_FILE)
		end if
!
! WEEKLY_HYDRO OVERLAY ! 8/17/00. TS, GAT. ACTIVATED 4/5/01.
!
         IF(trim(WEEKLY_HYDRO_FILE) /= 'NONE' .AND. &
		 OVERLAY_BOOK_IS_ACTIVE) then
			CALL WEEKLY_HYDRO_MAKEOVL(WEEKLY_HYDRO_FILE)
		end if
!
! RPS REQUIREMENTS OVERLAY ! 042209
!
         IF(trim(RPS_REQUIREMENTS_FILE) /= 'NONE' .AND.  &
			OVERLAY_BOOK_IS_ACTIVE) then
				CALL RPS_REQUIREMENTS_MAKEOVL(RPS_REQUIREMENTS_FILE)
		end if
!
! RPS STATE DEMAND OVERLAY ! 042709
!
         IF(trim(RPS_STATE_DEMAND) /= 'NONE' .AND. OVERLAY_BOOK_IS_ACTIVE) then
			CALL RPS_STATE_DEMAND_MAKEOVL(RPS_STATE_DEMAND)
		end if
!
! THERMAL RETROFIT OVERLAY ! 052609
!
         IF(trim(THERMAL_RETROFIT) /= 'NONE' .AND. OVERLAY_BOOK_IS_ACTIVE) then
			CALL THERMAL_RETROFIT_MAKEOVL(THERMAL_RETROFIT)
		end if
!
! RPS_PROGRAM OVERLAY ! 110218
!
         IF(trim(RPS_PROGRAM_FILE) /= 'NONE' .AND. OVERLAY_BOOK_IS_ACTIVE) then
			CALL RPS_PROGRAM_MAKEOVL(RPS_PROGRAM_FILE)
		end if
!
! CO2_PARAM OVERLAY ! 120809
!
         IF(trim(CO2_PARAM) /= 'NONE' .AND. OVERLAY_BOOK_IS_ACTIVE) then
			CALL CO2_PARAM_MAKEOVL(CO2_PARAM)
		end if
		
!
! WEATHER_DEMAND OVERLAY !040601. TS
!
         IF(trim(WEATHER_DEMAND_FILE) /= 'NONE' .AND. OVERLAY_BOOK_IS_ACTIVE) then
			CALL WEATHER_DEMAND_MAKEOVL(WEATHER_DEMAND_FILE)
		end if
		
!
! MARKET_GROUPS OVERLAY ! 6/8/01. GAT.
!
         IF(trim(MARKET_GROUPS_FILE) /= 'NONE' .AND. OVERLAY_BOOK_IS_ACTIVE) then
			CALL MK_MAKEOVL(MARKET_GROUPS_FILE)
		end if
!
! SCENARIO MAKER OVERLAY ! 11/12/98. GAT.
!
         IF(SCENARIO_MAKER_EXISTS) then
			CALL SCENARIO_MAKER_MAKEOVL(SCENARIO_MAKER_FILE,0)
		end if
...
!
         IF(REGIONAL_PARAMETER_EXISTS) then
            CALL REG_OUT_PARAM_MAKEOVL(REGIONAL_PARAMETER_FILE,0)
        end if
         ...
         IF(REGIONAL_OUTAGES_EXISTS) then
            CALL REG_OUT_PARAM_MAKEOVL(REGIONAL_OUTAGES_FILE,4)
        end if
!
! FUEL DERIVATIVES OVERLAY ! 12/23/02. GAT.
!
         IF(FUEL_DERIVATIVES_EXISTS) then
            CALL FUEL_DERIVATIVES_MAKEOVL(FUEL_DERIVATIVES_FILE)
        end if
!
...
!
         IF(trim(MARKET_PRICE_FILE) /= 'NONE' .AND. OVERLAY_BOOK_IS_ACTIVE) then
			CALL MARKET_PRICE_MAKEOVL(MARKET_PRICE_FILE)
		end if
!
! REFERENCE LOAD OVERLAY ! 1/16/98. GAT.
!
         IF(trim(REFERENCE_LOAD_DATA_FILE) /= 'NONE' .AND. &
		 OVERLAY_BOOK_IS_ACTIVE) then
...
!
         ...
         IF(CLASS_RUN_SPECS_EXISTS) then
			CALL CLASS_RUN_SWITCHES_MAKEOVL(CLASS_RUN_SPECS_FILE)
		end if
         IF(CLASS_REVENUE_FORECAST_EXISTS .AND. .NOT. WVPA()) then
			CALL CLASS_SALES_MAKEOVL(CLASS_REVENUE_FORECAST_FILE)
		end if
         CALL CLOSE_ASSET_VECTOR_FILE
      ENDIF

      IF(INDEX(ProSym_Interface_File(),'NONE') == 0 .AND. &
        .NOT. WVPA()) THEN
! TODO: Combine IF/Elseif
      ELSE IF(.NOT. MATCH_FOUND) THEN
          IF(OVERLAY_BOOK_ACTIVE()) THEN
             WRITE(4,*) 'For endpoint ',ep_loc,' no overlay ', &
				'files in the overlay family ',trim(OVLNAME),' were found.'
          ELSE
             WRITE(4,*) 'For endpoint ',ep_loc,' no overlay ', &
				'files with the name ',trim(OVLNAME),' were found.'
          ENDIF
      ENDIF
...
! 05/15/03. CLEARS NAME LINE AT BOTTOM OF FILE
      ...
      RUN_FUTURE_ASSETS = RUN_FUTURE_ASSETS .OR. FAE_EXISTS .OR. &
		DBE_EXISTS .OR. EAE_EXISTS .OR. DDE_EXISTS .OR.   &
        EPE_EXISTS .OR. NFE_EXISTS .OR. ASTRT_EXISTS .OR. ASTVEC_EXISTS
        ! Couldn't move OVERLAY_FILE_NAME array to module because
        ! of EQUIVALENCE concerns. Caching in separate variable in 
        ! module so the list can be printed.
        call fill_overlays(overlay_file_name)
      RETURN
...
!
       ...
       filename=get_osf_filename(R_PROJECT_NAME)
         ...
         if(len_trim(r_project_name)==0) then
            call end_program("R_PROJECT_NAME is empty in " // &
				" set_up_overlay_families entry.")
         endif
...
!
            ALLOCATE(FAMILY_NAMES(NUMBER_OF_DEFINITIONS), &
				OVERLAY_FILES_IN_FAMILY(NUMBER_OF_DATA_FILES, &
				NUMBER_OF_DEFINITIONS),   &
                     OVERLAY_FILE_EXISTS(NUMBER_OF_DATA_FILES, &
						NUMBER_OF_DEFINITIONS), &
						OVERLAY_FAMILY_ACTIVE(NUMBER_OF_DEFINITIONS))
...
!
               ...
               RECLN = trim(RECLN)//',,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,'   &
                             ...
                             //',,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,'   &
                             //',,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,'
               READ(RECLN,*) DELETE,FAMILY_NAMES(I), &
				(OVERLAY_FILES_IN_FAMILY(J,I),J=1,NUMBER_OF_DATA_FILES)
               DO J = 1, NUMBER_OF_DATA_FILES
                  IF(FILE_TYPES(J) == 'XXX') CYCLE
                  OVL_FILE_NAME = OVERLAY_FILES_IN_FAMILY(J,I)
!
                  IF(INDEX(OVL_FILE_NAME,'NONE') == 0 .AND. &
				  INDEX('NONE',trim(OVL_FILE_NAME)) == 0) THEN
                     INQUIRE(FILE=FILE_TYPES(J)//trim(OVL_FILE_NAME)// &
					 '.DAT',EXIST=THIS_FILE_EXISTS)
                     IF((J >= 170 .AND. J <= 174) .AND. &
						.NOT. THIS_FILE_EXISTS) THEN
                        INQUIRE(FILE='EXO'//trim(OVL_FILE_NAME)// &
							'.DAT',EXIST=THIS_FILE_EXISTS)
                     ENDIF
                     OVERLAY_FILE_EXISTS(J,I) = THIS_FILE_EXISTS
                     OVERLAY_FAMILY_ACTIVE(I) = OVERLAY_FAMILY_ACTIVE(I) &
						.OR. OVERLAY_FILE_EXISTS(J,I)
                  ENDIF
               ENDDO
            ENDDO
            CLOSE(10)
      ...
      ENTRY RETURN_OVERLAY_FAMILY_LIST(R_OVLNAME,R_MATCH_FOUND, &
	  R_OVERLAY_FILE_NAMES,R_FILE_EXISTS)
...
!
         ...
         IF(FAMILY_DEFINITION_EXISTS) THEN
            FAMILY_POSITION = 0
            DO I = 1, NUMBER_OF_DEFINITIONS
               IF(INDEX(FAMILY_NAMES(I),R_OVLNAME) /=0 .AND. &
			   INDEX(R_OVLNAME,FAMILY_NAMES(I)) /=0) THEN
                  FAMILY_POSITION = I
                  EXIT
               ENDIF
            ENDDO
            IF(FAMILY_POSITION /= 0) THEN
               ...
               IF(R_MATCH_FOUND) THEN
                     R_OVERLAY_FILE_NAMES(:) = &
					 OVERLAY_FILES_IN_FAMILY(:,FAMILY_POSITION)
                     R_FILE_EXISTS(1:NUMBER_OF_DATA_FILES) = &
					 OVERLAY_FILE_EXISTS(:,FAMILY_POSITION)
               ENDIF
            ENDIF
         ENDIF
      RETURN
