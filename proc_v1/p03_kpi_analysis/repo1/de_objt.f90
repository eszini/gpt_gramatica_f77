...
!
      ...
      use ft_interfaces
      implicit none
      character (len=1) :: enrg_limit_report_ch
      INTEGER(kind=2) :: IREC,INUNIT,DELETE,YEAR_loc,LRECL=214
      INTEGER :: IOS

      CHARACTER (len=1) :: FORECAST_REPORT,EXPANSION_REPORT
      character (len=2) :: DISPATCH_REPORT*2
      character (len=1) :: ALLOCATION_REPORT, &
      ...
      CHARACTER(len=256) :: SELECTED_DISPATCH_RPT_MONTHS*30,&
...
!  DECLARATION FOR RUN SPECS
      CHARACTER(len=1) :: CSV_REPORT,KEPCO_REPORTS,CONTRACT_REPORT, &
         ...
         DEPTH_OF_MARKET,HOURLY_COMMITMENT, &
      ...
      character (len=256) :: DATA_DRIVE_loc
...
!  CONVERT DETAILED REPORTS
      ENTRY DE_MAKEBIN
         DATA_DRIVE_loc = get_OUTPUT_DIRECTORY()
         FILE_NAME = trim(BASE_FILE_DIRECTORY())// &
                           "DEB"//trim(DETAILED_REPORT_FILE())//".DAT"
         INQUIRE(FILE=FILE_NAME,EXIST=FILE_EXISTS)
         OPEN(11,FILE=trim(DATA_DRIVE_loc)//"BCDETRPT.BIN", &
         ...
         DISPATCH_REPORT = 'F'
         ALLOCATION_REPORT = 'F'
         call set_ENRG_LIMIT_REPORT(logical(.false.,1))
         LOAD_PROB_PLOT = 'F'
         ...
         TRANS_HOURLY_DIAG = 'F'
         HYDRO_SALES_AS_NEGATIVE = logical(.false.,1)
!
         DEPTH_OF_MARKET = 'F'
         HOURLY_COMMITMENT = 'F'
         DAILY_COMMITMENT = 'F'
         ...
         DO
            IF(FILE_EXISTS) THEN
               ...
               IF(IOS == 0) THEN
                  RECLN = trim(RECLN)//',,,,,,,,,,,,,,,,,,,,,,,,,,,,,' &
                                       //',,,,,,,,,,,,,,,,,,,,,,,,,,,,,'
                  READ(RECLN,*) DELETE,YEAR_loc,FORECAST_REPORT, &
           EXPANSION_REPORT,DISPATCH_REPORT,ALLOCATION_REPORT, &
           ENRG_LIMIT_REPORT_ch,LOAD_PROB_PLOT,UNIT_OUTPUT_REPORT, &
          ...
          RPS_PROGRAMS_REPORT,OLD_RPS_PROGRAMS_REPORT,RPS_CURVE_REPORT,&
           HOURLY_GRX_STORAGE_REPORT
            ...
            ENDIF
            WRITE(11,REC=IREC) &
            DELETE,YEAR_loc,FORECAST_REPORT,EXPANSION_REPORT, &
            DISPATCH_REPORT,ALLOCATION_REPORT,ENRG_LIMIT_REPORT_ch, &
...
!
            ...
            IF(IREC > AVAIL_DATA_YEARS) then
                EXIT ! needed to cover extension period 11/06/06 Dr.G
            end if
         ENDDO
...
!
      ...
      ELSE
         ...
         CALL LOCATE(10,51)
      ENDIF
      DATA_DRIVE_loc = OUTPUT_DIRECTORY()
      FILE_NAME = trim(DATA_DRIVE_loc)//"DEO"// &
      ...
      IF(DET_RPT_OL == 'BC') THEN
         ...
         INUNIT = 11
      ENDIF
      OPEN(12,FILE=trim(DATA_DRIVE_loc)//"OLDETRPT.BIN",ACCESS="DIRECT", &
                                            STATUS="UNKNOWN",RECL=LRECL)
      CSV_BY_UNIT(0) = .FALSE.
      IREC = 0
      DO
         IREC = IREC + 1
         READ(INUNIT,REC=IREC,IOSTAT=IOS) DELETE,YEAR_loc, &
              FORECAST_REPORT,EXPANSION_REPORT,DISPATCH_REPORT, &
              ALLOCATION_REPORT,ENRG_LIMIT_REPORT_ch,LOAD_PROB_PLOT, &
         ...
         IF(IOS == 0) THEN
            RECLN = trim(RECLN)// &
                             ',,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,'
          READ(RECLN,*,IOSTAT=IOS) DELETE,YEAR_loc,FORECAST_REPORT, &
          EXPANSION_REPORT,DISPATCH_REPORT,ALLOCATION_REPORT, &
          ENRG_LIMIT_REPORT_ch,LOAD_PROB_PLOT,UNIT_OUTPUT_REPORT, &
          ...
          HOURLY_GRX_STORAGE_REPORT
         ENDIF
         WRITE(12,REC=IREC) DELETE,YEAR_loc,FORECAST_REPORT, &
          EXPANSION_REPORT,DISPATCH_REPORT,ALLOCATION_REPORT, &
          ENRG_LIMIT_REPORT_ch,LOAD_PROB_PLOT,UNIT_OUTPUT_REPORT, &
          ANNUAL_UNIT_OUTPUT_REPORT,DSM_FINANCIAL_REPORT,CSV_REPORT, &
          KEPCO_REPORTS,CONTRACT_REPORT,FUEL_USAGE_REPORT, &
...
!
      ...
      INTEGER(kind=2) :: YEAR_loc,OPYEAR
      INTEGER :: IOS
...
!
      ...
      CHARACTER(len=1) :: FINANCIAL_DRILLING
         character (len=30) :: SELECTED_DISPATCH_RPT_MONTHS, &
         SELECT_NEW_SUPPLY_RPT_MONTHS
         character (len=1) :: MONTHLY_COAL_DEMAND_REPORT_STR='F', &
      ...
      CHARACTER(len=1) :: &
         HOURLY_AFTER_TRANS_REPORT_STR='F', &
...
!
      ...
      RETURN
!
      ENTRY RETURN_DETAILED_REPORTS(YEAR_loc)
!
         RETURN_DETAILED_REPORTS = .TRUE.

         IREC=1 ! Avoid spurious comment from Lahey about IREC not set.
         ! needed to cover extension period Dr.G
         IREC = MIN(YEAR_loc,AVAIL_DATA_YEARS)
...
!
      ...
      RETURN
      END function reporting_asset_class
