...
subroutine update_energy_in_prod_arrays(PROD_BY_MK_BY_MWH, MK, MW_arg, ANNUAL_PROD_BY_MK_BY_MWH, ENRG, I, MAX_CL_UNITS)
real :: PROD_BY_MK_BY_MWH(:,:), MW_arg(:,:), ANNUAL_PROD_BY_MK_BY_MWH(:,:)
real :: enrg
integer :: mk
integer :: I, MAX_CL_UNITS

    PROD_BY_MK_BY_MWH(MK,1) = PROD_BY_MK_BY_MWH(MK,1) + ENRG
    PROD_BY_MK_BY_MWH(MK,2) = PROD_BY_MK_BY_MWH(MK,2) + MW_arg(2,I)
    PROD_BY_MK_BY_MWH(MK,3) = PROD_BY_MK_BY_MWH(MK,3) + 1
    ANNUAL_PROD_BY_MK_BY_MWH(MK,1) = ANNUAL_PROD_BY_MK_BY_MWH(MK,1) + ENRG
    ANNUAL_PROD_BY_MK_BY_MWH(MK,2) = ANNUAL_PROD_BY_MK_BY_MWH(MK,2) + MW_arg(2,I)
...
subroutine handle_annual_output_report(I,J,TOTAL_COST,CL_UNIT_ANNUAL_FIXED_COST, &
      ...
      integer, save :: file_trace_haor=0
      ...
      CHARACTER (len=2) :: GREEN_MRX_METHOD,CAPACITY_PLANNING_METHOD
            if(file_trace_haor==0) then
                file_trace_haor=open_trace("handle_annual_output_report.trace", &
                    rq_haor)
            end if
            call write_trace_bool1(file_trace_haor, "ANNUAL_OUTPUT_REPORT", &
                ANNUAL_OUTPUT_REPORT)
      
            IF(ANNUAL_OUTPUT_REPORT) THEN
               call write_trace_string(file_trace_haor, "CAPACITY_PLANNING_METHOD", &
                CAPACITY_PLANNING_METHOD())
               call write_trace_string(file_trace_haor, "GREEN_MRX_METHOD", &
                GREEN_MRX_METHOD())
                
               IF(CAPACITY_PLANNING_METHOD() == 'MX'  .AND. GREEN_MRX_METHOD() == 'GX') THEN
                  WRITE(ANN_CL_UNIT_NO,REC=ANN_CL_UNIT_REC) &   ! VARIABLE = 11 + # OF EMIS
                    PRT_ENDPOINT(), &
                    FLOAT(SAVE_YEAR+get_BASE_YEAR()), &
                    UNIT_NAME, &
                    ...
                    if (file_trace_haor/=BAD_TRACE_HANDLE) then
                        write(file_trace_haor, *) &
                        "EP: ", PRT_ENDPOINT(), &
                    "YEAR: ", FLOAT(SAVE_YEAR+get_BASE_YEAR()), &
                    "UNIT_NAME ", UNIT_NAME, &
                    "GRX_ITERATIONS ", FLOAT(GRX_ITERATIONS), &
                    "I ",  FLOAT(I), &
             "       MW ", MW(2,I), &
                     "CAP_FACTOR ", CAP_FACTOR, &
                    "ANNUAL_CL_UNIT_ENERGY ", ANNUAL_CL_UNIT_ENERGY(I)/1000., &
                    "AVERAGE_HEATRATE ", AVERAGE_HEATRATE, &
                    "AVERAGE_COST ", AVERAGE_COST, &
                    "CL_UNIT_ANNUAL_FUEL_COST ", CL_UNIT_ANNUAL_FUEL_COST, &
                    "CL_UNIT_ANNUAL_VAR_COST ", CL_UNIT_ANNUAL_VAR_COST, &
                    "CL_UNIT_ANNUAL_FIXED_COST ", CL_UNIT_ANNUAL_FIXED_COST, &
                    "TOTAL_COST ", TOTAL_COST, &
                    "ANNUAL_CL_UNIT_EMISSIONS", &
                    (ANNUAL_CL_UNIT_EMISSIONS(J,I),J=1,NUMBER_OF_EMISSION_TYPES), &
                    "CL_AI_INVESTMENT ", CL_AI_INVESTMENT(I)/1000000.
               ...
               call write_trace_int(file_trace_haor, "ANN_CL_UNIT_REC became ", &
                ANN_CL_UNIT_REC)
...
subroutine collect_fiscal_data(I,UNITNO,FISCAL_ECO_SALES_REV_FROM,FISCAL_ECO_SALES_ENRG_FROM, &
    FISCAL_ECO_PUCH_COST_FROM, &
