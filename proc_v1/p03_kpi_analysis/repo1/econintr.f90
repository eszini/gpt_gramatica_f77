...
!
      ...
      REAL :: SEGMENT_ENERGY_UNUSED,SEGMENT_GENERATION, &
      SEGMENT_ENERGY_BOUGHT(MAX_DISPATCH_BLOCKS), &
            SEGMENT_ENERGY_SOLD(MAX_DISPATCH_BLOCKS),RR_ENERGY_TO_BUY, &
            RR_ENERGY_TO_SELL, &
            ENERGY(2,MAX_CL_UNITS),EAVAIL(MAX_CL_UNITS),SEAS_HOURS, &
            MWBLOK(MAX_DISPATCH_BLOCKS),ANNUAL_ECONOMY_BOUGHT, &
            ANNUAL_ECONOMY_SOLD,MONTHLY_ECONOMY_BOUGHT, &
            MONTHLY_ECONOMY_SOLD,MONTHLY_ECONOMY_COST, &
            MONTHLY_ECONOMY_REVENUE, &
            ANNUAL_ECONOMY_COST,ANNUAL_ECONOMY_REVENUE, &
            LEFT(MAX_DISPATCH_BLOCKS),RIGHT(MAX_DISPATCH_BLOCKS), &
            RIGHT_RIGHT,RR_PROB, &
            TRANSACT_PROB,TIE_CAP,MAINTENANCE_RATE(MAX_CL_UNITS), &
            LAST_RIGHT,TOP_PROB,BOTTOM_PROB,LAST_PROB,SUM_PROB,TRANS_COST, &
            LEFT_HEAT(MAX_CL_UNITS),RIGHT_HEAT(MAX_CL_UNITS), &
            IMPORT_CAP,EXPORT_CAP,RR_COST, &
            CL_POOL_FRAC_OWN(MAX_CL_UNITS), &
            TEMP_BUY_ENERGY,TEMP_SELL_ENERGY, &
            EC_UTIL_PORTION(MAX_DISPATCH_BLOCKS)
      REAL :: EC_POOL_PORTION(MAX_DISPATCH_BLOCKS), &
      SEGMENT_ENERGY_COST(MAX_DISPATCH_BLOCKS), &
            SEGMENT_ENERGY_REV(MAX_DISPATCH_BLOCKS), &
            SEGMENT_BUYS(MAX_DISPATCH_BLOCKS), &
            SEGMENT_SELLS(MAX_DISPATCH_BLOCKS), &
            L_TEMP_HEAT(0:MAX_DISPATCH_BLOCKS), &
            R_TEMP_HEAT(0:MAX_DISPATCH_BLOCKS),BASE_MW, &
            FIRST_SEGMENT_MW_ABOVE_BASE, &
            LODDUR(CONVOLUTION_POINTS),TIE_PERCENT, &
            SELL_PRICE,BUY_PRICE,OBS_AT_LOAD_POINT(100), &
            RR_AT_LOAD_POINT(100)
!
      CHARACTER (LEN=2) :: EXT(16)
      CHARACTER (LEN=12) :: FILE_NAME
      DATA EXT /'96','97','98','99','00','01','02','03','04','05','06', &
      '07','08','09','10','11'/
...
!
      INTEGER (KIND=2) :: I,U,NUMBER_OF_LOAD_POINTS,NUMBER_OF_SEGMENTS, &
      BLKNO(MAX_DISPATCH_BLOCKS), &
               ECBLK,UNIT(MAX_DISPATCH_BLOCKS),K,BASE_K,NEXT_K,ICOST,KCOST,YR, &
               S_MAX,ILAST,PERIOD_COUNTER,LAST_BUY_SEGMENT, &
               LAST_SELL_SEGMENT,IPNT,LOAD_POINT
      LOGICAL (KIND=1) :: FIRST_UNIT_PASS,INCREMENT_KCOST,SPLIT_SAVINGS, &
      ECON_PRICING
      INTEGER (KIND=4) :: S
      REAL :: EXTENSION_MULT
! ECONOMY REPORT VARIABLES
      ...
      INTEGER (KIND=2) :: LAST_SEASON,TRANSACTION_POINT, &
      LAST_TRANSACTION,GENGRP(MAX_CL_UNITS),GRP
      REAL :: TRANSACTION_ENERGY_BOUGHT,TRANSACTION_COST,TRANSACTION_ENERGY_SOLD, &
      TRANSACTION_REVENUE,TRANSACTION_AVE_BUY, &
              TRANSACTION_AVE_SELL,SRP_SEGMENT_COST(*), &
              PERIOD_NON_DISPATCHING_REV,ANNUAL_NON_DISPATCHING_REV=0., &
              R_SRP_NON_DISPATCH_REV,TEMPEA(2,MAX_CL_UNITS), &
              GROUP_ECO_SALES(0:MAX_REPORTING_GROUPS), &
              GROUP_ECO_PURCHASES(0:MAX_REPORTING_GROUPS), &
              R_GROUP_ECO_SALES(MAX_REPORTING_GROUPS), &
      ...
      SAVE  ECONOMY_INTER_NO,LAST_SEASON,CL_MONTH_NAME,GROUP_ECO_SALES, &
        GROUP_ECO_PURCHASES
!
...
!
      EXTENSION_MULT = 1.
      IF(globecom_year > get_globecom_study_period()) THEN
         IF(globecom_YEAR == get_globecom_study_period() + 1 .AND. PERIOD_COUNTER == 1) THEN
            CALL CALC_ECONOMY_EXTENSION_MULT(globecom_YEAR)
         ENDIF
...
!
      ...
      IF(EXTERNAL_RR) THEN
         ...
         IF(PERIOD_COUNTER == 1) THEN
            ...
            IF(FILE_EXISTS) THEN
               OPEN(EXTERNAL_ECONOMY_INTER_NO,FILE= FILE_NAME, &
                ACCESS="DIRECT",RECL=3200)
            ELSE
...
!
      ...
      DO WHILE(RIGHT(BASE_K) >= 1. .AND. BASE_K .LT. NUMBER_OF_SEGMENTS)
         ...
         U = UNIT(BASE_K)
!                                                  ! FOR'S ARE CONVOLVED
         BASE_MW  = BASE_MW + MWBLOK(BASE_K) * (1. - MAINTENANCE_RATE(U))
         BASE_K = BASE_K+1
      ENDDO
!
      LOAD_POINT = 1
      ...
      ELSE
         DO WHILE(BASE_MW > LODDUR(LOAD_POINT) )
            ...
            ENDIF
         ENDDO
         FIRST_SEGMENT_MW_ABOVE_BASE = MWBLOK(BASE_K) - &
            (LODDUR(LOAD_POINT) - BASE_MW)
      ENDIF
!
      K = BASE_K
      S = K
...
!
               DO WHILE (TRANS_COST < BLOCK_DISP_COST(U,ECBLK) .AND. &
               TIE_CAP < IMPORT_CAP .AND. &
                                BLKNO(S) > 0 .AND. S >LAST_BUY_SEGMENT)
!
!                  U = UNIT(S)
                  IF((ECONOMY_TRANS_TYPE(U) == 'B') .OR. &
                    (ECONOMY_TRANS_TYPE(U) == 'T')) THEN
                     UNIT_POWER = MIN(IMPORT_CAP - TIE_CAP,MWBLOK(S))
                     TIE_CAP = TIE_CAP + UNIT_POWER
!
!                 BUY UNTIL THE RR PRICE IS GREATER THAN SEGMENT_COST
!
                     SEGMENT_GENERATION=ENERGY(ECBLK,U)*TEMPEA(ECBLK,U)
                     IF(FIRST_UNIT_PASS .AND. UNIT_POWER >= MWBLOK(S)) THEN
                        IF(TOP_PROB == 1.0) THEN
                           RR_ENERGY_TO_BUY = TRANSACT_PROB * (MWBLOK(S) - &
                           FIRST_SEGMENT_MW_ABOVE_BASE)* &
                          (1. - MAINTENANCE_RATE(U)) * EAVAIL(U)* &
                          (TRANSACT_PROB*.5 + MAX(LEFT(K)-TOP_PROB,0.))/RIGHT_RIGHT
                        ELSEIF(RIGHT_RIGHT > 0.000001) THEN
                           RR_ENERGY_TO_BUY = TRANSACT_PROB * MWBLOK(S) * &
                           (1. - MAINTENANCE_RATE(U)) * &
                           EAVAIL(U)* (TRANSACT_PROB*.5 + &
                           MAX(LEFT(K)-TOP_PROB,0.))/RIGHT_RIGHT
                        ELSE
                           RR_ENERGY_TO_BUY = 0.
                        ENDIF
                     ELSE
                        RR_ENERGY_TO_BUY = TRANSACT_PROB * UNIT_POWER * (1. - &
                            MAINTENANCE_RATE(U)) *EAVAIL(U)
                     ENDIF
! MODIFIED: 10/19/92 FOR POOLING TRANSACTIONS
                     IF(RR_ENERGY_TO_BUY .LT. SEGMENT_GENERATION) THEN
                        ...
                        ENERGY(ECBLK,U) = ENERGY(ECBLK,U) - &
                            TEMP_BUY_ENERGY/TEMPEA(ECBLK,U) ! 6/17/96. GAT.
                     ...
                     ELSE
                        TEMP_BUY_ENERGY = 0.
                     ENDIF
                     SEGMENT_ENERGY_BOUGHT(S) = TEMP_BUY_ENERGY + &
                        SEGMENT_ENERGY_BOUGHT(S)

                     TRANSACTION_ENERGY_BOUGHT = TRANSACTION_ENERGY_BOUGHT + &
                        TEMP_BUY_ENERGY
...
!
                        IF(EC_POOL_PORTION(U) .GT. 0.) THEN
                           P_CLASS_ASS_ECON_BUY(2) = P_CLASS_ASS_ECON_BUY(2) + &
                           TEMP_BUY_ENERGY*SEAS_HOURS * EC_POOL_PORTION(U)
                           P_CLASS_ASS_ECON_COST(2) = P_CLASS_ASS_ECON_COST(2) + &
                           TEMP_BUY_ENERGY*SEAS_HOURS * &
...
! TRAP FOR POOLING
                     ...
                     TRANSACTION_COST = TRANSACTION_COST + TEMP_BUY_ENERGY* &
                     BUY_PRICE
!     CHECKING THE IMPACT FOR BG&E OF EXTERNAL RUNNING COSTS

                     SEGMENT_ENERGY_COST(S) = SEGMENT_ENERGY_COST(S) + &
                     	TEMP_BUY_ENERGY*BUY_PRICE
...
! MODIFIED HEAT CALCULATION 10/30/92 MSG
   ...
   40          U = UNIT(S)
               ...
               SELL_AGAINST_UNIT = (ECONOMY_TRANS_TYPE(U) == 'S') .OR. &
               (ECONOMY_TRANS_TYPE(U) == 'T')
               ...
               IF(RR_COST > TRANS_COST .AND. TIE_CAP < EXPORT_CAP) THEN
                  ...
                  IF(FIRST_UNIT_PASS .AND. UNIT_POWER >= MWBLOK(S)) THEN
                     IF(TOP_PROB == 1.0) THEN
                        RR_ENERGY_TO_SELL = TRANSACT_PROB * &
                        FIRST_SEGMENT_MW_ABOVE_BASE * (1. - &
                        MAINTENANCE_RATE(U)) * EAVAIL(U)* &
                          (TRANSACT_PROB*.5 + &
                          MAX(BOTTOM_PROB-RIGHT(K),0.))/RIGHT_RIGHT
                  ...
                  ELSE
                     RR_ENERGY_TO_SELL = TRANSACT_PROB * &
                     UNIT_POWER * (1. - MAINTENANCE_RATE(U)) * EAVAIL(U)
                  ENDIF

                  SEGMENT_ENERGY_UNUSED = (MWBLOK(S) * (1. - MAINTENANCE_RATE(U)) - &
                  	ENERGY(ECBLK,U)) * EAVAIL(U)
                  IF(RR_ENERGY_TO_SELL .LT. SEGMENT_ENERGY_UNUSED) THEN
                     TEMP_SELL_ENERGY = RR_ENERGY_TO_SELL
                     ENERGY(ECBLK,U) = ENERGY(ECBLK,U) + &
                     	RR_ENERGY_TO_SELL / EAVAIL(U)
                  ...
                  SEGMENT_ENERGY_SOLD(S) = SEGMENT_ENERGY_SOLD(S) + &
                  	TEMP_SELL_ENERGY

                  TRANSACTION_ENERGY_SOLD = TRANSACTION_ENERGY_SOLD + &
                  	TEMP_SELL_ENERGY
                  IF(SPLIT_SAVINGS) THEN
...
!
                     IF(EC_POOL_PORTION(U) .GT. 0.) THEN
                        P_CLASS_ASS_ECON_SELL(2) = P_CLASS_ASS_ECON_SELL(2) + &
                        TEMP_SELL_ENERGY * SEAS_HOURS * EC_POOL_PORTION(U)
                        P_CLASS_ASS_ECON_REV(2) = P_CLASS_ASS_ECON_REV(2) + &
                        TEMP_SELL_ENERGY*SEAS_HOURS * &
                        EC_POOL_PORTION(U) * SELL_PRICE/10.
                        TEMP_SELL_ENERGY = TEMP_SELL_ENERGY * EC_UTIL_PORTION(U)
                     ENDIF
                     P_CLASS_ASS_ECON_SELL(1) = P_CLASS_ASS_ECON_SELL(1) + &
                     	TEMP_SELL_ENERGY*SEAS_HOURS
                     P_CLASS_ASS_ECON_REV(1) = P_CLASS_ASS_ECON_REV(1) + &
                     	TEMP_SELL_ENERGY * SEAS_HOURS *SELL_PRICE/10.
                  ENDIF ! END SELL POOLING SECTION
                  TRANSACTION_REVENUE = TRANSACTION_REVENUE + &
                  	TEMP_SELL_ENERGY * SELL_PRICE
!
                  PERIOD_NON_DISPATCHING_REV = PERIOD_NON_DISPATCHING_REV + &
                  	SEAS_HOURS*TEMP_SELL_ENERGY*(SRP_SEGMENT_COST(S) - &
                                 BLOCK_DISP_COST(U,ECBLK))/10.
!
                  SEGMENT_ENERGY_REV(S) = SEGMENT_ENERGY_REV(S) + &
                  	TEMP_SELL_ENERGY * SELL_PRICE
...
!
                  L_TEMP_HEAT(S) = MAX(MIN(R_TEMP_HEAT(S-1),TOP_PROB), &
                  L_TEMP_HEAT(S))
                  R_TEMP_HEAT(S) = MAX(R_TEMP_HEAT(S),MIN(L_TEMP_HEAT(S), &
                  TOP_PROB))
...
!              NO TRANSACTION TAKE PLACE
            ...
            MONTHLY_ECONOMY_BOUGHT = MONTHLY_ECONOMY_BOUGHT + &
            	TRANSACTION_ENERGY_BOUGHT
            ...
            MONTHLY_ECONOMY_REVENUE = MONTHLY_ECONOMY_REVENUE + &
            	TRANSACTION_REVENUE

            IF(ECONOMY_REPORT_ACTIVE .AND. .NOT. TESTING_PLAN .AND. &
            	ABS(TOP_PROB-BOTTOM_PROB) > .00000001) THEN
...
!
               IF(KCOST_BLKNO > 2 .OR. KCOST_UNIT > NUNITS .OR. KCOST_UNIT < 1 &
               	.OR. LAST_TRANS_UNIT > NUNITS .OR. &
...
!
               IF(TRANSACTION_ENERGY_BOUGHT > 0.) THEN
                  TRANSACTION_AVE_BUY = .1*TRANSACTION_COST/ &
                  	TRANSACTION_ENERGY_BOUGHT
               ELSE
                  TRANSACTION_AVE_BUY = 0.
               ENDIF
               IF(TRANSACTION_ENERGY_SOLD > 0.) THEN
                  TRANSACTION_AVE_SELL = .1*TRANSACTION_REVENUE/ &
                  	TRANSACTION_ENERGY_SOLD
               ELSE
...
!
               DO WHILE(NEXT_K <= NUMBER_OF_SEGMENTS .AND. &
               	RIGHT(NEXT_K) >= RIGHT(K))
                  NEXT_K = NEXT_K + 1
               ENDDO
            ENDIF
...
!
         ECO_SALES_REV_FROM(U) = ECO_SALES_REV_FROM(U) + &
         	SEGMENT_ENERGY_REV(S)*SEAS_HOURS/10.
         ECO_SALES_ENRG_FROM(U) = ECO_SALES_ENRG_FROM(U) + &
         	SEGMENT_ENERGY_SOLD(S)*SEAS_HOURS
         ECO_PUCH_COST_FROM(U) = ECO_PUCH_COST_FROM(U) + &
         	SEGMENT_ENERGY_COST(S)*SEAS_HOURS/10.
         ECO_PUCH_ENRG_FROM(U) = ECO_PUCH_ENRG_FROM(U) + &
         	SEGMENT_ENERGY_BOUGHT(S)*SEAS_HOURS
!
         GRP = GENGRP(U)
         IF(GRP > 0 .AND. GRP <= MAX_REPORTING_GROUPS) THEN
            GROUP_ECO_SALES(GRP) = GROUP_ECO_SALES(GRP) + &
            	SEGMENT_ENERGY_SOLD(S)*SEAS_HOURS
            GROUP_ECO_PURCHASES(GRP)=GROUP_ECO_PURCHASES(GRP) + &
            	SEGMENT_ENERGY_BOUGHT(S)*SEAS_HOURS
         ENDIF
!
      ENDDO
...
!
      ANNUAL_NON_DISPATCHING_REV = ANNUAL_NON_DISPATCHING_REV + &
      	PERIOD_NON_DISPATCHING_REV/1000000.
!
!
...
!
      SUBROUTINE SRP_NEW_ECONOMY_INTERCHANGE(ENERGY,NBLOK2,BLKNO, &
      ANNUAL_ECONOMY_BOUGHT,ANNUAL_ECONOMY_SOLD, &
               ANNUAL_ECONOMY_COST,ANNUAL_ECONOMY_REVENUE,UNIT, &
               SEAS_HOURS,MAINTENANCE_RATE,UNITNM,PERIOD_COUNTER, &
               TRANSACTION_BUY_SPREAD,TRANSACTION_SELL_SPREAD, &
               MONTHLY_ECONOMY_BOUGHT,MONTHLY_ECONOMY_COST, &
               MONTHLY_ECONOMY_SOLD,MONTHLY_ECONOMY_REVENUE, &
...
!
      ...
      REAL (KIND=4) :: &
            ENERGY(2,MAX_CL_UNITS),ANNUAL_ECONOMY_BOUGHT,ANNUAL_ECONOMY_SOLD, &
            	ANNUAL_ECONOMY_COST,ANNUAL_ECONOMY_REVENUE, &
            MONTHLY_ECONOMY_BOUGHT,MONTHLY_ECONOMY_COST,MONTHLY_ECONOMY_SOLD, &
            MONTHLY_ECONOMY_REVENUE,TRANSACTION_BUY_SPREAD, &
            TRANSACTION_SELL_SPREAD,MAINTENANCE_RATE(MAX_CL_UNITS), &
            MWBLOK(MAX_DISPATCH_BLOCKS), &
            EXCESS_ENERGY_SALES(MAX_CL_UNITS), &
            SEGMENT_ENERGY_UNUSED,TEMP_SELL_ENERGY,TEMPEA(2,MAX_CL_UNITS), &
            SEAS_HOURS,GET_VAR, &
            EXCESS_ENERGY_SALES_PERCENT,EAVAIL(MAX_CL_UNITS), &
            SRP_SEGMENT_COST(*),PERIOD_NON_DISPATCHING_REV, &
            ANNUAL_NON_DISPATCHING_REV=0.,R_SRP_NON_DISPATCH_REV
      INTEGER (KIND=2) :: NBLOK2,BLKNO(MAX_DISPATCH_BLOCKS), &
      UNIT(MAX_DISPATCH_BLOCKS),PERIOD_COUNTER,U,ECBLK,SEG_NO,NUNITS,ISEAS
      ...
      CALL GET_BLOCK_DISP_COST(UNIT,BLKNO,BLOCK_DISP_COST,NBLOK2)
         ...
         DO SEG_NO = 1, NBLOK2
            ...
            ELSEIF(EXCESS_ENERGY_SALES(U) < 0.) THEN
               EXCESS_ENERGY_SALES_PERCENT = &
               	GET_VAR(EXCESS_ENERGY_SALES(U),PERIOD_COUNTER,UNITNM(U))
            ELSE
               EXCESS_ENERGY_SALES_PERCENT = EXCESS_ENERGY_SALES(U)
            ENDIF
            ECBLK = MAX(int(1,2),BLKNO(SEG_NO))
            SEGMENT_ENERGY_UNUSED = (MWBLOK(SEG_NO) * &
            	(1. - MAINTENANCE_RATE(U)) - ENERGY(ECBLK,U)) * EAVAIL(U)
            IF(SEGMENT_ENERGY_UNUSED == 0. .OR. TEMPEA(ECBLK,U) == 0.) CYCLE
            TEMP_SELL_ENERGY = MIN(SEGMENT_ENERGY_UNUSED, &
            MAX(0.,SEGMENT_ENERGY_UNUSED * EXCESS_ENERGY_SALES_PERCENT/100.))
!
...
!
            IF(.NOT. MARGIN_ACTIVE) THEN
               MONTHLY_ECONOMY_REVENUE = MONTHLY_ECONOMY_REVENUE + &
               TEMP_SELL_ENERGY*(BLOCK_DISP_COST(U,ECBLK) + &
                                           10.*TRANSACTION_SELL_SPREAD)
               ECO_SALES_REV_FROM(U) = ECO_SALES_REV_FROM(U) + SEAS_HOURS* &
               TEMP_SELL_ENERGY * (BLOCK_DISP_COST(U,ECBLK)/10.+ &
                                                TRANSACTION_SELL_SPREAD)
            ELSE
               MONTHLY_ECONOMY_REVENUE = MONTHLY_ECONOMY_REVENUE + &
               	TEMP_SELL_ENERGY*(SRP_SEGMENT_COST(SEG_NO) + &
                                            10.*TRANSACTION_SELL_SPREAD)
               ECO_SALES_REV_FROM(U) = ECO_SALES_REV_FROM(U) + SEAS_HOURS* &
               TEMP_SELL_ENERGY*(SRP_SEGMENT_COST(SEG_NO)/10.+ &
                                                TRANSACTION_SELL_SPREAD)
            ENDIF
            PERIOD_NON_DISPATCHING_REV = PERIOD_NON_DISPATCHING_REV + &
            SEAS_HOURS*TEMP_SELL_ENERGY*(SRP_SEGMENT_COST(SEG_NO) -  &
                                                BLOCK_DISP_COST(U,ECBLK))/10.
