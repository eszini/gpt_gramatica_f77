...
!
      ...
      SAVE FILE_EXISTS
      CHARACTER (LEN=2) :: FILE_OL='BC',R_FILE_OL
      CHARACTER (len=2) :: VARIABLE_USEAGE
      character (len=22) :: TAX_TIMING_CLASSIFICATION
      INTEGER (KIND=2), allocatable, save :: BC_ASSET_CLASS_POINTER(:), &
       OL_ASSET_CLASS_POINTER(:)
      integer (kind=2), allocatable :: TEMP_ASSET_CLASS_POINTER(:)
...
!
      ...
      CHARACTER (LEN=1) :: CONSOLIDATED_CLASS,DATA_TYPE
      character (len=1) :: ANNUAL_VALUE_STATUS(5), ACCOUNT_ACTIVE
      CHARACTER (LEN=10) :: TAX_APPLIES_TO
      CHARACTER (LEN=30) :: TAX_TYPE, TAX_RATES, CARRY_FORWARDS
      character (len=30) :: TAX_ON_CAPITAL,TAX_CREDITS_ADJUSTMENTS, &
       DEFERRED_TAX_AMT
      character (len=30) :: TAX_DEDUCTIONS,TAX_ACTUALS
...
! CONVERT THE TAX INFORMATION FILE
      ...
      FILE_NAME = trim(BASE_FILE_DIRECTORY())// &
      	FILE_PREFIX//"B"//trim(BASE_FILE_NAME)//".DAT"
...
!\
         ...
         OPEN(11,FILE=trim(DATA_DRIVE)//FILE_BC_NAME, &
...
!                      
            DO
               ...
               TAX_TYPE,TAX_APPLIES_TO,DATA_VALUES,DESCRIPTION,&
               COMMENT,DATA_TYPE, &
               escalation_vector_ord, &
               ASSET_CLASS_NUM,ASSET_CLASS_VECTOR, &
               DEFERRED_TAX_AMT,TAX_RATES, &
               CARRY_FORWARDS,TAX_ON_CAPITAL,TAX_CREDITS_ADJUSTMENTS, &
               TAX_DEDUCTIONS,CONSOLIDATED_CLASS, &
               MONTHLY_TAX_VECTOR,VARIABLE_USEAGE, &
               TAX_TIMING_CLASSIFICATION,TAX_ACTUALS, &
               ANNUAL_VALUE_STATUS, & ! 5 VALUES
               ACCOUNT_ACTIVE
!
             IF(DELETE < 8 .AND.(INDEX('Cl',VARIABLE_USEAGE) /= 0 .OR. &
             	INDEX('Pa',VARIABLE_USEAGE) /= 0)) then
               ...
               DO I = 2, AVAIL_DATA_YEARS+1
                  IF(DATA_VALUES(I) == -999999.) then
                  	DATA_VALUES(I) = DATA_VALUES(I-1)
                  end if
               ENDDO
               IREC = IREC + 1
               WRITE(11,REC=IREC) DELETE,ACCTNO, &
               TAX_TYPE,TAX_APPLIES_TO,DATA_VALUES,DESCRIPTION, &
               DATA_TYPE,escalation_vector_ord, &
	              ASSET_CLASS_NUM,ASSET_CLASS_VECTOR,DEFERRED_TAX_AMT, &
	              TAX_RATES,CARRY_FORWARDS,TAX_ON_CAPITAL, &
	              TAX_CREDITS_ADJUSTMENTS,TAX_DEDUCTIONS, &
	              CONSOLIDATED_CLASS,MONTHLY_TAX_VECTOR,VARIABLE_USEAGE, &
	              TAX_TIMING_CLASSIFICATION,TAX_ACTUALS, &
	              ANNUAL_VALUE_STATUS,   ACCOUNT_ACTIVE
            ENDDO
            IF(IOS /= 0) EXIT
         ENDDO
         CLOSE(10)
      ...
      ELSE IF(INDEX(BASE_FILE_NAME,'NONE') == 0) THEN
         CALL STOP_NOFILE(FILE_TYPE,FILE_NAME, "msgmmtax:1111")
      ENDIF
      RETURN
...
! OVERLAY THE ADDENDUM FILE
      ...
      ENDIF
      DATA_DRIVE = OUTPUT_DIRECTORY()
      FILE_NAME = trim(DATA_DRIVE)//FILE_PREFIX//"O"// &
      ...
      IF(FILE_OL == 'BC') THEN
         ...
         INUNIT = 11
      ENDIF
      OPEN(12,FILE=trim(DATA_DRIVE)//FILE_OL_NAME,ACCESS="DIRECT", &
      ...
      DO
         DO
            ...
            IF(RECLN(1:1) == '7') EXIT
            IREC = IREC + 1
            READ(INUNIT,REC=IREC,IOSTAT=IOS_BASE) DELETE,ACCTNO,TAX_TYPE, &
            	TAX_APPLIES_TO,DATA_VALUES,DESCRIPTION,DATA_TYPE, &
                escalation_vector_ord,ASSET_CLASS_NUM,ASSET_CLASS_VECTOR, &
                DEFERRED_TAX_AMT,TAX_RATES, &
                                     CARRY_FORWARDS,TAX_ON_CAPITAL, &
                                     TAX_CREDITS_ADJUSTMENTS,TAX_DEDUCTIONS, &
                                     CONSOLIDATED_CLASS, &
                                     MONTHLY_TAX_VECTOR,VARIABLE_USEAGE, &
                                     TAX_TIMING_CLASSIFICATION,TAX_ACTUALS, &
            ...
            IF(IOS == 0) THEN
               RECLN = trim(RECLN)//',,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,'
               READ(RECLN,*,ERR=200) DELETE,ACCTNO,TAX_TYPE,TAX_APPLIES_TO, &
               DATA_VALUES,TEMP_DESCRIPTION,COMMENT,DATA_TYPE, &
               escalation_vector_ord,ASSET_CLASS_NUM,ASSET_CLASS_VECTOR, &
               DEFERRED_TAX_AMT,TAX_RATES, &
               CARRY_FORWARDS,TAX_ON_CAPITAL,TAX_CREDITS_ADJUSTMENTS, &
               TAX_DEDUCTIONS,CONSOLIDATED_CLASS, &
               MONTHLY_TAX_VECTOR,VARIABLE_USEAGE,TAX_TIMING_CLASSIFICATION, &
               TAX_ACTUALS,ANNUAL_VALUE_STATUS, &
!                                5 VALUES
               ACCOUNT_ACTIVE
            ENDIF
            ...
            IF(DELETE < 8 .AND.(INDEX('Cl',VARIABLE_USEAGE) /= 0 .OR. &
             INDEX('Pa', VARIABLE_USEAGE) /= 0)) then
                        CALL SET_ASSET_CLASSES(ASSET_CLASS_NUM, &
                        NUMBER_OF_OL_CLASSES,MAX_OL_CLASS_ID_NUM, &
                        TEMP_ASSET_CLASS_POINTER)
             end if
             
            WRITE(12,REC=IREC) DELETE,ACCTNO, &
            TAX_TYPE,TAX_APPLIES_TO,DATA_VALUES,DESCRIPTION, &
            DATA_TYPE,escalation_vector_ord, &
            ASSET_CLASS_NUM,ASSET_CLASS_VECTOR,DEFERRED_TAX_AMT, &
            TAX_RATES,CARRY_FORWARDS,TAX_ON_CAPITAL, &
            TAX_CREDITS_ADJUSTMENTS,TAX_DEDUCTIONS, &
            CONSOLIDATED_CLASS,MONTHLY_TAX_VECTOR,VARIABLE_USEAGE, &
            TAX_TIMING_CLASSIFICATION,TAX_ACTUALS,ANNUAL_VALUE_STATUS, &
            ACCOUNT_ACTIVE
         ENDDO
         IF(IOS_BASE /= 0) EXIT
      ENDDO
      ...
      ENTRY OPEN_TAX_LOSS_FILE(R_UNIT_NUM,R_FILE_OL)
         ...
         IF(R_FILE_OL == 'BC') THEN
            OPEN(UNIT_NUM,FILE=trim(OUTPUT_DIRECTORY())// &
            	FILE_BC_NAME,ACCESS="DIRECT",STATUS="UNKNOWN",RECL=LRECL)
         ELSE
            OPEN(UNIT_NUM,FILE=trim(OUTPUT_DIRECTORY())// &
            	FILE_OL_NAME,ACCESS="DIRECT",STATUS="UNKNOWN",RECL=LRECL)
         ENDIF
      RETURN
...
!
      INTEGER (KIND=2) :: RUN_YEARS,EXTENSION_YEARS,FINANCIAL_SIMULATION_YEARS, &
      	DRILLING_REPORTING_YEARS
      INTEGER (KIND=2) :: NUM_OF_CLASSES,MAX_CLASS_NUM,TAX_TYPE
      INTEGER (kind=2) :: BASE_CASE_NUM_OF_CLASSES, &
               BASE_CASE_MAX_CLASS_NUM
      ...
      INTEGER (KIND=2) :: R_ASSET_CLASS,YR,TAX_POSITION,POINTER, &
      	ACTUAL_TAX_POSITION
      INTEGER (KIND=2) :: R_YR,APP
      REAL (KIND=4) :: TAX_VALUES(:,:,:,:,:), &
      TAX_VALUES_NOLS(:,:,:,:),ACTUAL_MONTHLY_TAX_VALUES(:,:,:,:)
      LOGICAL (KIND=1) :: ACTUAL_VALUE_FOUND(:,:)
      ALLOCATABLE :: TAX_VALUES,TAX_VALUES_NOLS, &
      ACTUAL_MONTHLY_TAX_VALUES, &
      	ACTUAL_VALUE_FOUND
      REAL (KIND=4) :: R_PROPERTY_TAX,R_OTHER_TAXES,R_REVENUE_TAXES
      REAL (KIND=4) :: R_FED_TAX_CREDITS,R_STATE_TAX_CREDITS, &
      	R_FED_INCOME_TAX_ADJ,R_STATE_INCOME_TAX_ADJ,R_M1_FED_ADDITIONS, &
        R_M1_FED_DEDUCTIONS,R_M1_STATE_ADDITIONS,R_M1_STATE_DEDUCTIONS, &
        R_DEFERRED_TAXES_CR,R_DEFERRED_TAXES_DR, &
...
!
      ...
      CHARACTER (LEN=10) :: TAX_APPLIES_TO
      CHARACTER (LEN=30) :: TAX_TYPE_STR,TAX_RATES,CARRY_FORWARDS, &
      	TAX_ON_CAPITAL,TAX_CREDITS_ADJUSTMENTS,DEFERRED_TAX_AMT, &
...
!
      ...
      REAL (KIND=4) :: MONTHLY_VALUES(12,LAST_AVAILABLE_MONTHLY_YEAR), &
          ALLOC_MONTHLY_TAX(12,LAST_AVAILABLE_MONTHLY_YEAR), &
...
!
      ...
      LOGICAL (KIND=1) :: DRILLING_REPORT_IS_INCOME, &
      	DRILLING_REPORT_IS_TAX,VALUE_IS_A_RATE, &
      	DONT_ADD_VALUES=.FALSE.
...
!
      REAL (KIND=4), allocatable :: DATA_VALUES(:),ALLOC_TAX(:), &
      	ANNUAL_TAX_VALUES(:),ASSET_CLASS_LIST(:),ASSET_ALLOCATION_LIST(:), &
...
!
      ...
      LOGICAL (KIND=1) :: TAX_ITEMS_IN_DRILLING,DRILLING_ITEM_CAN_BE_REPORTED, &
        NEEDS_ALLOCATION_VECTOR,ACTUAL_TAX_VALUES
...
!
      REAL (KIND=4) :: R_ACE_TAX_DEPRECIATION,R_TAX_PREFERENCE_DEPRECIATION, &
      	R_BTL_DEFERRED_TAXES_CR,R_BTL_DEFERRED_TAXES_DR, &
...
!
      ...
      REAL (KIND=4) :: R_PROPERTY_TAX_ADJUSTMENT(0:12), &
      	R_OTHER_TAX_ADJUSTMENT(0:12),R_REVENUE_TAX_ADJUSTMENT(0:12), &
        R_STATE_M1_ADDITIONS(0:12),R_STATE_M1_DEDUCTIONS(0:12), &
        R_FEDERAL_M1_ADDITIONS(0:12),R_FEDERAL_M1_DEDUCTIONS(0:12), &
        R_STATE_BTL_MISC_DEDUCTIONS(0:12), &
        R_FEDERAL_BTL_MISC_DEDUCTIONS(0:12),R_STATE_INCOME_TAX_ADJUSTMENT(0:12), &
        R_FEDERAL_INCOME_TAX_ADJUSTMENT(0:12)
      
      REAL (KIND=4) :: RM_TEMP_FED_TAX_RATE(0:12), &
      	RM_ITC_AMORTIZATION_RATE(0:12),RM_STATE_TAX_RATE(0:12), &
      	RM_OPREV_TAX_RATE(0:12), &
             RM_PROPERTY_TAX_RATE(0:12),RM_CLASS_OTHER_TAXES_RATE(0:12), &
             RM_OTHER_TAXES_PERCENT_OF_EXPEN(0:12), &
             RM_STATE_DEDUC_PERCT_OF_TIB4DED(0:12), &
             RM_FED_DEDUC_PERCT_OF_TIB4DEDUC(0:12)
      CHARACTER (LEN=1) :: R_CLASS_TYPE
      REAL (KIND=4) :: DEFAULT,CURRENT
!
      REAL (KIND=4) :: R_ACTUAL_DEFERRED_TAXES_CR(0:12), &
      	R_ACTUAL_FED_INCOME_TAXES(0:12),&
      	R_ACTUAL_STATE_INCOME_TAXES(0:12), &
        R_ACTUAL_DEFERRED_TAXES_DR(0:12)
      INTEGER (KIND=2) :: R_CLASS
!
      REAL (KIND=4) :: R_ACTUAL_INCOME_TAXES(0:12)
      REAL (KIND=4) :: ANNUAL_AMOUNT,REMAINING_ANNUAL_BALANCE,&
      UNALLOCATED_BALANCE,RATIO
!
      REAL (KIND=4) :: R_ANNUAL_STATE_INCOME_TAX,R_ANNUAL_FEDERAL_INCOME_TAX, &
        R_DEFERRED_TAXES_ACTUAL_DR,R_DEFERRED_TAXES_ACTUAL_CR
      LOGICAL (KIND=1) :: R_USE_ACTUAL_STATE_TAXES,R_USE_ACTUAL_FEDERAL_TAXES, &
        R_USE_ACTUAL_DEFERRED_TAXES_DR, &
                          R_USE_ACTUAL_DEFERRED_TAXES_CR
!
      REAL (KIND=4) :: R_FED_NOLS(*),R_STATE_NOLS(*),R_AMT_NOLS(*), &
        R_GENERAL_TAX_CREDITS(*),R_SEC_42_TAX_CREDITS(*), &
                       R_STATE_BUSINESS_CREDITS(*)
      LOGICAL (KIND=1) :: R_CLASS_EXITS
!
      REAL (KIND=4) :: R_TEMP_FED_TAX_RATE,R_ITC_AMORTIZATION_RATE, &
        R_STATE_TAX_RATE,R_OPREV_TAX_RATE,R_PROPERTY_TAX_RATE, &
             R_CLASS_OTHER_TAXES_RATE,R_OTHER_TAXES_PERCENT_OF_EXPEN, &
             R_STATE_DEDUC_PERCT_OF_TIB4DED, &
             R_FED_DEDUC_PERCT_OF_TIB4DEDUC
!
      REAL (KIND=4) :: R_PROVINCIAL_CAPITAL_TAX_RATE, &
      R_FEDERAL_CAPITAL_TAX_RATE,R_PROVINCIAL_CAP_TAX_DEDUCTION, &
             R_FEDERAL_CAPITAL_TAX_DEDUCTION,R_FEDERAL_CAPITAL_TAX_ADDENDUM, &
             R_PROVINCIAL_CAP_TAX_ADDENDUM
      ...
      ENDIF
!
      FINANCIAL_SIMULATION_YEARS = MAX(INT(15,2),LAST_AVAILABLE_MONTHLY_YEAR, &
      	RUN_YEARS() + EXTENSION_YEARS())
    ! the 15 is for lose carry forwards

      DRILLING_REPORTING_YEARS =  MAX(LAST_AVAILABLE_MONTHLY_YEAR, &
      	RUN_YEARS() + EXTENSION_YEARS())
      ...
      ALLOCATE(DATA_VALUES(0:AVAIL_DATA_YEARS), &
      	ANNUAL_TAX_VALUES(0:FINANCIAL_SIMULATION_YEARS), &
        REPORTING_MONTHLY_VALUES(0:12,0:FINANCIAL_SIMULATION_YEARS), &
        ALLOC_TAX(FINANCIAL_SIMULATION_YEARS), &
        OUTPUT_VALUE(FINANCIAL_SIMULATION_YEARS), &
      ...
      DO
         IREC = IREC + 1
         READ(11,REC=IREC,IOSTAT=IOS) DELETE,ACCTNO,TAX_TYPE_STR,TAX_APPLIES_TO, &
         	DATA_VALUES,DESCRIPTION,DATA_TYPE, &
            escalation_vector_ord,ASSET_CLASS_ID, &
            ASSET_ALLOCATION_VECTOR,DEFERRED_TAX_AMT,TAX_RATES, &
            CARRY_FORWARDS,TAX_ON_CAPITAL, &
            TAX_CREDITS_ADJUSTMENTS,TAX_DEDUCTIONS, &
            CONSOLIDATED_CLASS, &
            MONTHLY_TAX_VECTOR,VARIABLE_USEAGE, &
!                                                     5 VALUES
            ...
            ACCOUNT_ACTIVE
         IF(IOS /= 0) EXIT
         IF(DELETE > 7 .OR. ACCOUNT_ACTIVE == 'N' .OR. &
         	VARIABLE_USEAGE == 'No') then
         		CYCLE ! LOGICAL DELETE OR NOT ACTIVE
         end if
...
!
            CALL GET_MONTHLY_ANNUAL_VALUES(MONTHLY_TAX_VECTOR,DATA_TYPE, &
            	VECTOR_TYPE,DATA_VALUES(1),MONTHLY_VALUES(1,1), &
           		MONTHLY_DATA_UNITS,MONTH_ENDING)
            
            CALL RIPPLE_MONTHLY_DATA(DATA_VALUES(1),MONTHLY_VALUES)
!
            IF(ACTUAL_TAX_VALUES) THEN
               CALL MONTHLY_ACTUAL_VALUES(DATA_VALUES(1), &
               	MONTHLY_VALUES,MONTHLY_DATA_UNITS, &
               	MONTH_ENDING,ANNUAL_VALUE_STATUS)
               DO YR = 1, FINANCIAL_SIMULATION_YEARS
                  ANNUAL_TAX_VALUES(YR) = &
                  	DATA_VALUES(MIN(YR,AVAIL_DATA_YEARS))
               ENDDO
            ELSEIF(VALUE_IS_A_RATE) THEN
               IF(.NOT.(MONTHLY_DATA_UNITS(1) == DOLLARS .OR.  &
               	MONTHLY_DATA_UNITS(1) == PERCENT)) &
                MONTHLY_DATA_UNITS(1) = USE_ANNUAL_VALUES
               DO YR = 2, LAST_AVAILABLE_MONTHLY_YEAR
                  IF(MONTHLY_DATA_UNITS(YR) == TREND) then
                  	MONTHLY_DATA_UNITS(YR) = TRENDED_RATE
                  end if
               ENDDO
               CALL MONTHLY_BOOK_VALUES_NO_ADD(DATA_VALUES(1), &
               	MONTHLY_VALUES,MONTHLY_DATA_UNITS, &
               	MONTH_ENDING,DONT_ADD_VALUES)
               DO YR = 1, FINANCIAL_SIMULATION_YEARS
                  ANNUAL_TAX_VALUES(YR) =  &
                  	DATA_VALUES(MIN(YR,AVAIL_DATA_YEARS))
               ENDDO
            ELSE
               CALL MONTHLY_BOOK_VALUES_IN_DOLLARS(DATA_VALUES(1), &
               	MONTHLY_VALUES,MONTHLY_DATA_UNITS,MONTH_ENDING)
               ! TODO: Need a better way to document data types, other than 
               ! "A"!ddendum.
               
               IF(EXTENSION_PERIOD_IS_ADDENUM) then
                DATA_TYPE = 'A' ! "A"!DDENDUM 11/14/06 DR.G
               end if

               CALL CREATE_DOLLAR_STREAM(FINANCIAL_SIMULATION_YEARS, &
               	DATA_TYPE,escalation_vector_ord,DATA_VALUES,ANNUAL_TAX_VALUES)
            ENDIF
         ELSE
            IF(EXTENSION_PERIOD_IS_ADDENUM) then
                DATA_TYPE = 'A'!DDENDUM 11/14/06 DR.G
            end if
            CALL CREATE_DOLLAR_STREAM(FINANCIAL_SIMULATION_YEARS,DATA_TYPE, &
            	escalation_vector_ord,DATA_VALUES,ANNUAL_TAX_VALUES)
            ...
            ELSE
               ...
               ENDDO
            ENDIF
            CALL MONTHLY_BOOK_VALUES_IN_DOLLARS(ANNUAL_TAX_VALUES(1), &
            MONTHLY_VALUES,MONTHLY_DATA_UNITS,MONTH_ENDING)
         ENDIF
         DATA_TYPE = DOLLARS
...
!
         IF(VARIABLE_USEAGE == 'Co' .OR. CONSOLIDATED_CLASS == 'Y') THEN 
            ! Applying only to the Consolidated
            IF(ACTUAL_TAX_VALUES) THEN
               CALL ACTUAL_TAXES_ACCUMULATION(INT(-1,2), &
               ANNUAL_TAX_VALUES(1),TAX_CLASSIFICATION,MONTHLY_VALUES)
            ELSE
               CALL TAXES_INCOME_AND_OTHER(INT(-1,2),ANNUAL_TAX_VALUES(1), &
               	TAX_CLASSIFICATION,TAX_APPLIES_TO(1:1),MONTHLY_VALUES)
            ENDIF
         ENDIF
!
...
!
            IF(VARIABLE_USEAGE(1:1) == 'A') THEN
               DO ASSET_CLASS = -5, -2
                  CALL TAXES_INCOME_AND_OTHER(ASSET_CLASS,ANNUAL_TAX_VALUES(1), &
                  	TAX_CLASSIFICATION,TAX_APPLIES_TO(1:1), &
                  	MONTHLY_VALUES)
               ENDDO
            ELSE
         ...
         ELSEIF(INDEX(VARIABLE_USEAGE,'Co') == 0) THEN
           ...
           ALLOC_MONTHLY_TAX(MO,YR) = ASSET_ALLOCATOR * MONTHLY_VALUES(MO,YR)
                        ...
                        ENDIF
                     ELSE
                        IF(YR <= AVAIL_DATA_YEARS) ASSET_ALLOCATOR = &
                        	ALLOCATION_VALUE(YR)/100.
                        ALLOC_TAX(YR) = ASSET_ALLOCATOR * ANNUAL_TAX_VALUES(YR)
                        IF(YR <= LAST_AVAILABLE_MONTHLY_YEAR) THEN
                           DO MO = 1, 12
                              ALLOC_MONTHLY_TAX(MO,YR)=ASSET_ALLOCATOR * &
                              	MONTHLY_VALUES(MO,YR)
                           ENDDO
                        ENDIF
                     ENDIF
...
!
               IF(ACTUAL_TAX_VALUES) THEN
                  CALL ACTUAL_TAXES_ACCUMULATION(ASSET_CLASS,ALLOC_TAX, &
                  	TAX_CLASSIFICATION,ALLOC_MONTHLY_TAX)
               ELSE
                  CALL TAXES_INCOME_AND_OTHER(ASSET_CLASS,ALLOC_TAX, &
                    TAX_CLASSIFICATION,TAX_APPLIES_TO(1:1),ALLOC_MONTHLY_TAX)
!
               ENDIF
...
!
         IF(TAX_ITEMS_IN_DRILLING .AND. (DRILLING_REPORT_IS_TAX() .OR. &
         	DRILLING_REPORT_IS_INCOME())) THEN
...
!
            SELECT CASE (trim(DRILLING_NAME))
               ...
               CASE ('M1 Additions','M1 Deductions', &
                         'BTL Tax Deductions')
...
!
            ...
            DO YR = 1, DRILLING_REPORTING_YEARS
               ...
               ENDIF
            ENDDO
            VOID_INT2 = WRITE_DRILLING_RPT(DRILLING_NAME,DRILLING_ACCOUNT_NAME, &
            	REPORTING_MONTHLY_VALUES,REPORT_TYPE, &
                ANNUAL_INFO_ACTIVE)
           ...
           IF(.FALSE.) THEN
            ...
            IF(DRILLING_REPRT_LEVEL == 'A') THEN
               WRITE(DRILLING_REPORT_UNIT,REC=IREC) PRT_ENDPOINT(), &
               	DRILLING_NAME,DRILLING_ACCOUNT_NAME, &
                (ANNUAL_TAX_VALUES(YR),YR = 1,DRILLING_REPORTING_YEARS)
            ELSEIF(DRILLING_REPRT_LEVEL == 'M') THEN
               WRITE(DRILLING_REPORT_UNIT,REC=IREC) PRT_ENDPOINT(),DRILLING_NAME, &
               SHORT_MONTH_NAMES(0),DRILLING_ACCOUNT_NAME, &
                       (ANNUAL_TAX_VALUES(YR),YR = 1,DRILLING_REPORTING_YEARS)
               DO MO = 1, 12
                  DO YR = 1, DRILLING_REPORTING_YEARS
                     IF(YR >= 1 .AND. YR <= LAST_AVAILABLE_MONTHLY_YEAR) THEN
                  ...
                  ENDDO
                  WRITE(DRILLING_REPORT_UNIT,REC=IREC) PRT_ENDPOINT(), &
                  	DRILLING_NAME,SHORT_MONTH_NAMES(MO),DRILLING_ACCOUNT_NAME, &
...
!
      ...
      DEALLOCATE(DATA_VALUES,ALLOC_TAX,ANNUAL_TAX_VALUES,ASSET_CLASS_LIST, &
      	ASSET_ALLOCATION_LIST,OUTPUT_VALUE,& 
      ...
      ENTRY SET_TAX_ARRAYS
         ...
         CALL RETURN_INITIALIZATION_CLASSES(NUM_OF_EXP_CLASSES, &
         	MAX_EXP_CLASS_NUM)
...
!
         IF(ALLOCATED(TAX_VALUES)) then
         	DEALLOCATE(TAX_VALUES,TAX_VALUES_NOLS, &
         		ACTUAL_MONTHLY_TAX_VALUES,ACTUAL_VALUE_FOUND)
         end if
         
         ALLOCATE(TAX_VALUES(FINANCIAL_SIMULATION_YEARS,0:12, &
         	-5:NUM_OF_CLASSES,LAST_TAX_ITEM,3))
         	
         TAX_VALUES = 0.
         ALLOCATE(TAX_VALUES_NOLS(0:15,0:12,-5:NUM_OF_CLASSES,35:40))
         TAX_VALUES_NOLS = 0.
         ALLOCATE(ACTUAL_MONTHLY_TAX_VALUES(0:12,FINANCIAL_SIMULATION_YEARS, &
         	-1:NUM_OF_CLASSES,LAST_ACTUAL_TAX_ITEM))
...
!
         ...
         DO CLASSES = -5, NUM_OF_CLASSES
            DO PERIOD = 0, 12
               DO YR = 1, FINANCIAL_SIMULATION_YEARS
                  TAX_VALUES(YR,PERIOD,CLASSES, &
                  	Federal_Capitial_Tax_Rate,Both) = -999999.
                  TAX_VALUES(YR,PERIOD,CLASSES, &
                  	Local_Capitial_Tax_Rate,Both) = -999999.
               ENDDO
            ENDDO
         ENDDO
!
      ...
      ENTRY TAXES_INCOME_AND_OTHER(R_ASSET_CLASS,R_ALLOC_TAX, &
      	R_TAX_CLASSIFICATION,R_TAX_APPLICATION,R_ALLOC_MONTHLY_TAX)

!
         PERIOD = 0.
         TAX_TYPE = TAX_POSITION(R_TAX_CLASSIFICATION, &
         	R_TAX_APPLICATION,ADDED_TO_CURRENT_VALUE)
...
!
             TAX_VALUES_NOLS(YR,PERIOD,R_ASSET_CLASS,TAX_TYPE) = &
             	TAX_VALUES_NOLS(YR,PERIOD,R_ASSET_CLASS,TAX_TYPE) &
                 + R_ALLOC_TAX(YR)
             IF(YR <= LAST_AVAILABLE_MONTHLY_YEAR) THEN 
                DO MO = 1, 12
                   TAX_VALUES_NOLS(YR,MO,R_ASSET_CLASS,TAX_TYPE) = &
                   	TAX_VALUES_NOLS(YR,MO,R_ASSET_CLASS,TAX_TYPE) &
          ...
          ENDIF
                  IF(R_TAX_APPLICATION == 'B' .OR. R_TAX_APPLICATION == 'S') THEN
                     TAX_VALUES_NOLS(YR,PERIOD,R_ASSET_CLASS, &
                     	State_Credits_Carried_Forward) = &
                         TAX_VALUES_NOLS(YR,PERIOD,R_ASSET_CLASS, &
                         State_Credits_Carried_Forward) + R_ALLOC_TAX(YR)
                     IF(YR <= LAST_AVAILABLE_MONTHLY_YEAR) THEN 
                        DO MO = 1, 12
                           TAX_VALUES_NOLS(YR,MO,R_ASSET_CLASS, &
                           	State_Credits_Carried_Forward) = &
                            TAX_VALUES_NOLS(YR,MO,R_ASSET_CLASS, &
                            State_Credits_Carried_Forward) + &
                            R_ALLOC_MONTHLY_TAX(MO,YR)
      ...
      ENDDO
         ...
         IF(TAX_TYPE /= -99) THEN
            ...
            IF(ADDED_TO_CURRENT_VALUE) THEN
               THIS_IS_A_TIMING_VALUE = TAX_TYPE == &
               	M1_Additions .OR. TAX_TYPE == M1_Deductions .OR. &
               	TAX_TYPE == BTL_Tax_Deductions
     
               IF(THIS_IS_A_TIMING_VALUE) THEN
                  Timing_Type = Permanent_ATL_Tax_Differences ! 44
                  IF(INDEX(TAX_TIMING_CLASSIFICATION,'Oper') /=  0) THEN
                     IF(INDEX(TAX_TIMING_CLASSIFICATION,'Temp') /=  0) &
                     	Timing_Type = Temporary_ATL_Tax_Differences ! 42
                  ELSEIF(INDEX(TAX_TIMING_CLASSIFICATION,'BTL')/= 0)THEN
                     Timing_Type = Permanent_BTL_Tax_Differences ! 45
                     IF(INDEX(TAX_TIMING_CLASSIFICATION,'Temp') /=  0) then
                     	Timing_Type = Temporary_BTL_Tax_Differences ! 43
                     end if
                  ENDIF
               ENDIF
!
...
!
               IF(Timing_Type /= 0) THEN
                  SIGN = 1.0
                  IF(TAX_TYPE == M1_Deductions .OR. TAX_TYPE == &
                  	BTL_Tax_Deductions) SIGN = -1.0
                  DO YR = 1, FINANCIAL_SIMULATION_YEARS
                     TAX_VALUES(YR,PERIOD,R_ASSET_CLASS, &
                     	Timing_Type,APP) = SIGN * R_ALLOC_TAX(YR) + &
                        TAX_VALUES(YR,PERIOD,R_ASSET_CLASS,Timing_Type,APP)
                     IF(YR <= LAST_AVAILABLE_MONTHLY_YEAR) THEN 
        DO MO = 1, 12
           TAX_VALUES(YR,MO,R_ASSET_CLASS, &
               Timing_Type,APP) = SIGN * R_ALLOC_MONTHLY_TAX(MO,YR) + &
      ...
      ENTRY ACTUAL_TAXES_ACCUMULATION(R_ASSET_CLASS,R_ALLOC_TAX, &
      	R_TAX_CLASSIFICATION,R_ALLOC_MONTHLY_TAX)
            ...
            IF(.NOT.ACTUAL_VALUE_FOUND(R_ASSET_CLASS,TAX_TYPE))THEN
               DO YR = 1, FINANCIAL_SIMULATION_YEARS
                  IF(R_ALLOC_TAX(YR) /= -99999.) THEN
                     IF(ACTUAL_MONTHLY_TAX_VALUES(0,YR, &
                     R_ASSET_CLASS,TAX_TYPE) == -99999.) THEN
                        ACTUAL_MONTHLY_TAX_VALUES(0,YR,R_ASSET_CLASS,TAX_TYPE) = 0.
                     ENDIF
                     ACTUAL_MONTHLY_TAX_VALUES(0,YR,R_ASSET_CLASS,TAX_TYPE) = &
                     	ACTUAL_MONTHLY_TAX_VALUES(0,YR, &
                        R_ASSET_CLASS,TAX_TYPE) + R_ALLOC_TAX(YR)
                  ENDIF
                  IF(YR > LAST_AVAILABLE_MONTHLY_YEAR) CYCLE
                  DO MO = 1, 12
                     IF(R_ALLOC_MONTHLY_TAX(MO,YR) == -99999.) CYCLE
                     IF(ACTUAL_MONTHLY_TAX_VALUES(MO,YR, &
                        R_ASSET_CLASS,TAX_TYPE) == &
                     -99999.) THEN
                        ACTUAL_MONTHLY_TAX_VALUES(MO,YR,R_ASSET_CLASS, &
                            TAX_TYPE) = 0.
                     ENDIF
                     ACTUAL_MONTHLY_TAX_VALUES(MO,YR,R_ASSET_CLASS,TAX_TYPE) = &
                     	ACTUAL_MONTHLY_TAX_VALUES(MO,YR, &
...
!
      ...
      ENTRY RETURN_M1_TIMING_DIFFERENCES(R_ASSET_CLASS,R_YR,R_TAX_TYPE, &
        MONTHLY_VARS)
...
!
         IF(ALLOCATED(TAX_VALUES) .AND. R_ASSET_CLASS <= MAX_CLASS_NUM) THEN
            ...
            IF(POINTER > 0 .OR. R_ASSET_CLASS == 0) THEN
               DO MO = 0, 12
                  ! TODO: Long equations like this should be in 
                  ! a function
                  MONTHLY_VARS(MO,monthly_permanent_atl_tax_dif) = &
                  	TAX_VALUES(YR,MO,POINTER,Permanent_ATL_Tax_Differences, &
                    R_TAX_TYPE) + TAX_VALUES(YR,MO,POINTER, &
                    Permanent_ATL_Tax_Differences,Both)
                    
                  ! TODO: Long equations like this should be in 
                  ! a function                    
                  MONTHLY_VARS(MO,monthly_permanent_btl_tax_dif) = &
                  	TAX_VALUES(YR,MO,POINTER,Permanent_BTL_Tax_Differences, &
                    R_TAX_TYPE) + TAX_VALUES(YR,MO,POINTER, &
                    Permanent_BTL_Tax_Differences,Both)
                    
                  MONTHLY_VARS(MO,monthly_temporary_atl_tax_dif) = &
                  	TAX_VALUES(YR,MO,POINTER,Temporary_ATL_Tax_Differences, &
                    R_TAX_TYPE) + TAX_VALUES(YR,MO,POINTER, &
                    Temporary_ATL_Tax_Differences,Both)
                  MONTHLY_VARS(MO,monthly_temporary_btl_tax_dif) = &
                  TAX_VALUES(YR,MO,POINTER,Temporary_BTL_Tax_Differences, &
                  R_TAX_TYPE) + TAX_VALUES(YR,MO,POINTER, &
                  Temporary_BTL_Tax_Differences,Both)
               ENDDO
            ENDIF
         ENDIF
      RETURN

      ENTRY RETURN_TAX_INFO(R_ASSET_CLASS,R_YR,R_PROPERTY_TAX,R_OTHER_TAXES, &
      	R_REVENUE_TAXES,R_FED_TAX_CREDITS,R_STATE_TAX_CREDITS, &
        R_FED_INCOME_TAX_ADJ,R_STATE_INCOME_TAX_ADJ,R_M1_FED_ADDITIONS, &
        R_M1_FED_DEDUCTIONS, &
        R_M1_STATE_ADDITIONS,R_M1_STATE_DEDUCTIONS,R_DEFERRED_TAXES_CR, &
        R_DEFERRED_TAXES_DR,R_ITC_AMORTIZATION, &
        R_SEC_29_CREDITS,R_SEC_42_CREDITS,R_ACE_TAX_DEPRECIATION, &
        R_TAX_PREFERENCE_DEPRECIATION, &
        R_BTL_DEFERRED_TAXES_CR,R_BTL_DEFERRED_TAXES_DR,R_AMT_INCOME_ADDENDUM, &
        R_INCOME_TAX_DEPRECIATION, &
...
!
         ...
         IF(ALLOCATED(TAX_VALUES) .AND. R_ASSET_CLASS <= MAX_CLASS_NUM) THEN
            ...
            PERIOD = 0
            IF(POINTER > 0 .OR. R_ASSET_CLASS == 0) THEN 
               R_PROPERTY_TAX = R_PROPERTY_TAX + &
               	TAX_VALUES(YR,PERIOD,POINTER,PropertyTaxes,Both)            
               R_OTHER_TAXES = R_OTHER_TAXES + TAX_VALUES(YR,PERIOD, &
               POINTER,OtherTaxes,Both)            
               R_REVENUE_TAXES = R_REVENUE_TAXES + &
               TAX_VALUES(YR,PERIOD,POINTER,Revenue_Taxes,Both)
               R_FED_TAX_CREDITS = R_FED_TAX_CREDITS + &
               TAX_VALUES(YR,PERIOD,POINTER,Income_Tax_Credits,Both) + &
               TAX_VALUES(YR,PERIOD,POINTER,Income_Tax_Credits,Federal)
               
               R_SEC_29_CREDITS = R_SEC_29_CREDITS + &
               	TAX_VALUES(YR,PERIOD,POINTER,Sec_29_Credits,Both) + &
                TAX_VALUES(YR,PERIOD,POINTER,Sec_29_Credits,Federal)
                
               R_SEC_42_CREDITS = R_SEC_42_CREDITS + &
               	TAX_VALUES(YR,PERIOD,POINTER,Sec42_Credits,Both) + &
                TAX_VALUES(YR,PERIOD,POINTER,Sec42_Credits,Federal)
                
               R_STATE_TAX_CREDITS = R_STATE_TAX_CREDITS + &
               	TAX_VALUES(YR,PERIOD,POINTER,Income_Tax_Credits,Both) + &
                TAX_VALUES(YR,PERIOD,POINTER,Income_Tax_Credits,State)
                
               R_FED_INCOME_TAX_ADJ = R_FED_INCOME_TAX_ADJ + &
               	TAX_VALUES(YR,PERIOD,POINTER,Income_Tax_Adjustments,Federal) + &
                TAX_VALUES(YR,PERIOD,POINTER,Income_Tax_Adjustments,Both)
                
               R_STATE_INCOME_TAX_ADJ = R_STATE_INCOME_TAX_ADJ + &
               	TAX_VALUES(YR,PERIOD,POINTER,Income_Tax_Adjustments,State) + &
                TAX_VALUES(YR,PERIOD,POINTER,Income_Tax_Adjustments,Both)
                        
               R_M1_FED_ADDITIONS = R_M1_FED_ADDITIONS + &
               	TAX_VALUES(YR,PERIOD,POINTER,M1_Additions,Federal) + &
                TAX_VALUES(YR,PERIOD,POINTER,M1_Additions,Both)
                
               R_M1_FED_DEDUCTIONS = R_M1_FED_DEDUCTIONS + &
               	TAX_VALUES(YR,PERIOD,POINTER,M1_Deductions,Federal) + &
                TAX_VALUES(YR,PERIOD,POINTER,M1_Deductions,Both)
                
               R_M1_STATE_ADDITIONS = R_M1_STATE_ADDITIONS + &
               	TAX_VALUES(YR,PERIOD,POINTER,M1_Additions,State) + &
                TAX_VALUES(YR,PERIOD,POINTER,M1_Additions,Both)
                
               R_M1_STATE_DEDUCTIONS = R_M1_STATE_DEDUCTIONS + &
               	TAX_VALUES(YR,PERIOD,POINTER,M1_Deductions,State) + &
                TAX_VALUES(YR,PERIOD,POINTER,M1_Deductions,Both)
                
               R_DEFERRED_TAXES_CR = R_DEFERRED_TAXES_CR + &
               	TAX_VALUES(YR,PERIOD,POINTER,Deferred_TaxesCr,Both)
               	
               R_BTL_DEFERRED_TAXES_CR = R_BTL_DEFERRED_TAXES_CR + &
               	TAX_VALUES(YR,PERIOD,POINTER,BTL_Deferred_TaxesCr,Both)
               	
               R_DEFERRED_TAXES_DR = R_DEFERRED_TAXES_DR + &
               	TAX_VALUES(YR,PERIOD,POINTER,Deferred_TaxesDr,Both)
               
               R_BTL_DEFERRED_TAXES_DR = R_BTL_DEFERRED_TAXES_DR + &
               	TAX_VALUES(YR,PERIOD,POINTER,BTL_Deferred_TaxesDr,Both)
               	
               R_ITC_AMORTIZATION = R_ITC_AMORTIZATION + &
               	TAX_VALUES(YR,PERIOD,POINTER,ITCAmortization,Both)
               R_ACE_TAX_DEPRECIATION = R_ACE_TAX_DEPRECIATION + &
                TAX_VALUES(YR,PERIOD, &
               	POINTER,AMT_ACE_Tax_Depreciation,Federal)
               	
               R_TAX_PREFERENCE_DEPRECIATION = R_TAX_PREFERENCE_DEPRECIATION + &
               	TAX_VALUES(YR,PERIOD,POINTER, &
                AMT_Depreciation_Preference,Federal)
                
               R_AMT_INCOME_ADDENDUM = TAX_VALUES(YR,PERIOD,POINTER, &
               	AMTIncome_Addendum,Federal)
               	
               R_INCOME_TAX_DEPRECIATION = R_INCOME_TAX_DEPRECIATION + &
               	TAX_VALUES(YR,PERIOD,POINTER,Tax_Depreciation,Both)
               	
               R_BTL_MISC_DEDUCTIONS = TAX_VALUES(YR,PERIOD,POINTER, &
               	BTL_Tax_Deductions,Both)
            ENDIF            
         ENDIF
      RETURN
...
!
         IF(ALLOCATED(TAX_VALUES)) THEN
            ...
            PERIOD = 0
            POINTER = -1
            R_DEFERRED_TAXES_DR = TAX_VALUES(YR,PERIOD,POINTER, &
            	Deferred_TaxesDr,Both)
         ELSE
      ...
      ENTRY MONTHLY_CONSOL_TAX_ITEMS(R_YR,R_CONSOL_DEFERRED_TAXES_DR)
         ...
         IF(ALLOCATED(TAX_VALUES)) THEN
            ...
            YR = R_YR - 1
            DO MO = 0, 12
               R_CONSOL_DEFERRED_TAXES_DR(MO)=TAX_VALUES(YR,MO,POINTER, &
               	Deferred_TaxesDr,Both)
            ENDDO
         ELSE
            DO MO = 0, 12
      ...
      ENTRY MONTHLY_PARENT_TAX_ITEMS(R_YR,R_ASSET_CLASS, &
        R_MONTHLY_DEFERRED_TAXES_DR)
...
!
         IF(ALLOCATED(TAX_VALUES) .AND. R_ASSET_CLASS <= MAX_CLASS_NUM) THEN
            ...
            IF(POINTER > 0) THEN
               YR = R_YR - 1
               DO MO = 0, 12
                  R_MONTHLY_DEFERRED_TAXES_DR(MO) = TAX_VALUES(YR,MO,POINTER, &
                  	Deferred_TaxesDr,Both)
               ENDDO
            ELSE
               DO MO = 0, 12
      ...
      ENTRY MONTHLY_TAX_ADJUSTMENTS(R_ASSET_CLASS,R_YR,R_PROPERTY_TAX_ADJUSTMENT, &
        R_OTHER_TAX_ADJUSTMENT,R_REVENUE_TAX_ADJUSTMENT, &
         R_STATE_M1_ADDITIONS,R_STATE_M1_DEDUCTIONS,R_FEDERAL_M1_ADDITIONS, &
         R_FEDERAL_M1_DEDUCTIONS, &
         R_STATE_BTL_MISC_DEDUCTIONS,R_FEDERAL_BTL_MISC_DEDUCTIONS, &
...
!
         ...
         IF(ALLOCATED(TAX_VALUES) .AND. R_ASSET_CLASS <= MAX_CLASS_NUM) THEN
            ...
            YR = R_YR
            IF(POINTER >= -1) THEN
               R_PROPERTY_TAX_ADJUSTMENT(:) = &
               	TAX_VALUES(YR,:,POINTER,PropertyTaxes,Both)
               	
               R_OTHER_TAX_ADJUSTMENT(:) = &
               	TAX_VALUES(YR,:,POINTER,OtherTaxes,Both)
               R_REVENUE_TAX_ADJUSTMENT(:) = &
               	TAX_VALUES(YR,:,POINTER,Revenue_Taxes,Both)
               R_FEDERAL_M1_ADDITIONS(:) = &
               	TAX_VALUES(YR,:,POINTER,M1_Additions,Federal) &
                + TAX_VALUES(YR,:,POINTER,M1_Additions,Both)
               R_FEDERAL_M1_DEDUCTIONS(:) = &
               	TAX_VALUES(YR,:,POINTER,M1_Deductions,Federal) &
                + TAX_VALUES(YR,:,POINTER,M1_Deductions,Both) + &
                TAX_VALUES(YR,:,POINTER,BTL_Tax_Deductions,Federal) &
                + TAX_VALUES(YR,:,POINTER,BTL_Tax_Deductions,Both)
                
               R_STATE_M1_ADDITIONS(:) = &
               	TAX_VALUES(YR,:,POINTER,M1_Additions,State) + &
               		TAX_VALUES(YR,:,POINTER,M1_Additions,Both)
               R_STATE_M1_DEDUCTIONS(:) = TAX_VALUES(YR,:, &
               	POINTER,M1_Deductions,State) &
                    + TAX_VALUES(YR,:,POINTER, &
                    M1_Deductions,Both) + TAX_VALUES(YR,:,POINTER, &
                    BTL_Tax_Deductions,State) &
                    + TAX_VALUES(YR,:,POINTER,BTL_Tax_Deductions,Both)
                    
               R_STATE_BTL_MISC_DEDUCTIONS(:) = TAX_VALUES(YR,:,POINTER, &
               BTL_Tax_Deductions,State) &
               + TAX_VALUES(YR,:,POINTER,BTL_Tax_Deductions,Both)
               ...
               R_FEDERAL_INCOME_TAX_ADJUSTMENT(:) = &
               	TAX_VALUES(YR,:,POINTER,Income_Tax_Adjustments,Federal) + &
                TAX_VALUES(YR,:,POINTER,Income_Tax_Adjustments,Both)
                
               R_STATE_INCOME_TAX_ADJUSTMENT(:) = &
               	TAX_VALUES(YR,:,POINTER,Income_Tax_Adjustments,State) + &
               		TAX_VALUES(YR,:,POINTER, &
...
!
      ...
      ENTRY MONTHLY_NON_INCOME_TAX_RATES(R_YR,R_ASSET_CLASS,RM_OPREV_TAX_RATE, &
        RM_PROPERTY_TAX_RATE,RM_CLASS_OTHER_TAXES_RATE, &
        RM_OTHER_TAXES_PERCENT_OF_EXPEN,R_CLASS_TYPE)

      NON_INCOME_TAXES = .TRUE. 

      ENTRY RETURN_MONTHLY_TAX_RATES(R_YR,R_ASSET_CLASS,RM_TEMP_FED_TAX_RATE, &
        RM_ITC_AMORTIZATION_RATE,RM_STATE_TAX_RATE, &
        RM_STATE_DEDUC_PERCT_OF_TIB4DED, &
            RM_FED_DEDUC_PERCT_OF_TIB4DEDUC,R_CLASS_TYPE)
...
!
                  IF(TAX_VALUES(YR,MO,int(CURRENT,2), &
                  	Operating_Revenue_Tax_Rate,Both) /= -999999.) THEN
                     	RM_OPREV_TAX_RATE(MO) = &
                     	TAX_VALUES(YR,MO,int(CURRENT,2), &
                     	Operating_Revenue_Tax_Rate,Both)/100.
                  ELSEIF(TAX_VALUES(YR,MO,int(DEFAULT,2), &
                  Operating_Revenue_Tax_Rate,Both) /= -999999.) THEN
                     RM_OPREV_TAX_RATE(MO) = &
                     	TAX_VALUES(YR,MO,int(DEFAULT,2), &
                     	Operating_Revenue_Tax_Rate,Both)/100.
                  ELSE
...
!
                  IF(TAX_VALUES(YR,MO,int(CURRENT,2), &
                  	Property_TaxRate,Both) /= -999999.) THEN
                     RM_PROPERTY_TAX_RATE(MO)=TAX_VALUES(YR,MO, &
                     int(CURRENT,2),Property_TaxRate,Both)/100.
                  ELSEIF(TAX_VALUES(YR,MO,int(DEFAULT,2), &
                  Property_TaxRate,Both) /= -999999.) THEN
                     RM_PROPERTY_TAX_RATE(MO)= &
                     TAX_VALUES(YR,MO,int(DEFAULT,2),Property_TaxRate,Both)/100.
                  ELSE
...
!
                  IF(TAX_VALUES(YR,MO,int(CURRENT,2), &
                  	Other_Taxes_of_Revenues_Rate,Both) /= -999999.) THEN
                    	RM_CLASS_OTHER_TAXES_RATE(MO) = &
                    		TAX_VALUES(YR,MO,int(CURRENT,2), &
                    		Other_Taxes_of_Revenues_Rate,Both)/100.
                  ELSEIF(TAX_VALUES(YR,MO,int(DEFAULT,2), &
                  	Other_Taxes_of_Revenues_Rate,Both) /= -999999.) THEN
                     RM_CLASS_OTHER_TAXES_RATE(MO) = &
                     	TAX_VALUES(YR,MO,int(DEFAULT,2), &
...
!
                  IF(TAX_VALUES(YR,MO,int(CURRENT,2), &
                  	Other_Taxes_of_Expenses_Rate,Both) /= -999999.) THEN
                     RM_OTHER_TAXES_PERCENT_OF_EXPEN(MO) = &
                     	TAX_VALUES(YR,MO,int(CURRENT,2), &
                     	Other_Taxes_of_Expenses_Rate,Both)/100.
                  ELSEIF(TAX_VALUES(YR,MO,int(DEFAULT,2), &
                  Other_Taxes_of_Expenses_Rate,Both) /= -999999.) THEN
                     RM_OTHER_TAXES_PERCENT_OF_EXPEN(MO) = &
                     TAX_VALUES(YR,MO,int(DEFAULT,2),&
...
!
                  IF(TAX_VALUES(YR,MO,int(CURRENT,2), &
                  	Federal_Income_Tax,Both) /= -999999.) THEN
                     RM_TEMP_FED_TAX_RATE(MO)=TAX_VALUES(YR,MO,int(CURRENT,2), &
                     Federal_Income_Tax,Both)
                  ELSEIF(TAX_VALUES(YR,MO,int(DEFAULT,2), &
                  Federal_Income_Tax,Both) /= -999999.) THEN
                     RM_TEMP_FED_TAX_RATE(MO)=TAX_VALUES(YR,MO, &
                     int(DEFAULT,2),Federal_Income_Tax,Both)
                  ENDIF
!
! ITC AMORTIZATION RATE
!     
                  IF(TAX_VALUES(YR,MO,int(CURRENT,2),ITC_AmortizationRate,Both) /= &
                  -999999.) THEN
                     RM_ITC_AMORTIZATION_RATE(MO)=TAX_VALUES(YR,MO, &
                        int(CURRENT,2),ITC_AmortizationRate,Both)
                  ELSEIF(TAX_VALUES(YR,MO,int(DEFAULT,2),ITC_AmortizationRate, &
                    Both) /= -999999.) THEN
                     RM_ITC_AMORTIZATION_RATE(MO)=TAX_VALUES(YR,MO, &
                        int(DEFAULT,2),ITC_AmortizationRate,Both)
                  ENDIF
!
! STATE TAX RATE
!
                  IF(TAX_VALUES(YR,MO,int(CURRENT,2),Local_TaxRate,Both) &
                    /= -999999.) THEN
                     RM_STATE_TAX_RATE(MO) = TAX_VALUES(YR,MO,int(CURRENT,2), &
                        Local_TaxRate,Both)
                  ELSEIF(TAX_VALUES(YR,MO,int(DEFAULT,2),Local_TaxRate,Both) /= &
                    -999999.) THEN
                     RM_STATE_TAX_RATE(MO) = TAX_VALUES(YR,MO,int(DEFAULT,2), &
                        Local_TaxRate,Both)
                  ENDIF
!
...
!
                  IF(TAX_VALUES(YR,MO,int(CURRENT,2), &
                    Taxable_Income_Deductions_Rate,Federal) /= -999999.) THEN
                     RM_FED_DEDUC_PERCT_OF_TIB4DEDUC(MO) = &
                        TAX_VALUES(YR,MO,int(CURRENT,2), &
                            Taxable_Income_Deductions_Rate,Federal)
                  ELSEIF(TAX_VALUES(YR,MO,int(CURRENT,2), &
                    Taxable_Income_Deductions_Rate,Both)/=-999999.) THEN
                     RM_FED_DEDUC_PERCT_OF_TIB4DEDUC(MO) = &
                        TAX_VALUES(YR,MO,int(CURRENT,2), &
                            Taxable_Income_Deductions_Rate,Both)
                  ELSEIF(TAX_VALUES(YR,MO,int(DEFAULT,2), &
                    Taxable_Income_Deductions_Rate,Federal)/= -999999.) THEN
                     RM_FED_DEDUC_PERCT_OF_TIB4DEDUC(MO) = &
                        TAX_VALUES(YR,MO,int(DEFAULT,2),Taxable_Income_Deductions_Rate, &
                            Federal)
                  ELSEIF(TAX_VALUES(YR,MO,int(DEFAULT,2), &
                    Taxable_Income_Deductions_Rate,Both)/=-999999.) THEN
                     RM_FED_DEDUC_PERCT_OF_TIB4DEDUC(MO) = &
...
!
                  IF(TAX_VALUES(YR,MO,int(CURRENT,2), &
                    Taxable_Income_Deductions_Rate,State)/=-999999.) THEN
                     RM_FED_DEDUC_PERCT_OF_TIB4DEDUC(MO) = &
                        TAX_VALUES(YR,MO,int(CURRENT,2), &
                            Taxable_Income_Deductions_Rate,State)
                  ELSEIF(TAX_VALUES(YR,MO,int(CURRENT,2), &
                    Taxable_Income_Deductions_Rate,Both)/=-999999.) THEN
                     RM_FED_DEDUC_PERCT_OF_TIB4DEDUC(MO) = &
                        TAX_VALUES(YR,MO,int(CURRENT,2),&
                  ...
                  ELSEIF(TAX_VALUES(YR,MO,int(DEFAULT,2), &
                    Taxable_Income_Deductions_Rate,State)/=-999999.) THEN
                     RM_FED_DEDUC_PERCT_OF_TIB4DEDUC(MO) = &
                        TAX_VALUES(YR,MO,int(DEFAULT,2),Taxable_Income_Deductions_Rate,State)
                  ELSEIF(TAX_VALUES(YR,MO,int(DEFAULT,2),&
                    Taxable_Income_Deductions_Rate,Both)/=-999999.) THEN
                     RM_FED_DEDUC_PERCT_OF_TIB4DEDUC(MO) = &
                        TAX_VALUES(YR,MO,int(DEFAULT,2), &
      ...
      RETURN
!**********************************************************************
      ENTRY RETURN_ACTUAL_TAX_VALUES(R_YR,R_CLASS, &
        R_ACTUAL_DEFERRED_TAXES_CR,R_ACTUAL_FED_INCOME_TAXES, &
                                     R_ACTUAL_STATE_INCOME_TAXES,R_ACTUAL_DEFERRED_TAXES_DR)
!**********************************************************************
!
         CALL ACTUAL_MONTHLY_INCOME_TAX_ITEMS(R_YR,R_CLASS, &
            R_ACTUAL_DEFERRED_TAXES_CR,INT(1,2))
         CALL ACTUAL_MONTHLY_INCOME_TAX_ITEMS(R_YR,R_CLASS, &
            R_ACTUAL_FED_INCOME_TAXES,INT(2,2)) ! Federal_Income_Tax
         CALL ACTUAL_MONTHLY_INCOME_TAX_ITEMS(R_YR,R_CLASS, &
            R_ACTUAL_STATE_INCOME_TAXES,INT(3,2)) ! STATE INCOME TAX
         CALL ACTUAL_MONTHLY_INCOME_TAX_ITEMS(R_YR,R_CLASS, &
            R_ACTUAL_DEFERRED_TAXES_DR,INT(4,2))
      RETURN

      ENTRY ACTUAL_MONTHLY_INCOME_TAX_ITEMS(R_YR,R_CLASS, &
        R_ACTUAL_INCOME_TAXES,R_TAX_TYPE)
...
!
                  IF(ACTUAL_VALUE_FOUND(ASSET_CLASS,R_TAX_TYPE)) THEN
                     ANNUAL_AMOUNT = 0.
                     DO MO = 1, 12
                        IF(ACTUAL_MONTHLY_TAX_VALUES(MO,YR,ASSET_CLASS, &
                            R_TAX_TYPE) == -99999.) THEN
                           IF(ACTUAL_MONTHLY_TAX_VALUES(0,YR, &
                            ASSET_CLASS,R_TAX_TYPE) == -99999.) THEN 
                              DO MO2 = MO, 12
                                 ANNUAL_AMOUNT = ANNUAL_AMOUNT + &
                                    R_ACTUAL_INCOME_TAXES(MO2)
                              ENDDO
                           ELSE ! MARK TO INPUT VALUE
                              ...
                              REMAINING_ANNUAL_BALANCE = &
                                ACTUAL_MONTHLY_TAX_VALUES(0,YR,ASSET_CLASS, &
                                    R_TAX_TYPE) - ANNUAL_AMOUNT
                              IF(UNALLOCATED_BALANCE /= 0.) THEN
                                 RATIO = REMAINING_ANNUAL_BALANCE/ &
                                 UNALLOCATED_BALANCE
                              ELSE
                                 RATIO = 1./FLOAT(13-MO)
                              ENDIF
                              DO MO2 = MO, 12
                                 R_ACTUAL_INCOME_TAXES(MO2) = RATIO * &
                                    R_ACTUAL_INCOME_TAXES(MO2)
                                 ANNUAL_AMOUNT = ANNUAL_AMOUNT + &
                                    R_ACTUAL_INCOME_TAXES(MO2)
                              ENDDO
                           ...
                           EXIT
                        ENDIF
                        R_ACTUAL_INCOME_TAXES(MO) = &
                            ACTUAL_MONTHLY_TAX_VALUES(MO,YR,ASSET_CLASS, &
                                R_TAX_TYPE)
      ...
      ENTRY RETURN_MONTHLY_ACTUAL_TAXES(R_YR,R_CLASS,R_ANNUAL_STATE_INCOME_TAX, &
       R_USE_ACTUAL_STATE_TAXES, &
       R_ANNUAL_FEDERAL_INCOME_TAX,R_USE_ACTUAL_FEDERAL_TAXES, &
       R_DEFERRED_TAXES_ACTUAL_DR, &
       R_USE_ACTUAL_DEFERRED_TAXES_DR,R_DEFERRED_TAXES_ACTUAL_CR,&
       R_USE_ACTUAL_DEFERRED_TAXES_CR)
...
! FEDERAL TAXES PAID
!
        R_ANNUAL_FEDERAL_INCOME_TAX = ACTUAL_MONTHLY_TAX_VALUES(0, &
        YR,ASSET_CLASS,2)
        IF(ACTUAL_VALUE_FOUND(ASSET_CLASS,2) .AND. &
            R_ANNUAL_FEDERAL_INCOME_TAX /= -99999.) then
...
!
       ...
       IF(ACTUAL_VALUE_FOUND(ASSET_CLASS,3) .AND. &
        R_ANNUAL_STATE_INCOME_TAX /= -99999.) then
...
!
       ...
       IF(ACTUAL_VALUE_FOUND(ASSET_CLASS,1) .AND. R_DEFERRED_TAXES_ACTUAL_CR &
        /= -99999.) then
...
! DEFERRED TAXES CR
!
                  R_DEFERRED_TAXES_ACTUAL_DR = ACTUAL_MONTHLY_TAX_VALUES(0, &
                    YR,ASSET_CLASS,4)
                  IF(ACTUAL_VALUE_FOUND(ASSET_CLASS,4) .AND. &
                    R_DEFERRED_TAXES_ACTUAL_DR /= -99999.) then
      ...
      ENTRY RETURN_TAX_NOLS_CREDITS(R_ASSET_CLASS,R_CLASS_EXITS,R_FED_NOLS,&
        R_STATE_NOLS,R_AMT_NOLS,R_GENERAL_TAX_CREDITS, &
        R_SEC_42_TAX_CREDITS,R_STATE_BUSINESS_CREDITS)

!
         R_CLASS_EXITS = .FALSE.
         IF(ALLOCATED(TAX_VALUES_NOLS) .AND. (R_ASSET_CLASS) <= MAX_CLASS_NUM) &
         THEN
            IF(R_ASSET_CLASS == 0) THEN
               POINTER = 0
            ELSEIF(R_ASSET_CLASS == -1) THEN
            ...
            IF(POINTER > 0 .OR. R_ASSET_CLASS == 0 .OR. R_ASSET_CLASS == -1) THEN
               ...
               DO YR = 1,  15
                  ...
                  R_GENERAL_TAX_CREDITS(YR) = TAX_VALUES_NOLS(YR,PERIOD, &
                    POINTER,General_Carry_Tax_Credits)
                  R_SEC_42_TAX_CREDITS(YR) = TAX_VALUES_NOLS(YR,PERIOD,POINTER, &
                    Sec_42_Carry_Tax_Credits)
                  R_STATE_BUSINESS_CREDITS(YR) = TAX_VALUES_NOLS(YR,PERIOD, &
                    POINTER,State_Credits_Carried_Forward)
               ENDDO
            ENDIF            
         ENDIF
      RETURN

      ENTRY RETURN_TAX_RATES(R_YR,R_ASSET_CLASS,R_TEMP_FED_TAX_RATE, &
      R_ITC_AMORTIZATION_RATE,R_STATE_TAX_RATE,R_OPREV_TAX_RATE, &
      R_PROPERTY_TAX_RATE,R_CLASS_OTHER_TAXES_RATE, &
        R_OTHER_TAXES_PERCENT_OF_EXPEN, &
...
! FEDERAL TAX RATE
!
        IF(TAX_VALUES(YR,PERIOD,int(CURRENT,2), &
            Federal_Income_Tax,Both) /= -999999.) THEN
           R_TEMP_FED_TAX_RATE = TAX_VALUES(YR,PERIOD, &
            int(CURRENT,2),Federal_Income_Tax,Both)
        ELSEIF(TAX_VALUES(YR,PERIOD,int(DEFAULT,2), &
            Federal_Income_Tax,Both) /= -999999.) THEN
           R_TEMP_FED_TAX_RATE = TAX_VALUES(YR,PERIOD,int(DEFAULT,2), &
            Federal_Income_Tax,Both)
        ENDIF
!
! ITC AMORTIZATION RATE
...
!
      ...
      ELSEIF(TAX_VALUES(YR,PERIOD,int(DEFAULT,2),Local_TaxRate,Both) /= -999999.) &
        THEN
...
!
      IF(TAX_VALUES(YR,PERIOD,int(CURRENT,2), &
      Operating_Revenue_Tax_Rate,Both) /= -999999.) THEN
         R_OPREV_TAX_RATE = TAX_VALUES(YR,PERIOD,int(CURRENT,2), &
         Operating_Revenue_Tax_Rate,Both)
      ELSEIF(TAX_VALUES(YR,PERIOD,int(DEFAULT,2), &
      Operating_Revenue_Tax_Rate,Both) /= -999999.) THEN
         R_OPREV_TAX_RATE = TAX_VALUES(YR,PERIOD,int(DEFAULT,2), &
         Operating_Revenue_Tax_Rate,Both)
      ENDIF
!
! PROPERTY TAX RATE
!
      IF(TAX_VALUES(YR,PERIOD,int(CURRENT,2), &
        Property_TaxRate,Both) /= -999999.) THEN
         R_PROPERTY_TAX_RATE = TAX_VALUES(YR,PERIOD, &
         int(CURRENT,2),Property_TaxRate,Both)
      ELSEIF(TAX_VALUES(YR,PERIOD,int(DEFAULT,2), &
      	Property_TaxRate,Both) /= -999999.) THEN
         R_PROPERTY_TAX_RATE = TAX_VALUES(YR,PERIOD, &
         	int(DEFAULT,2),Property_TaxRate,Both)
      ENDIF
!
! OTHER TAXES REVENUE RATE
!
               IF(TAX_VALUES(YR,PERIOD,int(CURRENT,2), &
               Other_Taxes_of_Revenues_Rate,Both) /= -999999.) THEN
                  R_CLASS_OTHER_TAXES_RATE = &
                  TAX_VALUES(YR,PERIOD,int(CURRENT,2), &
                    Other_Taxes_of_Revenues_Rate,Both)
               ELSEIF(TAX_VALUES(YR,PERIOD, &
               int(DEFAULT,2),Other_Taxes_of_Revenues_Rate,Both) /= -999999.) THEN
                  R_CLASS_OTHER_TAXES_RATE = &
                  TAX_VALUES(YR,PERIOD,int(DEFAULT,2), &
                    Other_Taxes_of_Revenues_Rate,Both)
               ENDIF
!
! OTHER TAXES EXPENSE RATE
!
               IF(TAX_VALUES(YR,PERIOD,int(CURRENT,2), &
                Other_Taxes_of_Expenses_Rate, Both) /= -999999.) THEN
                  R_OTHER_TAXES_PERCENT_OF_EXPEN = &
                  	TAX_VALUES(YR,PERIOD,int(CURRENT,2), &
                  		Other_Taxes_of_Expenses_Rate,Both)
               ELSEIF(TAX_VALUES(YR,PERIOD,int(DEFAULT,2), &
               Other_Taxes_of_Expenses_Rate,Both) /= -999999.) THEN
                  R_OTHER_TAXES_PERCENT_OF_EXPEN = TAX_VALUES(YR,PERIOD, &
                  	int(DEFAULT,2),Other_Taxes_of_Expenses_Rate,Both)
               ENDIF
!
! TAX DEDUCTIONS FEDERAL
!
               IF(TAX_VALUES(YR,PERIOD,int(CURRENT,2), &
                  ...
                  R_FED_DEDUC_PERCT_OF_TIB4DEDUC = &
                  	TAX_VALUES(YR,PERIOD,int(CURRENT,2), &
                  	Taxable_Income_Deductions_Rate,Federal)
               ELSEIF(TAX_VALUES(YR,PERIOD,int(CURRENT,2), &
               	Taxable_Income_Deductions_Rate,Both)/=-999999.) THEN
                  R_FED_DEDUC_PERCT_OF_TIB4DEDUC = &
                  	TAX_VALUES(YR,PERIOD,int(CURRENT,2), &
                  		Taxable_Income_Deductions_Rate,Both)
               ...
               ELSEIF(TAX_VALUES(YR,PERIOD,int(DEFAULT,2), &
               	Taxable_Income_Deductions_Rate,Both)/=-999999.) THEN
                  R_FED_DEDUC_PERCT_OF_TIB4DEDUC = TAX_VALUES(YR,PERIOD, &
                  int(DEFAULT,2),Taxable_Income_Deductions_Rate,Both)
               ENDIF
!
! TAX DEDUCTIONS STATE
!
...
!
         ...
         IF(ALLOCATED(TAX_VALUES)) THEN
            ...
            IF(POINTER >= -1 .OR. R_ASSET_CLASS == 0) THEN
               IF(TAX_VALUES(YR,PERIOD,POINTER, &
               Operating_Revenue_Tax_Rate,Both) /= -999999.) THEN
                  R_OPREV_TAX_RATE = TAX_VALUES(YR,PERIOD,POINTER, &
                  Operating_Revenue_Tax_Rate,Both)
               ELSEIF(TAX_VALUES(YR,PERIOD,-1,     &  ! ALL DEFAULT
                     Operating_Revenue_Tax_Rate,Both) /= -999999.) THEN
                  R_OPREV_TAX_RATE = TAX_VALUES(YR,PERIOD,-1, &
                  Operating_Revenue_Tax_Rate,Both)
               ELSEIF(TAX_VALUES(YR,PERIOD,-4,    &  ! SUB DEFAULT
                      Operating_Revenue_Tax_Rate,Both) /= -999999.) THEN
                  R_OPREV_TAX_RATE = TAX_VALUES(YR,PERIOD,-4, &
                  Operating_Revenue_Tax_Rate,Both)
               ELSEIF(TAX_VALUES(YR,PERIOD,-3,   &   ! SBU DEFAULT
                     Operating_Revenue_Tax_Rate,Both) /= -999999.) THEN
                  R_OPREV_TAX_RATE = TAX_VALUES(YR,PERIOD,-3, &
                  Operating_Revenue_Tax_Rate,Both)
               ELSEIF(TAX_VALUES(YR,PERIOD,-2,  &    ! PARENT DEFAULT
                     Operating_Revenue_Tax_Rate,Both) /= -999999.) THEN
                  R_OPREV_TAX_RATE = TAX_VALUES(YR,PERIOD,-2, &
                  Operating_Revenue_Tax_Rate,Both)
               ELSEIF(TAX_VALUES(YR,PERIOD,-5,  &    ! REG DEFAULT
                     Operating_Revenue_Tax_Rate,Both) /= -999999.) THEN
                  R_OPREV_TAX_RATE = TAX_VALUES(YR,PERIOD,-5, &
                  Operating_Revenue_Tax_Rate,Both)
               ENDIF
            ENDIF
         ELSE
      ...
      ENTRY RETURN_CAPITAL_TAX_ITEMS(R_YR,R_ASSET_CLASS, &
        R_PROVINCIAL_CAPITAL_TAX_RATE,R_FEDERAL_CAPITAL_TAX_RATE, &
         R_PROVINCIAL_CAP_TAX_DEDUCTION,R_FEDERAL_CAPITAL_TAX_DEDUCTION, &
         R_FEDERAL_CAPITAL_TAX_ADDENDUM,R_PROVINCIAL_CAP_TAX_ADDENDUM, &
         R_CLASS_TYPE)
...
!
               IF(TAX_VALUES(YR,PERIOD,int(CURRENT,2), &
               Local_Capitial_Tax_Rate,Both) /= -999999.) THEN
                  R_PROVINCIAL_CAPITAL_TAX_RATE = &
                  TAX_VALUES(YR,PERIOD,int(CURRENT,2), &
                    Local_Capitial_Tax_Rate,Both)
               ELSEIF(TAX_VALUES(YR,PERIOD,int(DEFAULT,2), &
               Local_Capitial_Tax_Rate,Both) /= -999999.) THEN
                  R_PROVINCIAL_CAPITAL_TAX_RATE = &
                  TAX_VALUES(YR,PERIOD,int(DEFAULT,2), &
               ...
               ENDIF
               IF(TAX_VALUES(YR,PERIOD,int(CURRENT,2), &
               Federal_Capitial_Tax_Rate,Both) /= -999999.) THEN
                  R_FEDERAL_CAPITAL_TAX_RATE = &
                  TAX_VALUES(YR,PERIOD,int(CURRENT,2), &
                    Federal_Capitial_Tax_Rate,Both)
               ELSEIF(TAX_VALUES(YR,PERIOD,int(DEFAULT,2), &
               Federal_Capitial_Tax_Rate,Both) /= -999999.) THEN
                  R_FEDERAL_CAPITAL_TAX_RATE = TAX_VALUES(YR,PERIOD, &
                  int(DEFAULT,2),Federal_Capitial_Tax_Rate,Both)
               ENDIF
               R_PROVINCIAL_CAP_TAX_DEDUCTION = R_PROVINCIAL_CAP_TAX_DEDUCTION + &
               	TAX_VALUES(YR,PERIOD,int(CURRENT,2), &
               	Local_Capitial_Tax_Deduction,Both)
               	
               R_FEDERAL_CAPITAL_TAX_DEDUCTION = &
               	R_FEDERAL_CAPITAL_TAX_DEDUCTION + &
               	TAX_VALUES(YR,PERIOD,int(CURRENT,2), &
                Federal_Capitial_Tax_Deduction,Both)
                
               R_FEDERAL_CAPITAL_TAX_ADDENDUM = &
               R_FEDERAL_CAPITAL_TAX_ADDENDUM + &
                TAX_VALUES(YR,PERIOD,int(CURRENT,2), &
      ...
      RETURN
!**********************************************************************
      ENTRY RETURN_TAX_INFO_ELIM(R_YR,R_ASSET_CLASS,R_PROPERTY_TAX, &
        R_OTHER_TAXES,R_REVENUE_TAXES)
!**********************************************************************
!
         IF(ALLOCATED(TAX_VALUES) .AND. (R_ASSET_CLASS) &
            <= MAX_CLASS_NUM) THEN
            PERIOD = 0
            IF(R_ASSET_CLASS == 0) THEN
               POINTER = 0
            ...
            IF(POINTER > 0 .OR. R_ASSET_CLASS == 0) THEN 
               YR = R_YR - 1
               R_PROPERTY_TAX = R_PROPERTY_TAX + TAX_VALUES(YR,PERIOD, &
                POINTER,PropertyTaxes,Both)            
               R_OTHER_TAXES = R_OTHER_TAXES + TAX_VALUES(YR,PERIOD, &
                POINTER,OtherTaxes,Both)            
               R_REVENUE_TAXES = R_REVENUE_TAXES + TAX_VALUES(YR,PERIOD, &
                POINTER,Revenue_Taxes,Both)
            ENDIF
         ENDIF
      RETURN
!**********************************************************************
      ENTRY RETURN_TAX_ADJUSTMENTS_ELIM(R_YR,R_ASSET_CLASS, &
        R_FED_INCOME_TAX_ADJ,R_STATE_INCOME_TAX_ADJ)
!**********************************************************************
!
         IF(ALLOCATED(TAX_VALUES)) THEN
            PERIOD = 0
            YR = R_YR - 1
            R_FED_INCOME_TAX_ADJ = R_FED_INCOME_TAX_ADJ + TAX_VALUES(YR, &
                PERIOD,-1,Income_Tax_Adjustments,Federal) &
            ...
            IF(R_ASSET_CLASS <= MAX_CLASS_NUM) THEN
               ...
               IF(POINTER > 0 .OR. R_ASSET_CLASS == 0) THEN 
                  R_FED_INCOME_TAX_ADJ = R_FED_INCOME_TAX_ADJ + TAX_VALUES(YR, &
                    PERIOD,POINTER,Income_Tax_Adjustments,Federal) &
                       + TAX_VALUES(YR,PERIOD,POINTER,Income_Tax_Adjustments,Both)
                  R_STATE_INCOME_TAX_ADJ = R_STATE_INCOME_TAX_ADJ + TAX_VALUES( &
                    YR,PERIOD,POINTER,Income_Tax_Adjustments,State) &
                       + TAX_VALUES(YR,PERIOD,POINTER, &
                       Income_Tax_Adjustments,Both)
               ENDIF
            ENDIF
...
!
         IF(ALLOCATED(TAX_VALUES)) THEN
            ...
            WRITE(10) TAX_VALUES,TAX_VALUES_NOLS,ACTUAL_MONTHLY_TAX_VALUES, &
                ACTUAL_VALUE_FOUND
            CLOSE(10)
...
!        CALL SET_TAX_ARRAYS
         ...
         IF(BASE_CASE_MAX_CLASS_NUM > 0) THEN
            ...
            CALL RETURN_TAX_CLASS_POINTER(ASSET_CLASS_POINTER)
         ENDIF
         IF(ALLOCATED(TAX_VALUES)) DEALLOCATE(TAX_VALUES,TAX_VALUES_NOLS, &
            ACTUAL_MONTHLY_TAX_VALUES,ACTUAL_VALUE_FOUND)
         ALLOCATE(TAX_VALUES(FINANCIAL_SIMULATION_YEARS,0:12, &
            -5:BASE_CASE_NUM_OF_CLASSES,LAST_TAX_ITEM,3))
         ALLOCATE(TAX_VALUES_NOLS(0:15,0:12,-5:BASE_CASE_NUM_OF_CLASSES,35:40))
         ALLOCATE(ACTUAL_MONTHLY_TAX_VALUES(0:12,FINANCIAL_SIMULATION_YEARS,-1: &
            BASE_CASE_NUM_OF_CLASSES,LAST_ACTUAL_TAX_ITEM))
         ALLOCATE(ACTUAL_VALUE_FOUND &
            (-1:BASE_CASE_NUM_OF_CLASSES,LAST_ACTUAL_TAX_ITEM))
         CALL OPEN_TAX_BASE_CASE_FILE(INT(10,2))
         READ(10) TAX_VALUES,TAX_VALUES_NOLS,ACTUAL_MONTHLY_TAX_VALUES, &
            ACTUAL_VALUE_FOUND
         CLOSE(10)
      RETURN
      END
...
!   ALL RIGHTS RESERVED
      ...
      FUNCTION READ_TAX_LOSSES(MAX_CLASS_NUM,NUM_OF_ACTIVE_CLASSES, &
        MASTER_CLASS_LIST)
...
! TAX LOSS VARIABLES
!
      INTEGER (KIND=2) :: MAX_CLASS_NUM,MASTER_CLASS_LIST(0:MAX_CLASS_NUM), &
        NUM_OF_ACTIVE_CLASSES
!
      INTEGER (KIND=2) :: YRS,READ_TAX_LOSSES,I,CLASS,CLASS_POS
      INTEGER (KIND=4) :: VALUES_2_ZERO
      REAL (KIND=4) :: FEDERAL_BOOK_LOSSES(:,:),STATE_BOOK_LOSSES(:,:), &
        GENERAL_BUSINESS_CREDITS(:,:),AMT_LOSSES(:,:), &
           UNUSED_FED_NOLS(:),UNUSED_STATE_NOLS(:),UNUSED_AMT_NOLS(:), &
            SEC_42_CREDITS(:,:),STATE_BUSINESS_CREDITS(:,:)
      REAL (KIND=4) :: UNUSED_FED_CREDITS(:),UNUSED_STATE_CREDITS(:), &
        UNUSED_FED_SEC_42_CREDITS(:)
      ALLOCATABLE :: FEDERAL_BOOK_LOSSES,STATE_BOOK_LOSSES, &
      GENERAL_BUSINESS_CREDITS,AMT_LOSSES,UNUSED_FED_NOLS,UNUSED_STATE_NOLS, &
                     UNUSED_AMT_NOLS,UNUSED_FED_CREDITS, &
                     UNUSED_FED_SEC_42_CREDITS,UNUSED_STATE_CREDITS,SEC_42_CREDITS, &
                     STATE_BUSINESS_CREDITS
      SAVE  FEDERAL_BOOK_LOSSES,STATE_BOOK_LOSSES,GENERAL_BUSINESS_CREDITS, &
      AMT_LOSSES,UNUSED_FED_NOLS,UNUSED_STATE_NOLS, &
            UNUSED_AMT_NOLS,UNUSED_FED_CREDITS,UNUSED_FED_SEC_42_CREDITS, &
            UNUSED_STATE_CREDITS,SEC_42_CREDITS,STATE_BUSINESS_CREDITS
      ...
      REAL (KIND=4) :: ADJUST_4_STATE_NOLS,ADJUST_4_FED_NOLS, &
        CLOSE_CLASS_STATE_TAX_INFO,CLOSE_CLASS_FED_TAX_INFO,ADJUST_4_AMT_NOLS
      REAL (KIND=4) :: ADJUST_4_FED_CONSOLIDATED_NOLS, &
      CLOSE_CONSOLIDATED_TAX_INFO,FED_CONSOLIDATED_NOLS_USED, &
      ...
      LOGICAL (KIND=1) :: RETURN_NOLS_INFO,RETURN_CONSOLIDATED_NOLS_INFO, &
        RETURN_CURRENT_NOLS_INFO
      REAL (KIND=4) :: ADJUST_4_FED_CREDIT,FED_CREDIT_USED,CLOSE_FED_CREDIT
      SAVE FED_CREDIT_USED
      REAL (KIND=4) :: AVAIL_CREDITS,R_AVAILABLE_CREDITS, &
        R_SEC_29_CREDITS_PASSED_UP,R_SEC_42_CREDITS_PASSED_UP, &
            R_SEC_29_CREDITS_USED
      REAL (KIND=4) :: R_STATE_NOL_GENERATED,R_STATE_NOLS_USED, &
      R_FEDERAL_NOL_GENERATED,R_FED_NOLS_USED,R_TAXES_B4_CREDITS, &
                       R_AMT_TAX,R_AMT_CREDITS_USED
      REAL (KIND=4) :: ADJUST_CONSOLD_4_FED_CREDIT,CONSOLIDATED_FED_CREDIT_USED, &
      UNUSED_CONSOLIDATED_FED_CREDITS, &
                       AVAIL_CONSOLIDATED_CREDITS
      SAVE CONSOLIDATED_FED_CREDIT_USED,UNUSED_CONSOLIDATED_FED_CREDITS, &
      AVAIL_CONSOLIDATED_CREDITS
      REAL (KIND=4) :: TAX_OWED,SEC_29_CREDITS_USED,SEC_29_CREDITS_LOST
      SAVE SEC_29_CREDITS_USED,SEC_29_CREDITS_LOST
      REAL (KIND=4) :: AMT_CREDITS_USED,AVAILABLE_AMT_CREDITS, &
        SAVE_AMT_PARENT_CREDITS,R_AVAILABLE_AMT_CREDITS
      ...
      REAL (KIND=4) :: R_STATE_CREDITS_PASSED_UP,R_STATE_CREDITS_USED, &
        STATE_CREDITS_USED
...
!
         ...
         ALLOCATE(FEDERAL_BOOK_LOSSES(-1:YRS,-1:MAX_CLASS_NUM), &
            SEC_42_CREDITS(-1:YRS,-1:MAX_CLASS_NUM), &
                  STATE_BOOK_LOSSES(-1:YRS,-1:MAX_CLASS_NUM), &
                  GENERAL_BUSINESS_CREDITS(-1:YRS,-1:MAX_CLASS_NUM), &
                  STATE_BUSINESS_CREDITS(-1:YRS,-1:MAX_CLASS_NUM), &
                  AMT_LOSSES(-1:YRS,-1:MAX_CLASS_NUM), &
                  UNUSED_FED_NOLS(-1:MAX_CLASS_NUM), &
                  UNUSED_AMT_NOLS(-1:MAX_CLASS_NUM), &
                  UNUSED_STATE_NOLS(-1:MAX_CLASS_NUM), &
                  UNUSED_FED_CREDITS(-1:MAX_CLASS_NUM), &
                  UNUSED_FED_SEC_42_CREDITS(-1:MAX_CLASS_NUM), &
...
!
         ...
         DO ! CLASS = 1, MAX_CLASS_NUM
            CALL RETURN_TAX_NOLS_CREDITS(CLASS,CLASS_EXITS, &
            FEDERAL_BOOK_LOSSES(2,CLASS),STATE_BOOK_LOSSES(2,CLASS), &
                   AMT_LOSSES(2,CLASS), &
                   GENERAL_BUSINESS_CREDITS(2,CLASS),SEC_42_CREDITS(2,CLASS), &
                   STATE_BUSINESS_CREDITS(2,CLASS))
            IF(CLASS_EXITS) THEN
               DO I = 2, 16 
                  UNUSED_STATE_NOLS(CLASS)=UNUSED_STATE_NOLS(CLASS) + &
                    STATE_BOOK_LOSSES(I,CLASS)   
                  UNUSED_FED_NOLS(CLASS) = UNUSED_FED_NOLS(CLASS) + &
                    FEDERAL_BOOK_LOSSES(I,CLASS)
                  UNUSED_AMT_NOLS(CLASS) = UNUSED_AMT_NOLS(CLASS) + &
                    AMT_LOSSES(I,CLASS)
                  UNUSED_FED_CREDITS(CLASS) = UNUSED_FED_CREDITS(CLASS) + &
                    GENERAL_BUSINESS_CREDITS(I,CLASS) &
                                     + SEC_42_CREDITS(I,CLASS)
                  SEC_42_CREDITS(I,CLASS) = SEC_42_CREDITS(I,CLASS) + &
                  GENERAL_BUSINESS_CREDITS(I,CLASS)
                  UNUSED_FED_SEC_42_CREDITS(CLASS) = &
                  UNUSED_FED_SEC_42_CREDITS(CLASS) + SEC_42_CREDITS(I,CLASS)
                  UNUSED_STATE_CREDITS(CLASS) = UNUSED_STATE_CREDITS(CLASS) + &
                  STATE_BUSINESS_CREDITS(I,CLASS)
               ENDDO
            ENDIF
            IF(CLASS == -1) THEN
               CLASS = 1
...
!
      ...
      ENTRY ADJUST_4_STATE_CREDITS(R_CLASS,R_TAXES_B4_CREDITS, &
      R_STATE_CREDITS_PASSED_UP,R_USE_ALL_CREDITS)

!
         AVAIL_CREDITS = UNUSED_STATE_CREDITS(R_CLASS) + &
            R_STATE_CREDITS_PASSED_UP
!
...
!
         ...
         DO YR = R_YEAR, R_YEAR+15
            IF(STATE_CREDITS_USED > 0.) THEN
               IF(STATE_CREDITS_USED > STATE_BUSINESS_CREDITS(YR,R_CLASS)) THEN
                  STATE_CREDITS_USED = STATE_CREDITS_USED - &
                    STATE_BUSINESS_CREDITS(YR,R_CLASS)
                  STATE_BUSINESS_CREDITS(YR,R_CLASS) = 0.
               ELSE   
                  STATE_BUSINESS_CREDITS(YR,R_CLASS) = &
                    STATE_BUSINESS_CREDITS(YR,R_CLASS) - STATE_CREDITS_USED
                  STATE_CREDITS_USED = 0.
               ENDIF
            ENDIF
            IF(YR == R_YEAR) CYCLE
            UNUSED_STATE_CREDITS(R_CLASS)=UNUSED_STATE_CREDITS(R_CLASS) + &
                STATE_BUSINESS_CREDITS(YR,R_CLASS)
      ...
      ENTRY ADJUST_4_FED_CREDIT(R_CLASS,R_AMT_TAX,R_TAXES_B4_CREDITS, &
      R_AVAILABLE_CREDITS,R_SEC_29_CREDITS_PASSED_UP, &
      R_SEC_42_CREDITS_PASSED_UP,R_SEC_29_CREDITS_USED,R_AMT_CREDITS_USED, &
        R_AVAILABLE_AMT_CREDITS)

!                                                      THESE ARE NEW CREDITS
         AVAIL_CREDITS = UNUSED_FED_CREDITS(R_CLASS) + R_AVAILABLE_CREDITS & 
                        + R_SEC_42_CREDITS_PASSED_UP
         AVAIL_CONSOLIDATED_CREDITS = UNUSED_CONSOLIDATED_FED_CREDITS + &
            R_AVAILABLE_CREDITS + R_SEC_42_CREDITS_PASSED_UP
!
         AMT_CREDITS_USED = 0.
         ...
         IF(TAX_OWED-SEC_29_CREDITS_USED >= R_AMT_TAX) THEN
            ...
            ELSE
               ...
               IF(AVAILABLE_AMT_CREDITS >=  TAX_OWED-R_AMT_TAX- &
               SEC_29_CREDITS_USED-FED_CREDIT_USED) THEN
               
                  AMT_CREDITS_USED = TAX_OWED - R_AMT_TAX - &
                    SEC_29_CREDITS_USED-FED_CREDIT_USED
               ELSE
      ...
      ENTRY CLOSE_FED_CREDIT(R_CLASS,R_YEAR,R_AVAILABLE_CREDITS, &
        R_AVAILABLE_AMT_CREDITS)      
...
!
         ...
         DO YR = R_YEAR, R_YEAR+15
            IF(FED_CREDIT_USED > 0.) THEN
               ...
               ELSE   
                  SEC_42_CREDITS(YR,R_CLASS)=SEC_42_CREDITS(YR,R_CLASS) - &
                    FED_CREDIT_USED
                  FED_CREDIT_USED = 0.
               ENDIF
            ENDIF
            IF(YR == R_YEAR) CYCLE
            UNUSED_FED_CREDITS(R_CLASS) = UNUSED_FED_CREDITS(R_CLASS) +  &
                SEC_42_CREDITS(YR,R_CLASS)
      ...
      ENTRY CLOSE_CLASS_AMT_TAX_INFO(R_CLASS,R_YEAR)
         ...
         DO I = R_YEAR+1, R_YEAR+15 
            UNUSED_AMT_NOLS(CLASS_POS) = UNUSED_AMT_NOLS(CLASS_POS) + &
                AMT_LOSSES(I,CLASS_POS)
         ENDDO
      RETURN
      ...
      ENTRY ADJUST_CONSOLD_4_FED_CREDIT(R_AMT_TAX,R_TAXES_B4_CREDITS, &
        R_AVAILABLE_CREDITS,R_SEC_29_CREDITS_PASSED_UP, &
        R_SEC_42_CREDITS_PASSED_UP,R_SEC_29_CREDITS_USED,R_AMT_CREDITS_USED, &
            R_AVAILABLE_AMT_CREDITS)
...
!
         ...
         ELSE
            ...
            SEC_29_CREDITS_LOST = 0.
            IF(TAX_OWED-SEC_29_CREDITS_USED >= R_AMT_TAX) THEN
               IF(AVAIL_CONSOLIDATED_CREDITS >= &
                TAX_OWED-R_AMT_TAX-SEC_29_CREDITS_USED) THEN
                  CONSOLIDATED_FED_CREDIT_USED = TAX_OWED-R_AMT_TAX- &
                    SEC_29_CREDITS_USED
               ELSE
                  CONSOLIDATED_FED_CREDIT_USED = AVAIL_CONSOLIDATED_CREDITS 
                  IF(CONSOLID_AVAILABLE_AMT_CREDITS >= TAX_OWED-R_AMT_TAX- &
                    SEC_29_CREDITS_USED-CONSOLIDATED_FED_CREDIT_USED) THEN
                     
                     CONSOLID_AMT_CREDITS_USED = TAX_OWED - R_AMT_TAX -  &
                        SEC_29_CREDITS_USED-CONSOLIDATED_FED_CREDIT_USED
                  ELSE
...
!
      ...
      ENTRY CLOSE_CONSOLIDATED_TAX_INFO()
         ...
         UNUSED_FED_NOLS(-1) = UNUSED_FED_NOLS(-1) + &
            FED_CONSOLIDATED_NOLS_GENERATED - FED_CONSOLIDATED_NOLS_USED
         STATE_NOLS_USED = 0.
         STATE_TAXABLE_INCOME = 0.
         ...
         FED_NOLS_USED = 0.
         FED_TAXABLE_INCOME = 0.
         UNUSED_CONSOLIDATED_FED_CREDITS = AVAIL_CONSOLIDATED_CREDITS - &
            CONSOLIDATED_FED_CREDIT_USED
         CONSOLID_AVAILABLE_AMT_CREDITS = CONSOLID_AVAILABLE_AMT_CREDITS - &
            CONSOLIDATED_FED_CREDIT_USED
!
      RETURN
      ...
      ENTRY CLOSE_CLASS_STATE_TAX_INFO(R_CLASS,R_YEAR,R_USE_ALL_CREDITS)
         ...
         IF(STATE_NOLS_USED > 0) THEN
            ...
            DO I = R_YEAR, R_YEAR + 14
               ...
               ELSE
                  STATE_BOOK_LOSSES(I,CLASS_POS) = - &
                    NOL_BALANCE + STATE_BOOK_LOSSES(I,CLASS_POS)
                  EXIT   
               ENDIF
            ENDDO
         ENDIF
         ...
         DO I = R_YEAR+1, R_YEAR+15 
            UNUSED_STATE_NOLS(CLASS_POS) = UNUSED_STATE_NOLS(CLASS_POS)+ &
                STATE_BOOK_LOSSES(I,CLASS_POS)   
         ENDDO
      RETURN
!
      ...
      ENTRY CLOSE_CLASS_FED_TAX_INFO(R_CLASS,R_YEAR,R_USE_ALL_CREDITS)
         ...
         IF(FED_NOLS_USED > 0) THEN
            ...
            DO I = R_YEAR, R_YEAR + 14
               ...
               ELSE
                  FEDERAL_BOOK_LOSSES(I,CLASS_POS) = -NOL_BALANCE + &
                    FEDERAL_BOOK_LOSSES(I,CLASS_POS)
                  EXIT   
               ENDIF
            ENDDO
         ENDIF
         ...
         DO I = R_YEAR+1, R_YEAR+15 
            UNUSED_FED_NOLS(CLASS_POS) = UNUSED_FED_NOLS(CLASS_POS) + &
                FEDERAL_BOOK_LOSSES(I,CLASS_POS)
         ENDDO
      RETURN

      ENTRY RETURN_NOLS_INFO(R_STATE_NOL_GENERATED,R_STATE_NOLS_USED, &
        R_FEDERAL_NOL_GENERATED,R_FED_NOLS_USED)
      ...
      ENTRY RETURN_CURRENT_NOLS_INFO(R_STATE_NOL_GENERATED,R_STATE_NOLS_USED, &
        R_FEDERAL_NOL_GENERATED,R_FED_NOLS_USED)
