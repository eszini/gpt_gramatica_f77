...
!     Last change: MSG 1/9/2012 2:44:35 PM
      ...
      SUBROUTINE TALLY_RESULTS_INFO(R_VARIABLE_NUMBER, &
        VARIABLES_IN,VARIABLES_OUT)

!
      INTEGER (KIND=2) :: R_VARIABLE_NUMBER
      REAL (KIND=4) :: VARIABLES_IN(0:R_VARIABLE_NUMBER)
      REAL (KIND=4) :: VARIABLES_OUT(0:R_VARIABLE_NUMBER)
      INTEGER (KIND=2) :: I
!
      ...
      ENTRY SUB_CONSOLIDATING_ADJUSTMENTS(R_VARIABLE_NUMBER,VARIABLES_IN, &
        VARIABLES_OUT)
      ...
      ENTRY PASS_PARENT_2_SBU_TOTALS(R_VARIABLE_NUMBER,VARIABLES_IN, &
        VARIABLES_OUT)
...
!
      ...
      INTEGER (KIND=2) :: MAX_CLASS_NUM,TEMP_LIST(MAX_CLASS_NUM), &
        MASTER_CLASS_LIST(0:MAX_CLASS_NUM)
!
...
!
         ...
         ! TODO: Replace with constants and get rid of comments.
         ! TRANSACT_MARKET_PURCHASE
         VARIABLE(470) = SBU_VARIABLES(470) + VARIABLE(470)  
         ! TRANSACT_MARKET_REVENUES
         VARIABLE(471) = SBU_VARIABLES(471) + VARIABLE(471)  
         ! OTHER_PURCHASE_POWER_EXPENSE
         VARIABLE(472) = SBU_VARIABLES(472) + VARIABLE(472)  
         ! COMPETITIVE_SALES_REVENUE
         VARIABLE(563) = SBU_VARIABLES(563) + VARIABLE(563)  
         ! UTILITY_SALES_REVENUE
         VARIABLE(564) = SBU_VARIABLES(564) + VARIABLE(564)  
         ! ICAP REVENUES
         VARIABLE(631) = SBU_VARIABLES(631) + VARIABLE(631)  
         ! NON_REG_REVENUES-10-15
         VARIABLE(671:676) = SBU_VARIABLES(671:676) + VARIABLE(671:676)  
         DO I = 155, 208
         ...
         ENDDO
         DO I = 561, 562
            VARIABLE(I) = SBU_VARIABLES(I) + VARIABLE(I) ! Ameren variables
         ENDDO
         DO I = 511, 560
...
!
!
! VARIABLES 626-COMMON SHARES ISSUED THIS PERIOD; AND 627-STD INTEREST RATES
! DO NOT GET SUMMED
!
...
!
      ...
      FUNCTION ASSET_CLASS_LINKAGES(ASSET_CLASS_loc,R_ASSET_CLASS_LIST, &
        ASSET_CLASS_VECTOR,R_ASSET_ALLOCATION_LIST)
...
!
      ...
      INTEGER (KIND=2) :: CLASS_POINTER,VECTOR_POINTER,ASSET_CLASS_loc,LINKED_CLASS, &
        ASSET_CLASS_VECTOR,I
      REAL (KIND=4) :: VECTOR_ASSET_CLASSES(AVAIL_DATA_YEARS), &
        VECTOR_CLASS_ALLOCATIONS(AVAIL_DATA_YEARS), &
             ASSET_CLASS_LIST(1024),ASSET_ALLOCATION_LIST(1024), &
                R_ASSET_CLASS_LIST(*),R_ASSET_ALLOCATION_LIST(*)
...
!
         ...
         IF(ASSET_CLASS_loc < 0) THEN
            CALL GET_ASSET_VAR(ABS(ASSET_CLASS_loc),DUMMY_TYPE,VECTOR_ASSET_CLASSES)
            CALL GET_ASSET_VAR(ABS(ASSET_CLASS_VECTOR),DUMMY_TYPE , &
                VECTOR_CLASS_ALLOCATIONS)
            VECTOR_POINTER = 1
            DO
               ASSET_CLASS_loc = VECTOR_ASSET_CLASSES(VECTOR_POINTER)
               ASSET_CLASS_LIST(CLASS_POINTER) = ASSET_CLASS_loc
         ...
         ELSE
            ...
            IF(ASSET_CLASS_VECTOR /= 0) THEN
                 ! POINTS TO A TIME VECTOR
               ASSET_ALLOCATOR = -ABS(ASSET_CLASS_VECTOR)
            ELSE   
               ASSET_ALLOCATOR = 100.
            ENDIF
            ASSET_ALLOCATION_LIST(CLASS_POINTER) = ASSET_ALLOCATOR
            DO
               CLASS_POINTER = CLASS_POINTER + 1
               CALL GET_DEBT_LINKED_CLASS(ASSET_CLASS_loc,LINKED_CLASS)
               IF(LINKED_CLASS < 0 .OR. CLASS_POINTER >= 1000) then
                EXIT 
               end if
               ASSET_CLASS_LIST(CLASS_POINTER) = LINKED_CLASS
               ASSET_ALLOCATION_LIST(CLASS_POINTER) = ASSET_ALLOCATOR
      ...
      ENTRY RETURN_ASSET_CLASS_LISTS(ASSET_CLASS_loc,R_ASSET_CLASS_LIST, &
        ASSET_CLASS_VECTOR,R_ASSET_ALLOCATION_LIST)
...
!
         ...
         CLASS_POINTER = 1
         IF(ASSET_CLASS_loc < 0) THEN
            CALL GET_ASSET_VAR(ABS(ASSET_CLASS_loc),DUMMY_TYPE, &
                VECTOR_ASSET_CLASSES)
            CALL GET_ASSET_VAR(ABS(ASSET_CLASS_VECTOR),DUMMY_TYPE, &
                VECTOR_CLASS_ALLOCATIONS)
...
!
      ...
      LOGICAL (KIND=1) :: RETAIN_POST_RETIREMENT_EARNINGS, &
...
!
      ...
      REAL (KIND=4) :: AI_LAGGED_GPV,AI_CLASS_CASH,AI_ANNUAL_CLASS_DEP, &
        AI_AFUDC_ON_CASH,AI_CLASS_EXPENSE
      REAL (KIND=4) :: OPENING_INVESTMENT_BALANCE
      REAL (KIND=4) :: FA_AFUDC_RATE,NF_AFUDC_RATE,AI_AFUDC_RATE, &
        AFUDC_BORROWED_RATE,AFUDC_ON_CASH
...
!
      ...
      REAL (KIND=4) :: SRP_FUNDS_FOR_DEBT_SERVICE,SRP_TOTAL_DEBT_SERVICE, &
        SRP_FUNDS_AFTER_DEBT_SERVICE,SRP_FUNDS_FOR_CORP_USE, &
             SRP_NON_DISPATCH_REV,SRP_DEBT_SERVICE_COVERAGE_RATIO,cawcd_energy_loc, &
                SRP_TOTAL_SALES,SRP_SALES_COST, &
             SRP_OPERATING_MARGIN,SRP_AVERAGE_SALES_COST,SRP_AVERAGE_MARGIN
      REAL (KIND=8) :: GET_TOTAL_SALES_ENERGY
!
      REAL (KIND=4) :: INTEREST_COVERAGE_WITHOUT_AFDC, &
        INTEREST_COVERAGE_WITH_AFDC,GOPINC,ITC_ITCAMT,TAXES_PAID,INTINC,OTHINC, &
             BTL_TAXES,RTEMP,LTD_STD_INTEREST,LTD_PS_RETIREMENT, &
                COVERAGE_DRIVER_RATIO,LEASE_PAYMENT_LIMIT,DEP_AMORTIZATION, &
             NOPINC,COVERAGE_RATIO_VALUE,FUNDS_FOR_CONSTRUCTION, &
                SUM_OPERATIONS,INTEREST
      REAL (KIND=4) :: NON_CASH_EXPENSE_ADJUSTMENTS,TAX_AND_OTHER_ADJUSTMENTS, &
        OTHER_CASH_SOURCES,CAPITAL_REQUIREMENTS, &
                 TOTAL_CAPITAL_SERVICE_PAYMENTS
! 
      REAL (KIND=4) :: TEMP_SRP_TOTAL_SALES,SRP_OP_FIN_TARGET_D, &
        SRP_OP_FIN_MARGIN_D,SRP_OP_FIN_TARGET_D_MWH, &
      ...
      REAL (KIND=4) :: GROSS_CASH_2_DEBT,GROSS_CASH_FLOW,TOTAL_DEBT, &
        THE_PERCENT_OF_A_TO_B
      REAL (KIND=4) :: CAPITAL_MULTIPILER,TOTAL_CAPITIAL_BAL, &
        SHORT_TERM_DEBT_BAL,PREFERRED_STOCK_BAL,LONG_TERM_DEBT_BAL, &
             COMMON_EQUITY_BAL,FUNDS_FROM_OPERATIONS, &
...
! 
         ...
         REPORTING_VARIABLES(0:NUM_ACTIVE_VARIABLES) =  &
            R_VARIABLE_VALUE(0:NUM_ACTIVE_VARIABLES)
         CALL RETURN_CLASS_EXTERNALITY_VALUES(PASS_THROUGH_VALUES)
         CALL RETURN_AI_CLASS_VALUES(AI_LAGGED_GPV,AI_CLASS_CASH, &
            AI_ANNUAL_CLASS_DEP,AI_AFUDC_ON_CASH,AI_CLASS_EXPENSE)
!
...
!
      ...
      ENTRY RETURN_ASSET_CLASS_VALUES(R_VARIABLE_VALUE,COVERAGE_RATIO_TYPE, &
        OPERATING_METHOD,LEASE_PAYMENTS_IN_TIER,SRP_RATIO)
...
!
         ...
         DO I = 1, 7
            ...
            R_VARIABLE_VALUE(463+I) = PASS_THROUGH_VALUES(3+I) 
         ENDDO
!
! AI CLASS OUTPUT
!
         ...
         IF(REPORTING_VARIABLES(47) /= 0.) THEN ! COMMON_EARNINGS
                                        ! AFDC1_TO_COMMON_EARNINGS
            R_VARIABLE_VALUE(87) = 100.*AFUDC_ON_CASH/REPORTING_VARIABLES(47) 
         ELSE
...
!
         ...
         IF(COVERAGE_RATIO_TYPE == 1) THEN
            ...
            IF(LTD_STD_INTEREST /= 0.) COVERAGE_DRIVER_RATIO = &
                GOPINC/LTD_STD_INTEREST
         ELSEIF(COVERAGE_RATIO_TYPE == 11) THEN
            ...
            IF(LTD_INTEREST_LESS_AFUDC /= 0.) COVERAGE_DRIVER_RATIO = &
                (LTD_INTEREST_LESS_AFUDC + REPORTING_VARIABLES(45))/ &
                                           LTD_INTEREST_LESS_AFUDC
         ELSEIF(COVERAGE_RATIO_TYPE == 2 .OR. COVERAGE_RATIO_TYPE == 3) THEN
            RTEMP = GOPINC+ITC_ITCAMT+TAXES_PAID+INTINC+OTHINC-BTL_TAXES - &
                REPORTING_VARIABLES(35)   ! CLASS_BTL_EXPENSES

            IF(COVERAGE_RATIO_TYPE == 2) LTD_STD_INTEREST = LTD_STD_INTEREST - &
                AFUDC_ON_CASH
            IF(LTD_STD_INTEREST /= 0.) COVERAGE_DRIVER_RATIO = &
                RTEMP/LTD_STD_INTEREST
         ELSEIF(COVERAGE_RATIO_TYPE == 8 .OR. COVERAGE_RATIO_TYPE == 9) THEN
            LEASE_PAYMENT_LIMIT = MAX(0.,LEASE_PAYMENTS_IN_TIER - .02* &
                REPORTING_VARIABLES(79))/3. ! COMEQU(2)
            RTEMP = GOPINC+ITC_ITCAMT+TAXES_PAID+INTINC+OTHINC-BTL_TAXES + &
                LEASE_PAYMENT_LIMIT
            ...
            IF(INTEREST+LTD_PS_RETIREMENT /= 0.) THEN
               INTEREST_COVERAGE_WITH_AFDC = (RTEMP + DEP_AMORTIZATION)/ &
               (INTEREST+LTD_PS_RETIREMENT)
               INTEREST_COVERAGE_WITHOUT_AFDC = (RTEMP + DEP_AMORTIZATION - &
                AFUDC_ON_CASH)/(INTEREST+LTD_PS_RETIREMENT)
            ENDIF
         ...
         ELSEIF(COVERAGE_RATIO_TYPE == 6 .OR. COVERAGE_RATIO_TYPE == 7) THEN
               SRP_TOTAL_DEBT_SERVICE = REPORTING_VARIABLES(39) + & 
                                        REPORTING_VARIABLES(119) + & 
                                        REPORTING_VARIABLES(154)   
! NOTE OTNINC IS NET OF BLT_EXPENSES

               SRP_FUNDS_FOR_DEBT_SERVICE = REPORTING_VARIABLES(31) + & ! GOPINC
                                            REPORTING_VARIABLES(34) - & ! INTINC - 
                                            REPORTING_VARIABLES(40) + & 
                                            REPORTING_VARIABLES(33) + & ! OTHINC +
                                            DEP_AMORTIZATION + &        
                                            REPORTING_VARIABLES(217)   ! TXPROP +
               IF(SRP_TOTAL_DEBT_SERVICE /= 0.) COVERAGE_DRIVER_RATIO = &
                SRP_FUNDS_FOR_DEBT_SERVICE/SRP_TOTAL_DEBT_SERVICE
...
!
!
         SUM_OPERATIONS = NOPINC + DEP_AMORTIZATION + ITC_ITCAMT - &
            AFUDC_ON_CASH + REPORTING_VARIABLES(29)  ! TXDEFP
         ...
         IF(FUNDS_FOR_CONSTRUCTION /= 0.) THEN
            FUNDS_FOR_CONSTRUCTION = 100.*(SUM_OPERATIONS - &
                REPORTING_VARIABLES(301))/FUNDS_FOR_CONSTRUCTION
         ELSE
            FUNDS_FOR_CONSTRUCTION = 0.
         ENDIF
         COVERAGE_RATIO_VALUE = GOPINC + ITC_ITCAMT + OTHINC + INTINC + &
            DEP_AMORTIZATION + REPORTING_VARIABLES(29) + &  ! TXDEFP
            REPORTING_VARIABLES(28)      ! TXNORM
         INTEREST = LTD_STD_INTEREST + LTD_PS_RETIREMENT
         IF(INTEREST /= 0.) THEN
...
!                                          Nuclear Burn Expense
         NON_CASH_EXPENSE_ADJUSTMENTS = REPORTING_VARIABLES(231) + & 
...
! KEPCO STUFF 5/29/97
!
         R_VARIABLE_VALUE(362) = REPORTING_VARIABLES(312)

         R_VARIABLE_VALUE(363) = REPORTING_VARIABLES(313) 
         R_VARIABLE_VALUE(364) = REPORTING_VARIABLES(314) 
      RETURN

!
! CALCULATED SRP OUTPUT VARIALBES
!
      ENTRY SRP_REPORTING_VARIABLES(R_RUN_YEAR,R_VARIABLE_VALUE, &
        R_CLASS_MHW_SALES,R_ANNUAL_ECONOMY_SOLD)
...
!
         ...
         R_VARIABLE_VALUE(480) = TOTAL_CAPITAL*(REPORTING_VARIABLES(81)+ & 
                                                REPORTING_VARIABLES(84)) 
         CALL RETURN_CAWCD_ENERGY(cawcd_energy_loc)
         SRP_TOTAL_SALES = (R_CLASS_MHW_SALES+GET_TOTAL_SALES_ENERGY()+ &
            R_ANNUAL_ECONOMY_SOLD + cawcd_energy_loc)/1000.
         ...
         SRP_OPERATING_MARGIN = REPORTING_VARIABLES(4)  - & ! OPREV(2)
                                ...
                                REPORTING_VARIABLES(35)   ! BTL_EXPENSES  
         R_VARIABLE_VALUE(556) = cawcd_energy_loc/1000.
         R_VARIABLE_VALUE(557) = SRP_OPERATING_MARGIN
         R_VARIABLE_VALUE(559) = SRP_SALES_COST
         SRP_NET_FINANCING_COSTS = R_VARIABLE_VALUE(55) + &
            R_VARIABLE_VALUE(56) - R_VARIABLE_VALUE(57)
         SRP_OP_FIN_TARGET_D = SRP_SALES_COST + SRP_NET_FINANCING_COSTS - &
            R_VARIABLE_VALUE(107)
...
!                             ATL Federal Income Taxes Paid
         TOTAL_INCOME_TAXES = REPORTING_VARIABLES(26) + & 
...
!
         IF(INTEREST /= 0.) THEN
            TIMES_INT_B4_INC_TAXES_W_AFDC = &
                (NOPINC+TOTAL_INCOME_TAXES)/INTEREST
            TIMES_INT_B4_INC_TAXES_WO_AFDC = &
                (NOPINC-AFDC1+TOTAL_INCOME_TAXES)/INTEREST
         ELSE   
...
!
         ! Preferred Dividends plus interest
         INTEREST_PS_DIV = INTEREST + REPORTING_VARIABLES(46) 
...
!
         ARTICLES_OF_INCORPORATION= ANNUALIZED_LTD_INTEREST + &
            ANNUALIZED_PS_DIVIDENDS + ANNUALIZED_STD_INTEREST
...
! EFFECTIVE TAX RATE
!
         IF(REPORTING_VARIABLES(45)+ &
            TOTAL_INCOME_TAXES-REPORTING_VARIABLES(37) /= 0.) THEN
            EFFECTIVE_TAX_RATE = 100.*TOTAL_INCOME_TAXES/ &
!                                    45 is net income
                (REPORTING_VARIABLES(45)+TOTAL_INCOME_TAXES- &
                REPORTING_VARIABLES(37)) 
         ELSE
...
!
         R_VARIABLE_VALUE(29) = R_VARIABLE_VALUE(681) + &
            R_VARIABLE_VALUE(682) + R_VARIABLE_VALUE(733)
         TOTAL_INCOME_TAXES = &
            R_VARIABLE_VALUE(26) + & ! ATL Federal Income Taxes Paid
...
!
         ...
         TOTAL_CAPITIAL_BAL = COMMON_EQUITY_BAL + SHORT_TERM_DEBT_BAL + &
            PREFERRED_STOCK_BAL + LONG_TERM_DEBT_BAL &
         ...
         R_VARIABLE_VALUE(366) = CAPITAL_MULTIPILER * (COMMON_EQUITY_BAL + &
            R_VARIABLE_VALUE(730))                         
         R_VARIABLE_VALUE(367) = SHORT_TERM_DEBT_BAL*CAPITAL_MULTIPILER
!
         FUNDS_FROM_OPERATIONS = R_VARIABLE_VALUE(632) - R_VARIABLE_VALUE(387) &
            - R_VARIABLE_VALUE(405)
         CONSTRUCTION_AND_NET_INVESTMENT = R_VARIABLE_VALUE(110) + &
            R_VARIABLE_VALUE(111)
         IF(CONSTRUCTION_AND_NET_INVESTMENT /= 0.) THEN
            R_VARIABLE_VALUE(368) = FUNDS_FROM_OPERATIONS/ &
            CONSTRUCTION_AND_NET_INVESTMENT
         ELSE
...
!
!                                 Preferred Dividends plus interest
         INTEREST_PS_DIV = INTEREST + R_VARIABLE_VALUE(46) 
...
!
         IF(R_VARIABLE_VALUE(45)+TOTAL_INCOME_TAXES-AFDC1 /= 0.) THEN
            EFFECTIVE_TAX_RATE = 100.*TOTAL_INCOME_TAXES/(R_VARIABLE_VALUE(45)+ &
                TOTAL_INCOME_TAXES-AFDC1) ! 45 is net income
         ELSE
            EFFECTIVE_TAX_RATE = NOT_AVAIL   
         ENDIF
!
...
!
         GROSS_CASH_FLOW = R_VARIABLE_VALUE(632) 

         TOTAL_DEBT =  R_VARIABLE_VALUE(634) 
...
!
         ...
         R_VARIABLE_VALUE(574) = ANNUALIZED_LTD_INTEREST + &
            ANNUALIZED_PS_DIVIDENDS + ANNUALIZED_STD_INTEREST
         
         IF(ABS(R_VARIABLE_VALUE(47)) >= 0.0001) THEN ! COMMON_EARNINGS
!                                         AFDC1_TO_COMMON_EARNINGS
            R_VARIABLE_VALUE(575) = 100.*AFDC1/R_VARIABLE_VALUE(47) 
         ELSE
         ...
         IF(R_VARIABLE_VALUE(365) == NOT_AVAIL .OR. &
            R_VARIABLE_VALUE(367) == NOT_AVAIL) THEN
            R_VARIABLE_VALUE(577) = NOT_AVAIL
         ELSE
            R_VARIABLE_VALUE(577) = R_VARIABLE_VALUE(365) + &
                R_VARIABLE_VALUE(367)
         ENDIF
         RETURN

         ENTRY COVERAGE_RATIO_RPT(R_VARIABLE_VALUE, &
            COVERAGE_RATIO_TYPE,OPERATING_METHOD,SRP_RATIO)
...
!                              Preferred Stock Retirements
         LTD_PS_RETIREMENT = R_VARIABLE_VALUE(154) + & 
                             R_VARIABLE_VALUE(119)     ! Debt Retirements
!
...
!                                 LTD BOOKED INTEREST WITH AMORT
            LTD_INTEREST_LESS_AFUDC = R_VARIABLE_VALUE(293) &  
                        - R_VARIABLE_VALUE(41)   ! afudc borrowed
            IF(LTD_INTEREST_LESS_AFUDC /= 0.) COVERAGE_DRIVER_RATIO = &
                (LTD_INTEREST_LESS_AFUDC + R_VARIABLE_VALUE(45))/ &
                                           LTD_INTEREST_LESS_AFUDC
         ELSEIF(COVERAGE_RATIO_TYPE == 2 .OR. COVERAGE_RATIO_TYPE == 3) THEN
            RTEMP = GOPINC+ITC_ITCAMT+TAXES_PAID+INTINC+ &
                OTHINC-BTL_TAXES - R_VARIABLE_VALUE(35)   ! CLASS_BTL_EXPENSES
            ...
            IF(LTD_STD_INTEREST /= 0.) COVERAGE_DRIVER_RATIO = &
                RTEMP/LTD_STD_INTEREST
         ELSEIF(COVERAGE_RATIO_TYPE == 8 .OR. COVERAGE_RATIO_TYPE == 9) THEN
            LEASE_PAYMENT_LIMIT = MAX(0.,R_VARIABLE_VALUE(778) - .02* &
                R_VARIABLE_VALUE(79))/3. ! COMEQU(2)
            RTEMP = GOPINC+ITC_ITCAMT+TAXES_PAID+INTINC+OTHINC-BTL_TAXES + &
                LEASE_PAYMENT_LIMIT
            INTEREST = LTD_STD_INTEREST - AFUDC_ON_CASH + LEASE_PAYMENT_LIMIT
            IF(COVERAGE_RATIO_TYPE == 9) INTEREST=INTEREST+AFUDC_ON_CASH
         ...
         ELSEIF(COVERAGE_RATIO_TYPE == 6 .OR. COVERAGE_RATIO_TYPE == 7) THEN
!                                           LTDINT(RUN_YEAR)
               SRP_TOTAL_DEBT_SERVICE = R_VARIABLE_VALUE(39) + &  
!                                       LTDRET(RUN_YEAR)
                                        R_VARIABLE_VALUE(119) + & 
...
! NOTE OTNINC IS NET OF BLT_EXPENSES
      ...
      SUBROUTINE LAG_CASH_USING_PATTERN_ALL_YRS(R_YRS,R_BOOK_VALUES, &
        R_CASH_LAG_PATTERN,R_CASH_PAYMENTS,R_CASH_VALUES)

!
      INTEGER (KIND=2) :: R_YRS
      REAL (KIND=4) :: R_BOOK_VALUES(0:12,0:R_YRS),R_CASH_LAG_PATTERN(12), &
        R_CASH_PAYMENTS(12),R_CASH_VALUES(0:12,0:R_YRS)
...
! MOVE CASH ACCURRED LAST YEAR TO THIS YEAR
            ...
            DO MO = 1, 12
               DO LAG_MO = 1, 13-MO
                  R_CASH_VALUES(MO+LAG_MO-1,YR) = &
                    R_CASH_VALUES(MO+LAG_MO-1,YR) + &
                    R_CASH_LAG_PATTERN(LAG_MO) * R_BOOK_VALUES(MO,YR)
               
               ENDDO
               DO LAG_MO = 14-MO, 12
                  R_CASH_PAYMENTS(LAG_MO+MO-13) = R_CASH_LAG_PATTERN(LAG_MO) * &
                    R_BOOK_VALUES(MO,YR)
               ENDDO
...
!
         ...
         ! TIE & Preferred Dividends Earned after Taxes with AFUDC,567,
         R_VARIABLES(567) = NOT_AVAIL  
         ! TIE & Preferred Dividends Earned after Taxes without AFUDC,568,
         R_VARIABLES(568) = NOT_AVAIL  
...
!
      ...
      REAL (KIND=4) :: R_VARIABLE(0:*),THE_RATIO_OF_A_TO_B,TARGET_DSC, &
        R_TARGET_DSC,PrinIntrPayments,MarginIntExpDep,TwoPercentTest
!
         TARGET_DSC = 1
         R_VARIABLE(761) = & ! GRE_TOTAL_INTEREST_EXPENSE), ! 761
                           R_VARIABLE(760) &  ! GRE_FALKIRK_INTEREST
         ...
         R_VARIABLE(763) = & ! GRE_TOTAL_DEPRECIATION), ! 763
                           ...
                           + R_VARIABLE(762)   ! GRE_FALKIRK_DEPRECIATION
         R_VARIABLE(766) = & ! GRE_TOTAL_PRINCIPAL_PAYMENTS), ! 766
                           ...
                           + R_VARIABLE(764) & ! GRE_OTHER_PRINCIPAL_ADJS
                           + R_VARIABLE(765)   ! GRE_PRINCIPAL_MANUAL_ADJS
         R_VARIABLE(768) = & ! GRE_TOTAL_INTEREST_PAYMENTS), ! 768
                           R_VARIABLE(39) & ! INTEREST EXPENSE
                           + R_VARIABLE(767)  ! GRE_INTEREST_PAYMENTS_MANUAL_ADJS
         ...
         R_VARIABLE(771) = & ! GRE_OTHER_INCOME), ! 771
                           R_VARIABLE(41) + R_VARIABLE(37) + &
                            R_VARIABLE(33) + R_VARIABLE(34)
         ...
         R_VARIABLE(769) = & ! GRE_NET_MARGIN), ! 769
                           ...
                           + R_VARIABLE(772)
         PrinIntrPayments = R_VARIABLE(766) & ! GRE_TOTAL_PRINCIPAL_PAYMENTS
                            + R_VARIABLE(768) ! GRE_TOTAL_INTEREST_PAYMENTS
         MarginIntExpDep = R_VARIABLE(769) &   ! GRE_NET_MARGIN), ! 769
                           + R_VARIABLE(761) & ! GRE_TOTAL_INTEREST_EXPENSE
                           + R_VARIABLE(763)   ! GRE_TOTAL_DEPRECIATION
         R_VARIABLE(773) = THE_RATIO_OF_A_TO_B(MarginIntExpDep,PrinIntrPayments)
      ...
      REAL (KIND=4) :: NOT_AVAIL
      PARAMETER(NOT_AVAIL=-999999.)
      REAL (KIND=4) :: TOTAL_OPERATING_EXPENSES,BTL_TOTALS, &
        TOTAL_TAXES_NON_INCOME,TOTAL_DEP_AMORT,EBIT,TOTAL_INTEREST, &
                       TOTAL_INCOME_TAXES,EBITDA,INCOME_AFTER_INTEREST, &
                       TOTAL_NON_REGULATED_REVENUES,TIER_INTEREST
      REAL (KIND=4) :: EBITDA_2_DEBT,THE_PERCENT_OF_A_TO_B,TOTAL_DEBT
!
         TOTAL_NON_REGULATED_REVENUES = R_VARIABLES(1) &    
...
!
         TOTAL_OPERATING_EXPENSES = R_VARIABLES(13) +    R_VARIABLES(5) + &
            R_VARIABLES(438) +   R_VARIABLES(233) &   
            !Salaries & Wages,13 Up| - Up| Fossil Fuel Expense,5 - Up| 
            ! ...                             Nuclear Fuel -   Up| Decommissioning
            + R_VARIABLES(232)   + R_VARIABLES(461)    + &
                R_VARIABLES(393)   +       R_VARIABLES(8) &
            ! DOE Deposal Up| - !Purchased Power Expense,461 Up|
            !            - Up|Deferred Fuel Expense,393 - Up|Fxd O&M Prod Expense,8
            + R_VARIABLES(363)   + R_VARIABLES(450)    + &
                R_VARIABLES(421)   + R_VARIABLES(422) &

            + R_VARIABLES(423)   + R_VARIABLES(9) + &
                R_VARIABLES(440)   + R_VARIABLES(11) &
            ...
            + R_VARIABLES(321)   + R_VARIABLES(322)    + R_VARIABLES(323) + &
                R_VARIABLES(324) &

            + R_VARIABLES(325)   + R_VARIABLES(354)    + &
                R_VARIABLES(13)  +    R_VARIABLES(14)  &
            !A&G Maintenance,325 |Up - Lease Expenses,354 |Up - !
            ! Up| DSM Program Expense - Up|DSM Program Rebate
...
!
         ...
         R_VARIABLES(495) = BTL_TOTALS &
           - R_VARIABLES(43) & ! BTL Totals w/out Subsidiary Income
...
!
         ...
         R_VARIABLES(781) = 100.*THE_RATIO_OF_A_TO_B(R_VARIABLES(780), &
            TotalDebt) ! FFO/Total Debt
!
...
!
 ...
 !                                as input in Financial Parameters file
...
!
         R_VARIABLES(783) = R_VARIABLES(482) & !  Interest Expense
          ...
          + R_VARIABLES(778)   !  +Lease Payment in Tier Calculation
!                                              Pre-Tax Interest Coverage
         R_VARIABLES(784) = THE_RATIO_OF_A_TO_B(R_VARIABLES(782), & 
                                                R_VARIABLES(783))

! FFO Interest Coverage
         R_VARIABLES(785) = THE_RATIO_OF_A_TO_B(R_VARIABLES(780), & 
                                                R_VARIABLES(783))
      RETURN
      END

      SUBROUTINE FIRST_ENERGY_PandL_REPORT(CLASS,R_VARIABLES, &
        CL_ANN_CLASS_ENERGY,EXP_FUEL_EXPENSE,EXP_PURCHASE_POWER_EXPENSE, &
      ...
      REAL :: TOTAL_FOSSIL_COST,TOTAL_GENERATION,TOTAL_SALES, &
        EXP_FUEL_EXPENSE,EXP_PURCHASE_POWER_EXPENSE, &
              EXP_FILE_VARIABLE_EXPENSE,EXP_FILE_FIXED_EXPENSE, &
                SUM_NON_OPERATING_EXPENSES
!
      LOGICAL (KIND=1) :: RETURN_FE_PNL_EXPENSES,VOID_LOGICAL
      INTEGER (KIND=2) :: FT,CLASS
      REAL :: FUEL_COST(6),PURCHASE_POWER_EXPENSE(11),VARIABLE_EXPENSE(6), &
        FIXED_EXPENSE(6),TOTAL_SALES_REVENUE(6), &
              CL_ANN_TOTAL_CLASS_ENERGY(11),CL_ANN_TOTAL_CAPACITY(11), &
                THE_RATIO_OF_A_TO_B,THE_PERCENT_OF_A_TO_B
      integer (kind=2) :: EastLake
      INTEGER (KIND=2), PARAMETER :: Coal=1,Gas=2,Oil=3,Nuclear=4,Water=5, &
        Other=6, &
        MarketSales=10,MarketPurchases=11, BayShore=3

! NOTE: 
!       1 == PRODUCTION FROM RESOURCE TO MEET NATIVE LOAD
...
!       4 == ECONOMY SALES WHICH INCREASE LOAD
      ...
      REAL :: UTILITY_SALES,UTILITY_SALES_QUANT,UTILITY_SALES_LOSSES, &
        COMPETITIVE_SALES,COMPETITIVE_SALES_QUANT, &
              COMPETITIVE_SALES_LOSSES,COMPETITIVE_SALES_CATEGY(1:9), &
                COMPETITIVE_SALES_QUANT_CATEGY(1:9), &
              COMPETITIVE_SALES_LOSSES_CATEGY(1:9),BAY_SHORE_FUEL_EXPENSE, &
                ENRGY_BOUGHT,ENRGY_BOUGHT_COST
...
!
         ...
         IF(CLASS >= 0) THEN
            VOID_LOGICAL = RETURN_FE_PNL_EXPENSES(CLASS,FUEL_COST, &
                PURCHASE_POWER_EXPENSE,VARIABLE_EXPENSE,FIXED_EXPENSE, &
                TOTAL_SALES_REVENUE,CL_ANN_TOTAL_CLASS_ENERGY, &
                CL_ANN_TOTAL_CAPACITY, BAY_SHORE_FUEL_EXPENSE)
            CALL FE_ENRG_LIMITED_INFO(CLASS,PURCHASE_POWER_EXPENSE(Water), &
                VARIABLE_EXPENSE(Water),FIXED_EXPENSE(Water), &
                TOTAL_SALES_REVENUE(Water),R_EL_MARKET_REVENUES, &
             ...
             IF(USE_BUDGET_VARIABLE_OM) THEN
               ...
               ENDIF
            ENDIF
            VOID_INT2 = RETURN_FE_PNL_REVENUES(CLASS,UTILITY_SALES, &
                UTILITY_SALES_QUANT,UTILITY_SALES_LOSSES, &
                COMPETITIVE_SALES_CATEGY,COMPETITIVE_SALES_QUANT_CATEGY, &
                COMPETITIVE_SALES_LOSSES_CATEGY)
            CALL RETURN_FE_COMPETITIVE_SALE_INFO(CLASS,COMPETITIVE_SALES_CATEGY, &
            COMPETITIVE_SALES_QUANT_CATEGY, &
                                       COMPETITIVE_SALES_LOSSES_CATEGY)
            CLASS_NUM = CLASS-INT(1,2)
            CALL GET_ANN_COST_ABOVE_RESOURCES(ENRGY_BOUGHT,ENRGY_BOUGHT_COST, &
                Annual_Amount_LOC,CLASS_NUM)
!
...
!
            ...
            DO I = 1, 9
               COMPETITIVE_SALES = COMPETITIVE_SALES + &
				COMPETITIVE_SALES_CATEGY(I)
               R_VARIABLES(595+I) = R_VARIABLES(595+I) + &
				COMPETITIVE_SALES_CATEGY(I)
               COMPETITIVE_SALES_QUANT = COMPETITIVE_SALES_QUANT + &
                COMPETITIVE_SALES_QUANT_CATEGY(I)
               R_VARIABLES(582+I) = R_VARIABLES(582+I) + &
                COMPETITIVE_SALES_QUANT_CATEGY(I)
               COMPETITIVE_SALES_LOSSES = COMPETITIVE_SALES_LOSSES + &
                COMPETITIVE_SALES_LOSSES_CATEGY(I)
            ENDDO

            R_VARIABLES(539) = COMPETITIVE_SALES_QUANT+R_VARIABLES(539) 
            R_VARIABLES(594) = COMPETITIVE_SALES_LOSSES+R_VARIABLES(594) 
            R_VARIABLES(540) = R_VARIABLES(540) + &
                CL_ANN_TOTAL_CLASS_ENERGY(MarketSales)
!           PURCHASE POWER QUANTIITIES
            R_VARIABLES(621) = R_VARIABLES(621) & 
                               + ENRGY_BOUGHT
            R_VARIABLES(614) = R_VARIABLES(614) & 
                               + CL_ANN_CLASS_ENERGY(2)

            R_VARIABLES(615) = R_VARIABLES(615) & 
                               + CL_ANN_TOTAL_CLASS_ENERGY(MarketPurchases)
!            IF(VOID_LOGICAL) THEN
! FUEL CALCULATIONS
!  Coal
               R_VARIABLES(511) = R_VARIABLES(511) + FUEL_COST(Coal) - &
...
!  Other
               R_VARIABLES(515) = R_VARIABLES(515) + FUEL_COST(Other) + &
                FUEL_COST(Oil)
! PURCHASE INFORMATION
! VAR and FIXED O&M CALCULATIONS
!  Coal
               R_VARIABLES(517) = R_VARIABLES(517) + VARIABLE_EXPENSE(Coal) + &
                VARIABLE_EXPENSE(BayShore) + VARIABLE_EXPENSE(Other)
               R_VARIABLES(520) = R_VARIABLES(520) + FIXED_EXPENSE(Coal) + &
                FIXED_EXPENSE(BayShore) + FIXED_EXPENSE(Other)
!  Gas
               R_VARIABLES(518) = R_VARIABLES(518) + VARIABLE_EXPENSE(Gas) + &
                VARIABLE_EXPENSE(Water)
               R_VARIABLES(521) = R_VARIABLES(521) + FIXED_EXPENSE(Gas) + &
                FIXED_EXPENSE(Water)
! Fossil Non-fuel expense
               R_VARIABLES(652) = R_VARIABLES(652) & 
...
! PURCHASE POWER EXPENSE
               R_VARIABLES(616) = R_VARIABLES(616) & ! FIXED PURCHASED POWER EXPENSE
                                 + PURCHASE_POWER_EXPENSE(Coal) + &
                PURCHASE_POWER_EXPENSE(Gas) + PURCHASE_POWER_EXPENSE(BayShore) &
                                 + PURCHASE_POWER_EXPENSE(Nuclear) + &
                                    PURCHASE_POWER_EXPENSE(Water) + &
                                        PURCHASE_POWER_EXPENSE(Other)
                !           OPPORTUNITY PURCHASED POWER EXPENSE
               R_VARIABLES(617) = R_VARIABLES(617) & 
                                 + PURCHASE_POWER_EXPENSE(MarketPurchases)
!                         OUT OF RESOURCES PURCHASED POWER EXPENSE
               R_VARIABLES(622) = R_VARIABLES(622) & 
                                 + ENRGY_BOUGHT_COST
! ENERGY INFORMATION
!  Coal
               R_VARIABLES(532) = R_VARIABLES(532) + &
                CL_ANN_TOTAL_CLASS_ENERGY(Coal)
!  Gas
               R_VARIABLES(533) = R_VARIABLES(533) + &
				CL_ANN_TOTAL_CLASS_ENERGY(Gas)
!  All Other 
               R_VARIABLES(534) = R_VARIABLES(534) + &
                CL_ANN_TOTAL_CLASS_ENERGY(Oil) + &
                    CL_ANN_TOTAL_CLASS_ENERGY(Water) &
                                  + CL_ANN_TOTAL_CLASS_ENERGY(Other)
!  Nuclear
               R_VARIABLES(531) = R_VARIABLES(531) + &
                CL_ANN_TOTAL_CLASS_ENERGY(Nuclear)
!            ENDIF
         ENDIF ! if class
!
!
...
!
         ...
         DO I = 517, 522 
            TOTAL_PRODUCTION_COSTS_NON_FUEL = TOTAL_PRODUCTION_COSTS_NON_FUEL + &
                R_VARIABLES(I)
         ENDDO
...
!                                    ROLLED-UP NUCLEAR FUEL COSTS
         TOTAL_ENERGY_GENERATION_COSTS = R_VARIABLES(438) & 
...
!
         ...
         GROSS_MARGIN_PERCENT = THE_PERCENT_OF_A_TO_B(GROSS_MARGIN_DOLLARS, &
            TOTAL_POWER_REVENUES)
...
!
         DO I = 0, 4  ! VARIABLE 543 TO 547 
            R_VARIABLES(543+I) = THE_RATIO_OF_A_TO_B(R_VARIABLES(526+I), &
                R_VARIABLES(538+I))
         ENDDO
...
! PURCHASE POWER COST-AVERAGEs
         ...
         DO I = 1, 9
            R_VARIABLES(604+I) = THE_RATIO_OF_A_TO_B(R_VARIABLES(595+I), &
                R_VARIABLES(582+I))
...
! TOTAL MWH SALES, LOSSES AND TOTAL OF BOTH
         R_VARIABLES(592) = R_VARIABLES(538) + R_VARIABLES(539) + &
            R_VARIABLES(540) + (R_VARIABLES(646) + R_VARIABLES(648))*.001 
! 02/04/04.
...
!
         ...
         R_VARIABLES(625) = THE_RATIO_OF_A_TO_B( &
            TOTAL_ENERGY_GENERATION_COSTS,R_VARIABLES(536))
...
!
      ...
      LOGICAL (KIND=1) :: TRANSFERS_ARE_ACTIVE,R_REV_TRANSFERS_ARE_ACTIVE, &
        R_EXP_TRANSFERS_ARE_ACTIVE,ASSET_ANALYST_ONLY
      INTEGER (KIND=2) :: TRANSFR_COST_CLASS_VECTOR,TRANSFR_COST_ALLOC_VECTOR, &
        TRANSFR_REV_CLASS_VECTOR,TRANSFR_REV_ALLOC_VECTOR
      REAL (KIND=4) :: TOTAL_AMEREN_MARKET_REVENUES, & ! USED BY AMEREN 
                       TOTAL_AMEREN_FUEL_EXPENSE,TOTAL_AMEREN_VARIABLE_EXPENSE
      REAL (KIND=4) :: TRANSFR_MARKET_COSTS(0:12), &
        TRANSFR_MARKET_COSTS_MWH(0:12),TRANSFR_MARKET_REVS(0:12), &
...
!
      INTEGER (KIND=2) :: YR,MO,ASSET_CLASS_loc,RESULT_PTR
      INTEGER (KIND=2) :: R_YEAR
      REAL (KIND=4) :: ASSET_ALLOCATOR,ALLOCATED_DOLLAR_AMOUNT(0:12), &
        ALLOCATED_QUANT_AMOUNT(0:12), &
                       ALLOCATED_FUEL_DOLLAR_AMOUNT(0:12), &
                        ALLOCATED_VOM_DOLLAR_AMOUNT(0:12)
      INTEGER (KIND=2) :: NUMBER_OF_COST_CLASSES,MAX_COST_CLASS_NUMBER
      SAVE MAX_COST_CLASS_NUMBER
      REAL TRANSFR_COST_QUANTITIES,TRANSFR_FUEL_COST_AMOUNT, &
        TRANSFR_VAR_COST_AMOUNT
      INTEGER (KIND=2) :: TRANSFR_COST_CLASS_POINTR
      ALLOCATABLE :: TRANSFR_COST_QUANTITIES(:,:),TRANSFR_COST_CLASS_POINTR(:), &
        TRANSFR_FUEL_COST_AMOUNT(:,:), &
                     TRANSFR_VAR_COST_AMOUNT(:,:)
      SAVE TRANSFR_COST_QUANTITIES,TRANSFR_COST_CLASS_POINTR, &
        TRANSFR_FUEL_COST_AMOUNT,TRANSFR_VAR_COST_AMOUNT
      ...
      ALLOCATABLE :: TRANSFR_REV_AMOUNT(:,:),TRANSFR_REV_QUANTITIES(:,:), &
        TRANSFR_REV_CLASS_POINTR(:)
      SAVE TRANSFR_REV_AMOUNT,TRANSFR_REV_QUANTITIES, &
        TRANSFR_REV_CLASS_POINTR
!
      INTEGER (KIND=2) :: ALLOCATION_VECTOR,CLASS_POINTER
      REAL (KIND=4) :: ALLOCATION_VALUE(0:12), &
...
!
      REAL :: R_CLASS_REVENUES,R_CLASS_REV_QUANITIES,R_CLASS_FUEL_EXPENSES, &
        R_CLASS_VOM_EXPENSES,R_CLASS_EXP_QUANITIES
...
!
         VOID_LOGICAL = RETURN_ASSET_CLASS_LISTS( &
           TRANSFR_COST_CLASS_VECTOR,ASSET_CLASS_LIST,TRANSFR_COST_ALLOC_VECTOR, &
                           ASSET_ALLOCATION_LIST)
         MAX_COST_CLASS_NUMBER = 0
         CLASS_POINTER = 1
         DO 
            MAX_COST_CLASS_NUMBER = MAX(MAX_COST_CLASS_NUMBER, &
                int(ASSET_CLASS_LIST(CLASS_POINTER),2)+INT(1,2))
         ...
         IF(ALLOCATED(TRANSFR_FUEL_COST_AMOUNT)) then
            DEALLOCATE(TRANSFR_FUEL_COST_AMOUNT,TRANSFR_VAR_COST_AMOUNT, &
...
!
            IF(ASSET_ALLOCATION_LIST(CLASS_POINTER) < 0.) THEN
               ...
               IF(MONTHLY_MIDAS_ACTIVE) THEN
                  CALL GET_MONTHLY_ANNUAL_VALUES(ALLOCATION_VECTOR,DATA_TYPE, &
                    VECTOR_TYPE,ANNUAL_VECTOR_VALUES(1), &
...
!                     
            ...
            DO MO = 1, 12
               ALLOCATED_FUEL_DOLLAR_AMOUNT(MO) = ALLOCATION_VALUE(MO) * &
                TRANSFR_MONTHLY_WHOLE_FUEL_COST(MO)
               ALLOCATED_FUEL_DOLLAR_AMOUNT(0) = &
                ALLOCATED_FUEL_DOLLAR_AMOUNT(0) + ALLOCATED_FUEL_DOLLAR_AMOUNT(MO)
               ALLOCATED_VOM_DOLLAR_AMOUNT(MO) = ALLOCATION_VALUE(MO) * &
                TRANSFR_MONTHLY_WHOLE_VOM_COST(MO)
               ALLOCATED_VOM_DOLLAR_AMOUNT(0) = ALLOCATED_VOM_DOLLAR_AMOUNT(0) + &
                ALLOCATED_VOM_DOLLAR_AMOUNT(MO)
               ALLOCATED_QUANT_AMOUNT(MO) = ALLOCATION_VALUE(MO) * &
                TRANSFR_MARKET_COSTS_MWH(MO)
               ALLOCATED_QUANT_AMOUNT(0) = ALLOCATED_QUANT_AMOUNT(0) + &
                ALLOCATED_QUANT_AMOUNT(MO)
            ENDDO
            IF(TRANSFR_COST_CLASS_POINTR(ASSET_CLASS_loc) == 0) THEN
               TRANSFR_COST_CLASS_POINTR(ASSET_CLASS_loc) = CLASS_POINTER
               RESULT_PTR = CLASS_POINTER
            ELSE
               RESULT_PTR = TRANSFR_COST_CLASS_POINTR(ASSET_CLASS_loc)
            ENDIF
            DO MO = 0, 12
               TRANSFR_FUEL_COST_AMOUNT(MO,RESULT_PTR) = &
                ALLOCATED_FUEL_DOLLAR_AMOUNT(MO)
               TRANSFR_VAR_COST_AMOUNT(MO,RESULT_PTR) = &
                ALLOCATED_VOM_DOLLAR_AMOUNT(MO)
               TRANSFR_COST_QUANTITIES(MO,RESULT_PTR) = ALLOCATED_QUANT_AMOUNT(MO)
            ENDDO
            TRANSFR_FUEL_COST_AMOUNT(0,RESULT_PTR) = ALLOCATION_VALUE(0) * &
                TOTAL_AMEREN_FUEL_EXPENSE
            TRANSFR_VAR_COST_AMOUNT(0,RESULT_PTR) = ALLOCATION_VALUE(0) * &
...
!
         VOID_LOGICAL = RETURN_ASSET_CLASS_LISTS(TRANSFR_REV_CLASS_VECTOR, &
            ASSET_CLASS_LIST,TRANSFR_REV_ALLOC_VECTOR, &
                                                 ASSET_ALLOCATION_LIST)
         MAX_REV_CLASS_NUMBER = 0
         CLASS_POINTER = 1
         DO 
            MAX_REV_CLASS_NUMBER = MAX(MAX_REV_CLASS_NUMBER, &
                int(ASSET_CLASS_LIST(CLASS_POINTER),2)+int(1,2))
         ...
         IF(ALLOCATED(TRANSFR_REV_AMOUNT)) DEALLOCATE(TRANSFR_REV_AMOUNT, &
            TRANSFR_REV_QUANTITIES,TRANSFR_REV_CLASS_POINTR)
         ALLOCATE(TRANSFR_REV_AMOUNT(0:12,NUMBER_OF_REV_CLASSES), &
            TRANSFR_REV_QUANTITIES(0:12,NUMBER_OF_REV_CLASSES), &
...
!
                  ...
                  ENDDO
               ELSE
                  CALL GET_ASSET_VAR(ALLOCATION_VECTOR,DUMMY_TYPE, &
...
!                     
            ...
            ALLOCATED_QUANT_AMOUNT(0) = 0.
            DO MO = 1, 12
               ALLOCATED_DOLLAR_AMOUNT(MO) = ALLOCATION_VALUE(MO) * &
                TRANSFR_MARKET_REVS(MO)
               ALLOCATED_DOLLAR_AMOUNT(0) = ALLOCATED_DOLLAR_AMOUNT(0) + &
                ALLOCATED_DOLLAR_AMOUNT(MO)
               ALLOCATED_QUANT_AMOUNT(MO) = ALLOCATION_VALUE(MO) * &
                TRANSFR_MARKET_REVS_MWH(MO)
               ALLOCATED_QUANT_AMOUNT(0) = ALLOCATED_QUANT_AMOUNT(0) + &
                ALLOCATED_QUANT_AMOUNT(MO)
            ENDDO
            IF(TRANSFR_REV_CLASS_POINTR(ASSET_CLASS_loc) == 0) THEN
               TRANSFR_REV_CLASS_POINTR(ASSET_CLASS_loc) = CLASS_POINTER
               RESULT_PTR = CLASS_POINTER
            ELSE
               RESULT_PTR = TRANSFR_REV_CLASS_POINTR(ASSET_CLASS_loc)
            ENDIF
            DO MO = 0, 12
            ...
            ENDDO
            TRANSFR_REV_AMOUNT(0,RESULT_PTR) = ALLOCATION_VALUE(0) * &
...
!
      ENTRY RETURN_CLASS_TRANSFR_AMOUNTS(R_ASSET_CLASS, &
        R_REV_TRANSFERS_ARE_ACTIVE,R_EXP_TRANSFERS_ARE_ACTIVE,R_CLASS_REVENUES, &
        R_CLASS_REV_QUANITIES,R_CLASS_FUEL_EXPENSES,R_CLASS_VOM_EXPENSES, &
        R_CLASS_EXP_QUANITIES)
...
!
      ...
      REAL (KIND=4), PARAMETER :: NOT_AVAIL=-999999.
      REAL (KIND=4) :: GOPINC,ITC_ITCAMT,TAXES_PAID,INTINC,OTHINC, &
        BTL_TAXES,RTEMP,LTD_STD_INTEREST,LTD_PS_RETIREMENT, &
        COVERAGE_DRIVER_RATIO,LEASE_PAYMENT_LIMIT,DEP_AMORTIZATION,NOPINC, &
        COVERAGE_RATIO_VALUE, &
        FUNDS_FOR_CONSTRUCTION,SUM_OPERATIONS,INTEREST,AFUDC_ON_CASH, &
        SRP_TOTAL_DEBT_SERVICE, &
...
!                                LTD BOOKED INTEREST WITH AMORT
         ...
         ELSEIF(COVERAGE_RATIO == 2 .OR. COVERAGE_RATIO == 3) THEN
            RTEMP = GOPINC+ITC_ITCAMT+TAXES_PAID+INTINC+OTHINC-BTL_TAXES - &
                R_VARIABLE_VALUE(35)   ! CLASS_BTL_EXPENSES

            IF(COVERAGE_RATIO == 2) then
                LTD_STD_INTEREST = LTD_STD_INTEREST - AFUDC_ON_CASH
            end if
            IF(LTD_STD_INTEREST /= 0.) then
                COVERAGE_DRIVER_RATIO = RTEMP/LTD_STD_INTEREST
            end if
...
! 031207.
          ELSEIF(COVERAGE_RATIO == 13 .OR. COVERAGE_RATIO == 16 .OR. &
            COVERAGE_RATIO == 14 .OR. COVERAGE_RATIO == 17) THEN
